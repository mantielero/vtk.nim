...
## !!!Ignored construct:  # vtkOStreamWrapper_h [NewLine] # vtkOStreamWrapper_h [NewLine] # vtkCommonCoreModule.h [NewLine] # VTK_SYSTEM_INCLUDES_INSIDE [NewLine] Do_not_include_vtkOStreamWrapper_directly_vtkSystemIncludes_includes_it ;
## Error: token expected: ; but got: [identifier]!!!

discard "forward decl of vtkIndent"
discard "forward decl of vtkObjectBase"
discard "forward decl of vtkLargeInteger"
discard "forward decl of vtkSmartPointer"
discard "forward decl of vtkSmartPointerBase"
discard "forward decl of vtkStdString"
## !!!Ignored construct:  class VTKCOMMONCORE_EXPORT VTK_WRAPEXCLUDE vtkOStreamWrapper { class std_string ; public : /@{ *
##  Construct class to reference a real ostream.  All methods and
##  operators will be forwarded.
##  vtkOStreamWrapper ( ostream & os ) ; vtkOStreamWrapper ( vtkOStreamWrapper & r ) ; /@} virtual ~ vtkOStreamWrapper ( ) ; *
##  Type for a fake endl.
##  struct EndlType { } ; /@{ *
##  Forward this output operator to the real ostream.
##  vtkOStreamWrapper & operator << ( const EndlType & ) ; vtkOStreamWrapper & operator << ( const vtkIndent & ) ; vtkOStreamWrapper & operator << ( vtkObjectBase & ) ; vtkOStreamWrapper & operator << ( const vtkLargeInteger & ) ; vtkOStreamWrapper & operator << ( const vtkSmartPointerBase & ) ; vtkOStreamWrapper & operator << ( const vtkStdString & ) ; vtkOStreamWrapper & operator << ( const char * ) ; vtkOStreamWrapper & operator << ( void * ) ; vtkOStreamWrapper & operator << ( char ) ; vtkOStreamWrapper & operator << ( short ) ; vtkOStreamWrapper & operator << ( int ) ; vtkOStreamWrapper & operator << ( long ) ; vtkOStreamWrapper & operator << ( long long ) ; vtkOStreamWrapper & operator << ( unsigned char ) ; vtkOStreamWrapper & operator << ( unsigned short ) ; vtkOStreamWrapper & operator << ( unsigned int ) ; vtkOStreamWrapper & operator << ( unsigned long ) ; vtkOStreamWrapper & operator << ( unsigned long long ) ; vtkOStreamWrapper & operator << ( float ) ; vtkOStreamWrapper & operator << ( double ) ; vtkOStreamWrapper & operator << ( bool ) ; /@}  Work-around for IBM Visual Age bug in overload resolution. # defined ( __IBMCPP__ ) [NewLine] vtkOStreamWrapper & WriteInternal ( const char * ) ; vtkOStreamWrapper & WriteInternal ( void * ) ; template < typename T > vtkOStreamWrapper & operator << ( T * p ) { return this -> WriteInternal ( p ) ; } # [NewLine] vtkOStreamWrapper & operator << ( void ( * ) ( void * ) ) ; vtkOStreamWrapper & operator << ( void * ( * ) ( void * ) ) ; vtkOStreamWrapper & operator << ( int ( * ) ( void * ) ) ; vtkOStreamWrapper & operator << ( int * ( * ) ( void * ) ) ; vtkOStreamWrapper & operator << ( float * ( * ) ( void * ) ) ; vtkOStreamWrapper & operator << ( const char * ( * ) ( void * ) ) ; vtkOStreamWrapper & operator << ( void ( * ) ( void * , int * ) ) ;  Accept std::string without a declaration. template < template < typename , typename , typename > class S > vtkOStreamWrapper & operator << ( const S < char , std :: char_traits < char > , std :: allocator < char >> & s ) { return * this << reinterpret_cast < std_string const & > ( s ) ; }  Accept vtkSmartPointer for output. template < typename T > vtkOStreamWrapper & operator << ( const vtkSmartPointer < T > & ptr ) { this -> ostr << ( static_cast < T * > ( ptr ) ) ; return * this ; } *
...
##  or may not be used.
##  static void UseEndl ( const EndlType & ) { } /@} protected :  Reference to the real ostream. ostream & ostr ; private : vtkOStreamWrapper & operator = ( const vtkOStreamWrapper & r ) = delete ; vtkOStreamWrapper & operator << ( std_string const & ) ; } ;
## Error: token expected: ; but got: [identifier]!!!
