...
import
  vtkMeta, vtkRangeIterableTraits
...
## !!!Ignored construct:  template < typename IterablePtr , typename ... Options > [end of template] auto Range ( IterablePtr iterable , Options && ... opts ) -> typename detail :: IterableTraits < typename detail :: StripPointers < IterablePtr > :: type > :: RangeType { using Iterable = typename detail :: StripPointers < IterablePtr > :: type ; using RangeType = typename detail :: IterableTraits < Iterable > :: RangeType ; return RangeType { iterable , std :: forward < Options > ( opts ) ... } ; } }
