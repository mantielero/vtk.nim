...
## !!!Ignored construct:  # vtkProcessGroup_h [NewLine] # vtkProcessGroup_h [NewLine] # vtkObject.h [NewLine] # vtkParallelCoreModule.h  For export macro [NewLine] class vtkMultiProcessController ;
## Error: token expected: ; but got: [identifier]!!!

discard "forward decl of vtkCommunicator"
## !!!Ignored construct:  class VTKPARALLELCORE_EXPORT vtkProcessGroup : public vtkObject { public : protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkObject Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkObject :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkProcessGroup :: IsTypeOf ( type ) ; } static vtkProcessGroup * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkProcessGroup * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkProcessGroup * NewInstance ( ) const { return vtkProcessGroup :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkObject :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkProcessGroup :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkProcessGroup :: New ( ) ; } public : ; static vtkProcessGroup * New ( ) ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; /@{ *
...
##  in the same order.
##  void Initialize ( vtkMultiProcessController * controller ) ; void Initialize ( vtkCommunicator * communicator ) ; /@} /@{ *
##  Get the communicator on which this group is based on.
##  virtual vtkCommunicator * GetnameCommunicator ( ) { vtkDebugWithObjectMacro ( this , <<  returning  Communicator  address  << static_cast < vtkCommunicator * > ( this -> Communicator ) ) ; return this -> Communicator ; } ; /@} *
...
##  virtual int GetNumberOfProcessIds ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << NumberOfProcessIds  of  << this -> NumberOfProcessIds ) ; return this -> NumberOfProcessIds ; } ; /@} *
...
##  void Copy ( vtkProcessGroup * group ) ; protected : vtkProcessGroup ( ) ; ~ vtkProcessGroup ( ) override ; int * ProcessIds ; int NumberOfProcessIds ; vtkCommunicator * Communicator ; private : vtkProcessGroup ( const vtkProcessGroup & ) = delete ; void operator = ( const vtkProcessGroup & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
