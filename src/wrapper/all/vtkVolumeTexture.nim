...
## !!!Ignored construct:  # vtkVolumeTexture_h [NewLine] # vtkVolumeTexture_h [NewLine] # < map >  For ImageDataBlockMap # < vector >  For ImageDataBlocks [NewLine] # vtkMatrix4x4.h  For vtkMatrix4 # vtkNew.h  For vtkNew # vtkObject.h [NewLine] # vtkRenderingVolumeOpenGL2Module.h  For export macro # vtkSmartPointer.h  For SmartPointer # vtkTimeStamp.h  For UploadTime # vtkTuple.h  For Size6 and Size3 [NewLine] class vtkDataArray ;
...
## !!!Ignored construct:  class VTKRENDERINGVOLUMEOPENGL2_EXPORT vtkVolumeTexture : public vtkObject { typedef vtkTuple < int , 6 > Size6 ; typedef vtkTuple < int , 3 > Size3 ; public : static vtkVolumeTexture * New ( ) ; struct VolumeBlock { VolumeBlock ( vtkDataSet * dataset , vtkTextureObject * tex , Size3 const & texSize ) {  Block extent is stored in vtkDataSet DataSet = dataset ; TextureObject = tex ; TextureSize = texSize ; TupleIndex = 0 ; this -> Extents [ 0 ] = VTK_INT_MAX ; this -> Extents [ 1 ] = VTK_INT_MIN ; this -> Extents [ 2 ] = VTK_INT_MAX ; this -> Extents [ 3 ] = VTK_INT_MIN ; this -> Extents [ 4 ] = VTK_INT_MAX ; this -> Extents [ 5 ] = VTK_INT_MIN ; } vtkDataSet * DataSet ; vtkTextureObject * TextureObject ; Size3 TextureSize ; vtkIdType TupleIndex ; vtkNew < vtkMatrix4x4 > TextureToDataset ; vtkNew < vtkMatrix4x4 > TextureToDatasetInv ; float CellStep [ 3 ] ; double DatasetStepSize [ 3 ] ; *
##  LoadedBounds are corrected for cell-data (if that is the case). So they
##  are not equivalent to vtkDataSet::GetBounds().
##  double LoadedBounds [ 6 ] ; double LoadedBoundsAA [ 6 ] ; double VolumeGeometry [ 24 ] ; int Extents [ 6 ] ; } ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkObject Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkObject :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkVolumeTexture :: IsTypeOf ( type ) ; } static vtkVolumeTexture * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkVolumeTexture * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkVolumeTexture * NewInstance ( ) const { return vtkVolumeTexture :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkObject :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkVolumeTexture :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkVolumeTexture :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; *
##   Set a number of blocks per axis.
##  void SetPartitions ( int const x , int const y , int const z ) ; const Size3 & GetPartitions ( ) ; *
...
##  static void GetScaleAndBias ( const int scalarType , float * scalarRange , float & scale , float & bias ) ; vtkDataArray * GetLoadedScalars ( ) ; bool HandleLargeDataTypes ; float Scale [ 4 ] ; float Bias [ 4 ] ; float ScalarRange [ 4 ] [ 2 ] ; float CellSpacing [ 3 ] ; int InterpolationType ; vtkTimeStamp UploadTime ; int IsCellData = 0 ; vtkNew < vtkMatrix4x4 > CellToPointMatrix ; float AdjustedTexMin [ 4 ] ; float AdjustedTexMax [ 4 ] ; vtkSmartPointer < vtkTextureObject > CoordsTex ; int CoordsTexSizes [ 3 ] ; float CoordsScale [ 3 ] ; float CoordsBias [ 3 ] ; vtkSmartPointer < vtkTextureObject > BlankingTex ; protected : vtkVolumeTexture ( ) ; ~ vtkVolumeTexture ( ) override ; private : vtkVolumeTexture ( const vtkVolumeTexture & ) = delete ; void operator = ( const vtkVolumeTexture & ) = delete ; *
...
##  void SplitVolume ( vtkImageData * imageData , Size3 const & part ) ; void CreateBlocks ( unsigned int const format , unsigned int const internalFormat , int const type ) ; void AdjustExtentForCell ( Size6 & extent ) ; Size3 ComputeBlockSize ( int * extent ) ; *
...
##  void SelectTextureFormat ( unsigned int & format , unsigned int & internalFormat , int & type , int const scalarType , int const noOfComponents ) ; *
##  Clean-up any acquired host side resources (image blocks, etc.).
##  void ClearBlocks ( ) ; *
##  Computes loaded bounds in data-coordinates.
##   can be combined into one call void ComputeBounds ( VolumeBlock * block ) ; void UpdateTextureToDataMatrix ( VolumeBlock * block ) ; *
...
##  bool AreDimensionsValid ( vtkTextureObject * texture , int const width , int const height , int const depth ) ; bool SafeLoadTexture ( vtkTextureObject * texture , int const width , int const height , int const depth , int numComps , int dataType , void * dataPtr ) ; /@} void UpdateInterpolationType ( int const interpolation ) ; void SetInterpolation ( int const interpolation ) ; ---------------------------------------------------------------------------- vtkTimeStamp UpdateTime ; vtkSmartPointer < vtkTextureObject > Texture ; std :: vector < vtkDataSet * > ImageDataBlocks ; std :: map < vtkDataSet * , VolumeBlock * > ImageDataBlockMap ; std :: vector < VolumeBlock * > SortedVolumeBlocks ; size_t CurrentBlockIdx ; bool StreamBlocks ; std :: vector < Size3 > TextureSizes ; Size6 FullExtent ; Size3 FullSize ; Size3 Partitions ; vtkDataArray * Scalars ; } ;
## Error: token expected: ; but got: [identifier]!!!
