...
## !!!Ignored construct:  # vtkStreamingDemandDrivenPipeline_h [NewLine] # vtkStreamingDemandDrivenPipeline_h [NewLine] # vtkCommonExecutionModelModule.h  For export macro # vtkDemandDrivenPipeline.h [NewLine] # VTK_UPDATE_EXTENT_COMBINE 1 [NewLine] # VTK_UPDATE_EXTENT_REPLACE 2 [NewLine] class vtkInformationDoubleKey ;
...
discard "forward decl of vtkInformationIterator"
...
## !!!Ignored construct:  class VTKCOMMONEXECUTIONMODEL_EXPORT vtkStreamingDemandDrivenPipeline : public vtkDemandDrivenPipeline { public : static vtkStreamingDemandDrivenPipeline * New ( ) ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkDemandDrivenPipeline Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkDemandDrivenPipeline :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkStreamingDemandDrivenPipeline :: IsTypeOf ( type ) ; } static vtkStreamingDemandDrivenPipeline * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkStreamingDemandDrivenPipeline * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkStreamingDemandDrivenPipeline * NewInstance ( ) const { return vtkStreamingDemandDrivenPipeline :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkDemandDrivenPipeline :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkStreamingDemandDrivenPipeline :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkStreamingDemandDrivenPipeline :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; *
##  Generalized interface for asking the executive to fulfill update
##  requests.
##  vtkTypeBool ProcessRequest ( vtkInformation * request , vtkInformationVector * * inInfo , vtkInformationVector * outInfo ) override ; /@{ *
##  Bring the outputs up-to-date.
##  vtkTypeBool Update ( ) override ; vtkTypeBool Update ( int port ) override ; virtual vtkTypeBool UpdateWholeExtent ( ) ; /@} *
...
##  int PropagateTime ( int outputPort ) ; int UpdateTimeDependentInformation ( int outputPort ) ; /@} /@{ *
...
##  static int SetWholeExtent ( vtkInformation * , int extent [ 6 ] ) ; static void GetWholeExtent ( vtkInformation * , int extent [ 6 ] ) ; static int * GetWholeExtent ( vtkInformation * ) VTK_SIZEHINT ( 6 ) ; /@} /@{ *
...
##  int SetRequestExactExtent ( int port , int flag ) ; int GetRequestExactExtent ( int port ) ; /@} *
...
##  static void GetUpdateExtent ( vtkInformation * , int extent [ 6 ] ) ; static int * GetUpdateExtent ( vtkInformation * ) ; /@}
## /@{ *
##  Set/Get the update piece, update number of pieces, and update
##  number of ghost levels for an output port.  Similar to update
##  extent in 3D.
##  static int GetUpdatePiece ( vtkInformation * ) ; static int GetUpdateNumberOfPieces ( vtkInformation * ) ; static int GetUpdateGhostLevel ( vtkInformation * ) ; /@} protected : vtkStreamingDemandDrivenPipeline ( ) ; ~ vtkStreamingDemandDrivenPipeline ( ) override ; *
...
##  \ingroup InformationKeys
##  static vtkInformationDoubleKey * PREVIOUS_UPDATE_TIME_STEP ( ) ;  Does the time request correspond to what is in the data?
##  Returns 0 if yes, 1 otherwise. virtual int NeedToExecuteBasedOnTime ( vtkInformation * outInfo , vtkDataObject * dataObject ) ;  Setup default information on the output after the algorithm
##  executes information. int ExecuteInformation ( vtkInformation * request , vtkInformationVector * * inInfoVec , vtkInformationVector * outInfoVec ) override ;  Copy information for the given request. void CopyDefaultInformation ( vtkInformation * request , int direction , vtkInformationVector * * inInfoVec , vtkInformationVector * outInfoVec ) override ;  Helper to check output information before propagating it to inputs. virtual int VerifyOutputInformation ( int outputPort , vtkInformationVector * * inInfoVec , vtkInformationVector * outInfoVec ) ;  Override this check to account for update extent. int NeedToExecuteData ( int outputPort , vtkInformationVector * * inInfoVec , vtkInformationVector * outInfoVec ) override ;  Override these to handle the continue-executing option. void ExecuteDataStart ( vtkInformation * request , vtkInformationVector * * inInfoVec , vtkInformationVector * outInfoVec ) override ; void ExecuteDataEnd ( vtkInformation * request , vtkInformationVector * * inInfoVec , vtkInformationVector * outInfoVec ) override ;  Override this to handle cropping and ghost levels. void MarkOutputsGenerated ( vtkInformation * request , vtkInformationVector * * inInfoVec , vtkInformationVector * outInfoVec ) override ;  Remove update/whole extent when resetting pipeline information. void ResetPipelineInformation ( int port , vtkInformation * ) override ;  Flag for when an algorithm returns with CONTINUE_EXECUTING in the
##  request. int ContinueExecuting ; vtkInformation * UpdateExtentRequest ; vtkInformation * UpdateTimeRequest ; vtkInformation * TimeDependentInformationRequest ; vtkInformationIterator * InformationIterator ;  did the most recent PUE do anything ? int LastPropogateUpdateExtentShortCircuited ; private : vtkStreamingDemandDrivenPipeline ( const vtkStreamingDemandDrivenPipeline & ) = delete ; void operator = ( const vtkStreamingDemandDrivenPipeline & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
