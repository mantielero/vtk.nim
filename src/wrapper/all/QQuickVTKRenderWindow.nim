...
## !!!Ignored construct:  # QQuickVTKRenderWindow_h [NewLine] # QQuickVTKRenderWindow_h [NewLine]  vtk includes # vtkSmartPointer.h  For vtkSmartPointer [NewLine]  Qt includes # < QOpenGLFunctions >  For QOpenGLFunctions # < QPointer >  For QPointer # < QQuickItem > [NewLine] # vtkGUISupportQtQuickModule.h  for export macro [NewLine]  Forward declarations class QEvent ;
## Error: token expected: ; but got: [identifier]!!!

discard "forward decl of QQuickVTKInteractorAdapter"
...
discard "forward decl of vtkWindowToImageFilter"
## !!!Ignored construct:  class VTKGUISUPPORTQTQUICK_EXPORT QQuickVTKRenderWindow : public QQuickItem , protected QOpenGLFunctions { Q_OBJECT typedef QQuickItem Superclass ; public : *
##  Constructor
...
##  time.
##  virtual void setRenderWindow ( vtkRenderWindow * renWin ) ; virtual void setRenderWindow ( vtkGenericOpenGLRenderWindow * renWin ) ; vtkRenderWindow * renderWindow ( ) const ; /@} *
...
##  virtual vtkSmartPointer < vtkImageData > captureScreenshot ( ) ; virtual vtkSmartPointer < vtkImageData > captureScreenshot ( double * viewport ) ; /@} *
...
##  virtual bool isInitialized ( ) const ; public Q_SLOTS : *
...
##  \sa renderNow()
##  virtual void render ( ) ; protected Q_SLOTS : virtual void handleWindowChanged ( QQuickWindow * w ) ; protected : QPointer < QQuickVTKInteractorAdapter > m_interactorAdapter ; vtkSmartPointer < vtkGenericOpenGLRenderWindow > m_renderWindow ; bool m_initialized = false ;  Screenshot stuff bool m_screenshotScheduled = false ; vtkNew < vtkWindowToImageFilter > m_screenshotFilter ; vtkNew < vtkRenderer > m_dummyRenderer ;  Event handlers # QT_VERSION < QT_VERSION_CHECK ( 6 , 0 , 0 ) [NewLine] void geometryChanged ( const QRectF & newGeometry , const QRectF & oldGeometry ) override ; # [NewLine] void geometryChange ( const QRectF & newGeometry , const QRectF & oldGeometry ) override ; # [NewLine] *
##  Check the scenegraph backend and graphics API being used.
##  bool checkGraphicsBackend ( ) ; private : QQuickVTKRenderWindow ( const QQuickVTKRenderWindow & ) = delete ; void operator = ( const QQuickVTKRenderWindow ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
