...
## !!!Ignored construct:  # vtkImageStencilIterator_h [NewLine] # vtkImageStencilIterator_h [NewLine] # vtkImagePointDataIterator.h [NewLine] template < class DType > [end of template] class VTKIMAGINGCORE_EXPORT vtkImageStencilIterator : public vtkImagePointDataIterator { public : /@{ *
##  Default constructor, its use must be followed by Initialize().
##  vtkImageStencilIterator ( ) { this -> Increment = 0 ; this -> BasePointer = nullptr ; this -> Pointer = nullptr ; this -> SpanEndPointer = nullptr ; } /@} /@{ *
...
##  void NextSpan ( ) { this -> vtkImagePointDataIterator :: NextSpan ( ) ; this -> UpdatePointer ( ) ; } /@} *
...
##  DType * EndSpan ( ) { return this -> SpanEndPointer ; } protected : /@{ *
##  Update the pointer (called automatically when a new span begins).
##  void UpdatePointer ( ) { this -> Pointer = this -> BasePointer + this -> Id * this -> Increment ; this -> SpanEndPointer = this -> BasePointer + this -> SpanEnd * this -> Increment ; } /@}  The pointer must be incremented by this amount for each pixel. int Increment ;  Pointers DType * BasePointer ;  pointer to the first voxel DType * Pointer ;  current iterator position within data DType * SpanEndPointer ;  end of current span } ;
## Error: token expected: ; but got: [identifier]!!!

when not defined(vtkImageStencilIterator_cxx):
  ...
  vtkExternTemplateMacro(extern, `template`, class, VTKIMAGINGCORE_EXPORT,
                         vtkImageStencilIterator)
  when defined(_MSC_VER):
    discard
