...
## !!!Ignored construct:  # vtkDIYGhostUtilities_h [NewLine] # vtkDIYGhostUtilities_h [NewLine] # vtkBoundingBox.h  For ComputeLinkMap # vtkDIYExplicitAssigner.h  For DIY assigner # vtkDIYUtilities.h  For Block # vtkObject.h [NewLine] # vtkParallelDIYModule.h  For export macros # vtkQuaternion.h  For vtkImageData # vtkSmartPointer.h  For vtkSmartPointer [NewLine] # < array >  For VectorType and ExtentType # < map >  For BlockMapType # < set >  For Link # < vector >  For LinkMap [NewLine]  clang-format off # vtk_diy2.h  Third party include # VTK_DIY2 ( diy / assigner . hpp ) [NewLine] # VTK_DIY2 ( diy / master . hpp ) [NewLine] # VTK_DIY2 ( diy / partners / all - reduce . hpp ) [NewLine]  clang-format on class vtkAbstractPointLocator ;
...
## !!!Ignored construct:  class VTKPARALLELDIY_EXPORT vtkDIYGhostUtilities : public vtkObject { public : protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkObject Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkObject :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkDIYGhostUtilities :: IsTypeOf ( type ) ; } static vtkDIYGhostUtilities * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkDIYGhostUtilities * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkDIYGhostUtilities * NewInstance ( ) const { return vtkDIYGhostUtilities :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkObject :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkDIYGhostUtilities :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkDIYGhostUtilities :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; /@{ *
##  Convenient typedefs.
##  using VectorType = std :: array < double , 3 > ; using QuaternionType = vtkQuaternion < double > ; using ExtentType = std :: array < int , 6 > ; template < class T > using BlockMapType = std :: map < int , T > ; using Links = std :: set < int > ; using LinkMap = std :: vector < Links > ; /@} *
...
##  template < class DataSetT > struct DataSetTypeToBlockTypeConverter ; protected : *
##  Base block structure for data sets.
##  struct DataSetBlockStructure { vtkSmartPointer < vtkFieldData > GhostCellData = nullptr ; vtkSmartPointer < vtkFieldData > GhostPointData = nullptr ; } ; struct DataSetInformation { *
...
##  ExtentType ShiftedExtent ; ExtentType ReceivedGhostExtent = ExtentType { 1 , - 1 , 1 , - 1 , 1 , - 1 } ; *
...
##  struct GridInformation : public DataSetInformation { bool InputNeedsGhostsPeeledOff ( ) const override { return this -> Extent != this -> InputExtent ; } *
##  Extent without ghost layers.
##  ExtentType Extent = ExtentType { 1 , - 1 , 1 , - 1 , 1 , - 1 } ; *
##  Input extent without any modification.
##  ExtentType InputExtent = ExtentType { 1 , - 1 , 1 , - 1 , 1 , - 1 } ; ExtentType ExtentGhostThickness ; } ; struct ImageDataInformation : public GridInformation { vtkImageData * Input ; } ; *
...
##  ImageDataBlockStructure ( const int extent [ 6 ] , int dim , const double origin [ 3 ] , const double spacing [ 3 ] , const double orientationQuaternion [ 4 ] ) ; ImageDataBlockStructure ( const int extent [ 6 ] , int dim , const double origin [ 3 ] , const double spacing [ 3 ] , vtkMatrix3x3 * directionMatrix ) ; /@} *
##  Copy constructor.
...
##  QuaternionType OrientationQuaternion ; } ; struct RectilinearGridInformation : public GridInformation { /@{ *
##  Point coordinates without ghosts.
##  vtkSmartPointer < vtkDataArray > XCoordinates ; vtkSmartPointer < vtkDataArray > YCoordinates ; vtkSmartPointer < vtkDataArray > ZCoordinates ; /@} *
...
##  vtkSmartPointer < vtkDataArray > CoordinateGhosts [ 6 ] ; vtkRectilinearGrid * Input ; } ; *
##  Block structure storing information being communicated by neighboring blocks for
##  `vtkRectilinearGrid`.
...
##  vtkSmartPointer < vtkDataArray > XCoordinates ; vtkSmartPointer < vtkDataArray > YCoordinates ; vtkSmartPointer < vtkDataArray > ZCoordinates ; /@} } ; struct StructuredGridInformation : public GridInformation { *
...
##  vtkPoints * InputPoints ; vtkStructuredGrid * Input ; } ; *
##  Block structure storing information being communicated by neighboring blocks for
##  `vtkStructuredGrid`.
...
##  vtkNew < vtkPoints > GhostPoints ; } ; struct UnstructuredDataInformation { bool InputNeedsGhostsPeeledOff ( ) const { return this -> OutputToInputCellIdRedirectionMap != nullptr ; } ; *
...
##  the output (with input ghosts peeled off).
##  vtkSmartPointer < vtkIdList > InputToOutputPointIdRedirectionMap = nullptr ; vtkSmartPointer < vtkIdList > OutputToInputPointIdRedirectionMap = nullptr ; /@} *
...
##  vtkIdType CurrentMaxPointId ; vtkIdType CurrentMaxCellId ; /@} /@{ *
##  Number of input points / cell in the input when ghosts are removed.
##  vtkIdType NumberOfInputPoints ; vtkIdType NumberOfInputCells ; /@} } ; struct UnstructuredDataBlockStructure : public DataSetBlockStructure { *
...
##  vtkNew < vtkIdTypeArray > SharedPointIds ; vtkSmartPointer < vtkIdTypeArray > ReceivedSharedPointIds ; /@} *
...
##  vtkSmartPointer < vtkFieldData > InterfacingPointData ; } ; struct UnstructuredGridInformation : public UnstructuredDataInformation { *
...
##  vtkIdType CurrentConnectivitySize = 0 ; vtkIdTypeArray * Faces = nullptr ; vtkIdTypeArray * FaceLocations = nullptr ; vtkUnstructuredGrid * Input ; *
...
##  vtkIdType InputFacesSize = 0 ; } ; struct UnstructuredGridBlockStructure : public UnstructuredDataBlockStructure { *
##  Topology information for cells to be exchanged.
##  struct TopologyBufferType { vtkSmartPointer < vtkUnsignedCharArray > Types = nullptr ; vtkSmartPointer < vtkIdTypeArray > Faces = nullptr ; vtkSmartPointer < vtkIdTypeArray > FaceLocations = nullptr ; vtkNew < vtkCellArray > CellArray ; } ; TopologyBufferType SendBuffer ; TopologyBufferType ReceiveBuffer ; /@{ *
##  Handle to the faces / connectivity size that we have to send to the neighboring block.
##  vtkIdType FacesSize = 0 ; vtkIdType ConnectivitySize = 0 ; /@} } ; struct PolyDataInformation : public UnstructuredDataInformation { vtkPolyData * Input ; /@{ *
##  This is a cursor telling how much the corresponding output connectivity array is filled.
##  This is used at the very end of the pipeline.
##  vtkIdType CurrentPolyConnectivitySize ; vtkIdType CurrentStripConnectivitySize ; vtkIdType CurrentLineConnectivitySize ; /@} /@{ *
##  This is a cursor telling how many cells of corresponding types have been added so far.
##  This is used at the very end of the pipeline.
##  vtkIdType CurrentMaxPolyId = 0 ; vtkIdType CurrentMaxStripId = 0 ; vtkIdType CurrentMaxLineId = 0 ; /@} /@{ *
##  Number of cells of respective type when the input has its ghost cells removed
##  vtkIdType NumberOfInputVerts ; vtkIdType NumberOfInputPolys ; vtkIdType NumberOfInputStrips ; vtkIdType NumberOfInputLines ; /@} /@{ *
##  Cell connectivity array size of the input if ghost cells are removed.
##  vtkIdType InputVertConnectivitySize ; vtkIdType InputPolyConnectivitySize ; vtkIdType InputStripConnectivitySize ; vtkIdType InputLineConnectivitySize ; /@{ /@{ *
##  In the event that the input has ghost cells, this maps the output cells (with input ghosts
##  removed) to the input cells.
##  vtkNew < vtkIdList > OutputToInputVertCellIdRedirectionMap ; vtkNew < vtkIdList > OutputToInputLineCellIdRedirectionMap ; vtkNew < vtkIdList > OutputToInputPolyCellIdRedirectionMap ; vtkNew < vtkIdList > OutputToInputStripCellIdRedirectionMap ; /@} } ; struct PolyDataBlockStructure : public UnstructuredDataBlockStructure { /@{ *
##  This lists the ids of the cells that we own and need to send to the current neighboring
##  block.
##  vtkNew < vtkIdList > PolyIdsToSend ; vtkNew < vtkIdList > StripIdsToSend ; vtkNew < vtkIdList > LineIdsToSend ; /@} struct TopologyBufferType { vtkNew < vtkCellArray > Polys ; vtkNew < vtkCellArray > Strips ; vtkNew < vtkCellArray > Lines ; } ; TopologyBufferType SendBuffer ; TopologyBufferType ReceiveBuffer ; /@{ *
##  Handle on the number of cells to send of the corresponding type.
##  vtkIdType NumberOfPolysToSend = 0 ; vtkIdType NumberOfStripsToSend = 0 ; vtkIdType NumberOfLinesToSend = 0 ; /@} /@{ *
##  Handle on the number of cells of corresponding type to be sent to the neighbor block.
##  vtkIdType PolyConnectivitySize = 0 ; vtkIdType StripConnectivitySize = 0 ; vtkIdType LineConnectivitySize = 0 ; /@} } ; public : *
...
##  template < class BlockStructureT , class InformationT > struct Block { /@{ *
##  Typedef handle on block structure and block information
##  typedef BlockStructureT BlockStructureType ; typedef InformationT InformationType ; /@} *
...
##  InformationType Information ; BlockMapType < vtkBoundingBox > NeighborBoundingBoxes ; vtkBoundingBox BoundingBox ; vtkSmartPointer < vtkUnsignedCharArray > GhostCellArray ; vtkSmartPointer < vtkUnsignedCharArray > GhostPointArray ; } ; /@{ *
##  Block typedefs.
##  using ImageDataBlock = Block < ImageDataBlockStructure , ImageDataInformation > ; using RectilinearGridBlock = Block < RectilinearGridBlockStructure , RectilinearGridInformation > ; using StructuredGridBlock = Block < StructuredGridBlockStructure , StructuredGridInformation > ; using UnstructuredDataBlock = Block < UnstructuredDataBlockStructure , UnstructuredDataInformation > ; using UnstructuredGridBlock = Block < UnstructuredGridBlockStructure , UnstructuredGridInformation > ; using PolyDataBlock = Block < PolyDataBlockStructure , PolyDataInformation > ; @} *
...
##  template < class DataSetT > static int GenerateGhostCells ( std :: vector < DataSetT * > & inputsDS , std :: vector < DataSetT * > & outputsDS , int outputGhostLevels , vtkMultiProcessController * controller ) ; /@{ *
...
##  template parameter.
##  static int GenerateGhostCellsImageData ( std :: vector < vtkImageData * > & inputs , std :: vector < vtkImageData * > & outputs , int outputGhostLevels , vtkMultiProcessController * controller ) ; static int GenerateGhostCellsRectilinearGrid ( std :: vector < vtkRectilinearGrid * > & inputs , std :: vector < vtkRectilinearGrid * > & outputs , int outputGhostLevels , vtkMultiProcessController * controller ) ; static int GenerateGhostCellsStructuredGrid ( std :: vector < vtkStructuredGrid * > & inputs , std :: vector < vtkStructuredGrid * > & outputs , int outputGhostLevels , vtkMultiProcessController * controller ) ; static int GenerateGhostCellsPolyData ( std :: vector < vtkPolyData * > & inputs , std :: vector < vtkPolyData * > & outputs , int outputGhostLevels , vtkMultiProcessController * controller ) ; static int GenerateGhostCellsUnstructuredGrid ( std :: vector < vtkUnstructuredGrid * > & inputs , std :: vector < vtkUnstructuredGrid * > & outputs , int outputGhostLevels , vtkMultiProcessController * controller ) ; /@} protected : vtkDIYGhostUtilities ( ) ; ~ vtkDIYGhostUtilities ( ) override ; *
...
##  template < class DataSetT > static void InitializeGhostPointArray ( typename DataSetTypeToBlockTypeConverter < DataSetT > :: BlockType * block , DataSetT * output ) ; *
##  This method will set all ghosts cells in `output` to zero. It will also
##  allocate a new ghost array if none is already present.
##  template < class DataSetT > static void InitializeGhostCellArray ( typename DataSetTypeToBlockTypeConverter < DataSetT > :: BlockType * block , DataSetT * output ) ; /@{ *
##
##  static void CloneGeometricStructures ( std :: vector < vtkImageData * > & inputs , std :: vector < vtkImageData * > & outputs ) ; static void CloneGeometricStructures ( std :: vector < vtkRectilinearGrid * > & inputs , std :: vector < vtkRectilinearGrid * > & outputs ) ; static void CloneGeometricStructures ( std :: vector < vtkStructuredGrid * > & inputs , std :: vector < vtkStructuredGrid * > & outputs ) ; static void CloneGeometricStructures ( std :: vector < vtkUnstructuredGrid * > & inputs , std :: vector < vtkUnstructuredGrid * > & outputs ) ; static void CloneGeometricStructures ( std :: vector < vtkPolyData * > & inputs , std :: vector < vtkPolyData * > & outputs ) ; /@} /@{ *
...
##  output.
##  static void InitializeBlocks ( diy :: Master & master , std :: vector < vtkImageData * > & inputs ) ; static void InitializeBlocks ( diy :: Master & master , std :: vector < vtkRectilinearGrid * > & inputs ) ; static void InitializeBlocks ( diy :: Master & master , std :: vector < vtkStructuredGrid * > & inputs ) ; static void InitializeBlocks ( diy :: Master & master , std :: vector < vtkUnstructuredGrid * > & inputs ) ; static void InitializeBlocks ( diy :: Master & master , std :: vector < vtkPolyData * > & inputs ) ; /@} *
##  This method exchanges the bounding boxes among blocks.
##  template < class DataSetT > static void ExchangeBoundingBoxes ( diy :: Master & master , const vtkDIYExplicitAssigner & assigner , std :: vector < DataSetT * > & inputs ) ; template < class BlockT > static LinkMap ComputeLinkMapUsingBoundingBoxes ( const diy :: Master & master ) ; /@{ *
##  Method to be overloaded for each supported type of input data set.
...
##  static void ExchangeBlockStructures ( diy :: Master & master , std :: vector < vtkImageData * > & inputs ) ; static void ExchangeBlockStructures ( diy :: Master & master , std :: vector < vtkRectilinearGrid * > & inputs ) ; static void ExchangeBlockStructures ( diy :: Master & master , std :: vector < vtkStructuredGrid * > & inputs ) ; static void ExchangeBlockStructures ( diy :: Master & master , std :: vector < vtkUnstructuredGrid * > & inputs ) ; static void ExchangeBlockStructures ( diy :: Master & master , std :: vector < vtkPolyData * > & inputs ) ; /@} /@{ *
...
##  static LinkMap ComputeLinkMap ( const diy :: Master & master , std :: vector < vtkImageData * > & inputs , int outputGhostLevels ) ; static LinkMap ComputeLinkMap ( const diy :: Master & master , std :: vector < vtkRectilinearGrid * > & inputs , int outputGhostLevels ) ; static LinkMap ComputeLinkMap ( const diy :: Master & master , std :: vector < vtkStructuredGrid * > & inputs , int outputGhostLevels ) ; static LinkMap ComputeLinkMap ( const diy :: Master & master , std :: vector < vtkUnstructuredGrid * > & inputs , int outputGhostLevels ) ; static LinkMap ComputeLinkMap ( const diy :: Master & master , std :: vector < vtkPolyData * > & inputs , int outputGhostLevels ) ; /@} /@{ *
...
##  static void EnqueueGhosts ( const diy :: Master :: ProxyWithLink & cp , const diy :: BlockID & blockId , vtkImageData * input , ImageDataBlock * block ) ; static void EnqueueGhosts ( const diy :: Master :: ProxyWithLink & cp , const diy :: BlockID & blockId , vtkRectilinearGrid * input , RectilinearGridBlock * block ) ; static void EnqueueGhosts ( const diy :: Master :: ProxyWithLink & cp , const diy :: BlockID & blockId , vtkStructuredGrid * input , StructuredGridBlock * block ) ; static void EnqueueGhosts ( const diy :: Master :: ProxyWithLink & cp , const diy :: BlockID & blockId , vtkUnstructuredGrid * input , UnstructuredGridBlock * block ) ; static void EnqueueGhosts ( const diy :: Master :: ProxyWithLink & cp , const diy :: BlockID & blockId , vtkPolyData * input , PolyDataBlock * block ) ; /@} /@{ *
...
##  static void DequeueGhosts ( const diy :: Master :: ProxyWithLink & cp , int gid , ImageDataBlockStructure & blockStructure ) ; static void DequeueGhosts ( const diy :: Master :: ProxyWithLink & cp , int gid , RectilinearGridBlockStructure & blockStructure ) ; static void DequeueGhosts ( const diy :: Master :: ProxyWithLink & cp , int gid , StructuredGridBlockStructure & blockStructure ) ; static void DequeueGhosts ( const diy :: Master :: ProxyWithLink & cp , int gid , UnstructuredGridBlockStructure & blockStructure ) ; static void DequeueGhosts ( const diy :: Master :: ProxyWithLink & cp , int gid , PolyDataBlockStructure & blockStructure ) ; /@} *
##  Copy the inputs into the outputs. Shallow copying is performed if possible. If not,
##  a deep copy is done.
##  template < class DataSetT > static void CopyInputsAndAllocateGhosts ( diy :: Master & master , diy :: Assigner & assigner , diy :: RegularAllReducePartners & partners , std :: vector < DataSetT * > & inputs , std :: vector < DataSetT * > & outputs , int outputGhostLevels ) ; /@{ *
##  Method to be overloaded for each supported input data set type,
...
##  static void DeepCopyInputAndAllocateGhosts ( ImageDataBlock * block , vtkImageData * input , vtkImageData * outputs ) ; static void DeepCopyInputAndAllocateGhosts ( RectilinearGridBlock * block , vtkRectilinearGrid * input , vtkRectilinearGrid * outputs ) ; static void DeepCopyInputAndAllocateGhosts ( StructuredGridBlock * block , vtkStructuredGrid * input , vtkStructuredGrid * outputs ) ; static void DeepCopyInputAndAllocateGhosts ( UnstructuredGridBlock * block , vtkUnstructuredGrid * input , vtkUnstructuredGrid * outputs ) ; static void DeepCopyInputAndAllocateGhosts ( PolyDataBlock * block , vtkPolyData * input , vtkPolyData * outputs ) ; /@} *
##  This method exchanges ghosts between connected blocks.
##  template < class DataSetT > static void ExchangeGhosts ( diy :: Master & master , std :: vector < DataSetT * > & inputs ) ; *
##  This methods allocate a point and cell ghost array and fills it with 0.
##  template < class DataSetT > static void InitializeGhostArrays ( diy :: Master & master , std :: vector < DataSetT * > & outputs , int outputGhostLevels ) ; *
##  Adds ghost arrays, which are present in blocks of `master`, to `outputs` point and / or cell
##  data.
##  template < class DataSetT > static void AddGhostArrays ( diy :: Master & master , std :: vector < DataSetT * > & outputs ) ; /@{ *
##  This method sets the ghost arrays in the output. Ghosts have to be already allocated.
##  static void FillGhostArrays ( const diy :: Master & master , std :: vector < vtkImageData * > & outputs , int outputGhostLevels ) ; static void FillGhostArrays ( const diy :: Master & master , std :: vector < vtkRectilinearGrid * > & outputs , int outputGhostLevels ) ; static void FillGhostArrays ( const diy :: Master & master , std :: vector < vtkStructuredGrid * > & outputs , int outputGhostLevels ) ; static void FillGhostArrays ( const diy :: Master & master , std :: vector < vtkUnstructuredGrid * > & outputs , int outputGhostLevels ) ; static void FillGhostArrays ( const diy :: Master & master , std :: vector < vtkPolyData * > & outputs , int outputGhostLevels ) ; /@} private : vtkDIYGhostUtilities ( const vtkDIYGhostUtilities & ) = delete ; void operator = ( const vtkDIYGhostUtilities & ) = delete ; /@{ *
...
##  version is empty;
##  static void InflateBoundingBoxIfNecessary ( vtkDataSet * vtkNotUsed ( input ) , vtkBoundingBox & vtkNotUsed ( bb ) ) ; static void InflateBoundingBoxIfNecessary ( vtkPointSet * input , vtkBoundingBox & bb ) ; /@} } ;
## Error: token expected: ; but got: [identifier]!!!
