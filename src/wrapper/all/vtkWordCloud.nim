...
## !!!Ignored construct:  # vtkWordCloud_h [NewLine] # vtkWordCloud_h [NewLine] # vtkImageAlgorithm.h [NewLine] # vtkImageData.h  For ImageData # vtkInfovisCoreModule.h  For export macro # vtkSmartPointer.h  For SmartPointer [NewLine] # < array >  For stl array # < functional >  for function # < set >  for stl multiset # < string >  For stl string # < vector >  For stl vector [NewLine] *
##  @class   vtkWordCloud
...
##  class VTKINFOVISCORE_EXPORT vtkWordCloud : public vtkImageAlgorithm { public : protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkImageAlgorithm Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkImageAlgorithm :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkWordCloud :: IsTypeOf ( type ) ; } static vtkWordCloud * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkWordCloud * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkWordCloud * NewInstance ( ) const { return vtkWordCloud :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkImageAlgorithm :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkWordCloud :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkWordCloud :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; *
##  Construct object with vertex cell generation turned off.
##  static vtkWordCloud * New ( ) ;  Typedefs using ColorDistributionContainer = std :: array < double , 2 > ; using OffsetDistributionContainer = std :: array < int , 2 > ; using OrientationDistributionContainer = std :: array < double , 2 > ; using OrientationsContainer = std :: vector < double > ; using PairType = std :: tuple < std :: string , std :: string > ; using ReplacementPairsContainer = std :: vector < PairType > ; using SizesContainer = std :: array < int , 2 > ; using StopWordsContainer = std :: set < std :: string > ; using StringContainer = std :: vector < std :: string > ; /@{ *
##  Return the AdjustedSizes of the resized mask file.
##  /@} virtual SizesContainer GetAdjustedSizes ( ) { return AdjustedSizes ; } # SetStdContainerMacro ( name , container ) virtual void Set ## name ( container arg ) { bool changed = false ; if ( arg . size ( ) != name . size ( ) ) { changed = true ; } else { auto a = arg . begin ( ) ; for ( auto r : name ) { if ( * a != r ) { changed = true ; } a ++ ; } } if ( changed ) { name = arg ; this -> Modified ( ) ; } } [NewLine] /@{ *
##  Set/Get the vtkNamedColors name for the background(MidNightBlue).
##  /@} virtual void SetBackgroundColorName ( std :: string arg ) { if ( arg != BackgroundColorName ) { this -> Modified ( ) ; BackgroundColorName = arg ; } } virtual std :: string GetBackgroundColorName ( ) { return BackgroundColorName ; } /@{ *
##  Set/Get boolean that indicates the mask image is a single
##  channel(false).
##  /@} virtual void SetBWMask ( bool arg ) { if ( BWMask != arg ) { this -> Modified ( ) ; BWMask = arg ; } } virtual bool GetBWMask ( ) { return BWMask ; } /@{ *
...
##  /@} virtual void SetColorSchemeName ( std :: string arg ) { if ( ColorSchemeName != arg ) { this -> Modified ( ) ; ColorSchemeName = arg ; } } virtual std :: string GetColorSchemeName ( ) { return ColorSchemeName ; } /@{ *
...
##  /@} virtual void SetDPI ( int _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  DPI  to  << _arg ) ; if ( this -> DPI != _arg ) { this -> DPI = _arg ; this -> Modified ( ) ; } } ; virtual int GetDPI ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << DPI  of  << this -> DPI ) ; return this -> DPI ; } ; /@{ *
##  Set/Get FileName, the name of the file that contains the text to
##  be processed.
##  /@} virtual void SetFileName ( VTK_FILEPATH std :: string arg ) { if ( FileName != arg ) { this -> Modified ( ) ; FileName = arg ; } } virtual std :: string GetFileName ( ) VTK_FUTURE_CONST { return FileName ; } /@{ *
##  Set/Get FontFileName, If empty, the built-in Arial font is
##  used(). The FontFileName is the name of a file that contains a
##  TrueType font.
##  /@} virtual void SetFontFileName ( VTK_FILEPATH VTK_FUTURE_CONST std :: string arg ) { if ( FontFileName != arg ) { this -> Modified ( ) ; FontFileName = arg ; } } virtual std :: string GetFontFileName ( ) VTK_FUTURE_CONST { return FontFileName ; } /@{ *
##  Set/Get Gap, the space gap of words (2). The gap is the number
##  of spaces added to the beginning and end of each word.
##  /@} virtual void SetDPIGap ( int _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  Gap  to  << _arg ) ; if ( this -> Gap != _arg ) { this -> Gap = _arg ; this -> Modified ( ) ; } } ; virtual int GetDPIGap ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << Gap  of  << this -> Gap ) ; return this -> Gap ; } ; /@{ *
...
##  /@} virtual void SetMaskColorName ( std :: string arg ) { if ( MaskColorName != arg ) { this -> Modified ( ) ; MaskColorName = arg ; } } virtual std :: string GetMaskColorName ( ) { return MaskColorName ; } /@{ *
...
##  /@} virtual void SetMaskFileName ( VTK_FILEPATH VTK_FUTURE_CONST std :: string arg ) { if ( MaskFileName != arg ) { this -> Modified ( ) ; MaskFileName = arg ; } } virtual std :: string GetMaskFileName ( ) VTK_FUTURE_CONST { return MaskFileName ; } /@{ *
##  Set/Get MaxFontSize, the maximum font size(48).
##  /@} virtual void SetDPIGapMaxFontSize ( int _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  MaxFontSize  to  << _arg ) ; if ( this -> MaxFontSize != _arg ) { this -> MaxFontSize = _arg ; this -> Modified ( ) ; } } ; virtual int GetDPIGapMaxFontSize ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << MaxFontSize  of  << this -> MaxFontSize ) ; return this -> MaxFontSize ; } ; /@{ *
##  Set/Get MinFontSize, the minimum font size(8).
##  /@} virtual void SetDPIGapMaxFontSizeMinFontSize ( int _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  MinFontSize  to  << _arg ) ; if ( this -> MinFontSize != _arg ) { this -> MinFontSize = _arg ; this -> Modified ( ) ; } } ; virtual int GetDPIGapMaxFontSizeMinFontSize ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << MinFontSize  of  << this -> MinFontSize ) ; return this -> MinFontSize ; } ; /@{ *
##  Set/Get MinFrequency, the minimum word frequency
##  accepted(2). Words with frequencies less than this will be
##  ignored.
##  /@} virtual void SetDPIGapMaxFontSizeMinFontSizeMinFrequency ( int _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  MinFrequency  to  << _arg ) ; if ( this -> MinFrequency != _arg ) { this -> MinFrequency = _arg ; this -> Modified ( ) ; } } ; virtual int GetDPIGapMaxFontSizeMinFontSizeMinFrequency ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << MinFrequency  of  << this -> MinFrequency ) ; return this -> MinFrequency ; } ; /@{ *
##  Set/Get FontMultiplier, the font multiplier(6). The final
##  FontSize is this value the word frequency.
##  /@} virtual void SetDPIGapMaxFontSizeMinFontSizeMinFrequencyFontMultiplier ( int _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  FontMultiplier  to  << _arg ) ; if ( this -> FontMultiplier != _arg ) { this -> FontMultiplier = _arg ; this -> Modified ( ) ; } } ; virtual int GetDPIGapMaxFontSizeMinFontSizeMinFrequencyFontMultiplier ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << FontMultiplier  of  << this -> FontMultiplier ) ; return this -> FontMultiplier ; } ; /@{ *
##  Set/Get ColorDistribution, the distribution of random colors(.6
##  1.0), if WordColorName is empty.
##  /@} SetStdContainerMacro ( ColorDistribution , ColorDistributionContainer ) ; virtual ColorDistributionContainer GetColorDistribution ( ) { return ColorDistribution ; } /@{ *
...
##  /@} SetStdContainerMacro ( OffsetDistribution , OffsetDistributionContainer ) ; virtual OffsetDistributionContainer GetOffsetDistribution ( ) { return OffsetDistribution ; } /@{ *
...
##  /@} SetStdContainerMacro ( OrientationDistribution , OrientationDistributionContainer ) ; virtual OrientationDistributionContainer GetOrientationDistribution ( ) { return OrientationDistribution ; } /@{ *
##  Set/Add/Get Orientations, a vector of discrete orientations (). If
##  non-empty, these will be used instead of the orientations
##  distribution").
##  /@} SetStdContainerMacro ( Orientations , OrientationsContainer ) ; void AddOrientation ( double arg ) { Orientations . push_back ( arg ) ; this -> Modified ( ) ; } virtual OrientationsContainer GetOrientations ( ) { return Orientations ; } /@{ *
...
##  /@} SetStdContainerMacro ( ReplacementPairs , ReplacementPairsContainer ) ; void AddReplacementPair ( PairType arg ) { ReplacementPairs . push_back ( arg ) ; this -> Modified ( ) ; } virtual ReplacementPairsContainer GetReplacementPairs ( ) { return ReplacementPairs ; } /@{ *
##  Set/Get Sizes, the size of the output image(640 480).
##  /@} SetStdContainerMacro ( Sizes , SizesContainer ) ; virtual SizesContainer GetSizes ( ) { return Sizes ; } /@{ *
...
##  /@} SetStdContainerMacro ( StopWords , StopWordsContainer ) ; void AddStopWord ( std :: string word ) { StopWords . insert ( word ) ; this -> Modified ( ) ; } void ClearStopWords ( ) { StopWords . clear ( ) ; this -> Modified ( ) ; } virtual StopWordsContainer GetStopWords ( ) { return StopWords ; } /@{ *
##  Set/Get StopListFileName, the name of the file that contains the
##  stop words, one per line.
##  /@} virtual void SetStopListFileName ( VTK_FILEPATH VTK_FUTURE_CONST std :: string arg ) { if ( StopListFileName != arg ) { this -> Modified ( ) ; StopListFileName = arg ; } } virtual std :: string GetStopListFileName ( ) VTK_FUTURE_CONST { return StopListFileName ; } /@{ *
##  Set/Get Title, add this word to the document's words and set a
##  high frequency, so that is will be rendered first.
##  /@} virtual void SetTitle ( std :: string arg ) { if ( Title != arg ) { this -> Modified ( ) ; Title = arg ; } } virtual std :: string GetTitle ( ) { return Title ; } /@{ *
...
##  /@} virtual void SetWordColorName ( std :: string arg ) { if ( WordColorName != arg ) { this -> Modified ( ) ; WordColorName = arg ; } } virtual std :: string GetWordColorName ( ) { return WordColorName ; } /@{ *
...
##  /@} virtual std :: vector < std :: string > & GetStoppedWords ( ) { return StoppedWords ; } protected : vtkWordCloud ( ) ; ~ vtkWordCloud ( ) override = default ; int RequestInformation ( vtkInformation * , vtkInformationVector * * , vtkInformationVector * ) override ; int RequestData ( vtkInformation * , vtkInformationVector * * , vtkInformationVector * ) override ; vtkSmartPointer < vtkImageData > ImageData ; int WholeExtent [ 6 ] ; SizesContainer AdjustedSizes ; std :: string BackgroundColorName ; bool BWMask ; ColorDistributionContainer ColorDistribution ; std :: string ColorSchemeName ; int DPI ; std :: string FileName ; std :: string FontFileName ; int FontMultiplier ; int Gap ; std :: string MaskColorName ; std :: string MaskFileName ; int MaxFontSize ; int MinFontSize ; int MinFrequency ; OffsetDistributionContainer OffsetDistribution ; OrientationDistributionContainer OrientationDistribution ; OrientationsContainer Orientations ; ReplacementPairsContainer ReplacementPairs ; SizesContainer Sizes ; StopWordsContainer StopWords ; std :: string StopListFileName ; std :: string Title ; std :: string WordColorName ; std :: vector < std :: string > KeptWords ; std :: vector < std :: string > SkippedWords ; std :: vector < std :: string > StoppedWords ; private : vtkWordCloud ( const vtkWordCloud & ) = delete ; void operator = ( const vtkWordCloud & ) = delete ;  Declaring the type of Predicate that accepts 2 pairs and returns a bool typedef std :: function < bool ( std :: pair < std :: string , int > , std :: pair < std :: string , int > ) > Comparator ; std :: multiset < std :: pair < std :: string , int > , Comparator > FindWordsSortedByFrequency ( std :: string & , vtkWordCloud * ) ; struct ExtentOffset { ExtentOffset ( int _x = 0.0 , int _y = 0.0 ) : x ( _x ) , y ( _y ) { } int x , y ; } ; } ;
