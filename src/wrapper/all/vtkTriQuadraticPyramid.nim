...
##   Module:    vtkTriQuadraticPyramid.h
##
...
## !!!Ignored construct:  # vtkTriQuadraticPyramid_h [NewLine] # vtkTriQuadraticPyramid_h [NewLine] # vtkCommonDataModelModule.h  For export macro # vtkNew.h  initialize cells that are used for the implementation # vtkNonLinearCell.h [NewLine] class vtkQuadraticEdge ;
...
## !!!Ignored construct:  class VTKCOMMONDATAMODEL_EXPORT vtkTriQuadraticPyramid : public vtkNonLinearCell { public : static vtkTriQuadraticPyramid * New ( ) ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkNonLinearCell Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkNonLinearCell :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkTriQuadraticPyramid :: IsTypeOf ( type ) ; } static vtkTriQuadraticPyramid * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkTriQuadraticPyramid * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkTriQuadraticPyramid * NewInstance ( ) const { return vtkTriQuadraticPyramid :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkNonLinearCell :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkTriQuadraticPyramid :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkTriQuadraticPyramid :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; /@{ *
##  Implement the vtkCell API. See the vtkCell API for descriptions
##  of these methods.
##  int GetCellType ( ) override { return VTK_TRIQUADRATIC_PYRAMID ; } int GetCellDimension ( ) override { return 3 ; } int GetNumberOfEdges ( ) override { return 8 ; } int GetNumberOfFaces ( ) override { return 5 ; } vtkCell * GetEdge ( int edgeId ) override ; vtkCell * GetFace ( int faceId ) override ; /@} int CellBoundary ( int subId , const double pcoords [ 3 ] , vtkIdList * pts ) override ; void Contour ( double value , vtkDataArray * cellScalars , vtkIncrementalPointLocator * locator , vtkCellArray * verts , vtkCellArray * lines , vtkCellArray * polys , vtkPointData * inPd , vtkPointData * outPd , vtkCellData * inCd , vtkIdType cellId , vtkCellData * outCd ) override ; int EvaluatePosition ( const double x [ 3 ] , double closestPoint [ 3 ] , int & subId , double pcoords [ 3 ] , double & dist2 , double weights [ ] ) override ; void EvaluateLocation ( int & subId , const double pcoords [ 3 ] , double x [ 3 ] , double * weights ) override ; *
##  Line-edge intersection. Intersection has to occur within [0,1] parametric
##  coordinates and with specified tolerance.
##  int IntersectWithLine ( const double p1 [ 3 ] , const double p2 [ 3 ] , double tol , double & t , double x [ 3 ] , double pcoords [ 3 ] , int & subId ) override ; int Triangulate ( int index , vtkIdList * ptIds , vtkPoints * pts ) override ; void Derivatives ( int subId , const double pcoords [ 3 ] , const double * values , int dim , double * derivs ) override ; double * GetParametricCoords ( ) override ; *
...
##  double GetParametricDistance ( const double pcoords [ 3 ] ) override ; static void InterpolationFunctions ( const double pcoords [ 3 ] , double weights [ 19 ] ) ; static void InterpolationDerivs ( const double pcoords [ 3 ] , double derivs [ 57 ] ) ; /@{ *
##  Compute the interpolation functions/derivatives
##  (aka shape functions/derivatives)
##  void InterpolateFunctions ( const double pcoords [ 3 ] , double weights [ 19 ] ) override { vtkTriQuadraticPyramid :: InterpolationFunctions ( pcoords , weights ) ; } void InterpolateDerivs ( const double pcoords [ 3 ] , double derivs [ 57 ] ) override { vtkTriQuadraticPyramid :: InterpolationDerivs ( pcoords , derivs ) ; } /@} *
...
##  static const vtkIdType * GetEdgeArray ( vtkIdType edgeId ) ; static const vtkIdType * GetFaceArray ( vtkIdType faceId ) ; /@} protected : vtkTriQuadraticPyramid ( ) ; ~ vtkTriQuadraticPyramid ( ) override ; vtkNew < vtkQuadraticEdge > Edge ; vtkNew < vtkBiQuadraticTriangle > TriangleFace ; vtkNew < vtkBiQuadraticTriangle > TriangleFace2 ; vtkNew < vtkBiQuadraticQuad > QuadFace ; vtkNew < vtkTetra > Tetra ; vtkNew < vtkPyramid > Pyramid ; vtkNew < vtkDoubleArray > Scalars ;  used to avoid New/Delete in contouring/clipping private : vtkTriQuadraticPyramid ( const vtkTriQuadraticPyramid & ) = delete ; void operator = ( const vtkTriQuadraticPyramid & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
...
proc GetParametricCenter*(pcoords: array[3, cdouble]): cint {.
    ...
    header: "vtkTriQuadraticPyramid.h".}