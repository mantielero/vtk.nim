...
## !!!Ignored construct:  # vtkLagrangianParticle_h [NewLine] # vtkLagrangianParticle_h [NewLine] # vtkFiltersFlowPathsModule.h  For export macro # vtkNew.h  For vtkNew # vtkSystemIncludes.h  For PrintSelf signature and vtkType [NewLine] # < vector > [NewLine] class vtkAbstractCellLocator ;
## Error: token expected: ; but got: [identifier]!!!

discard "forward decl of vtkBilinearQuadIntersection"
...
## !!!Ignored construct:  class VTKFILTERSFLOWPATHS_EXPORT vtkLagrangianParticle { public : *
...
##  typedef enum ParticleTermination { PARTICLE_TERMINATION_NOT_TERMINATED = 0 , PARTICLE_TERMINATION_SURF_TERMINATED , PARTICLE_TERMINATION_FLIGHT_TERMINATED , PARTICLE_TERMINATION_SURF_BREAK , PARTICLE_TERMINATION_OUT_OF_DOMAIN , PARTICLE_TERMINATION_OUT_OF_STEPS , PARTICLE_TERMINATION_OUT_OF_TIME , PARTICLE_TERMINATION_TRANSFERRED , PARTICLE_TERMINATION_ABORTED } ParticleTermination ; *
...
##  typedef enum SurfaceInteraction { SURFACE_INTERACTION_NO_INTERACTION = 0 , SURFACE_INTERACTION_TERMINATED , SURFACE_INTERACTION_BREAK , SURFACE_INTERACTION_BOUNCE , SURFACE_INTERACTION_PASS , SURFACE_INTERACTION_OTHER } SurfaceInteraction ; *
...
##  inline vtkLagrangianThreadedData * GetThreadedData ( ) { return this -> ThreadedData ; } inline void SetThreadedData ( vtkLagrangianThreadedData * threadedData ) { this -> ThreadedData = threadedData ; } /@} *
...
##  virtual void SetParentId ( vtkIdType parentId ) ; virtual vtkIdType GetParentId ( ) ; /@} *
...
##  virtual void SetTermination ( int termination ) ; virtual int GetTermination ( ) ; /@} /@{ *
##  Set/Get particle interaction.
##  Values out of enum range are accepted
##  Values < 100 are system reserved and should not be used
##  virtual void SetInteraction ( int interaction ) ; virtual int GetInteraction ( ) ; /@} /@{ *
##  Set/Get user flag.
##  virtual void SetUserFlag ( int flag ) ; virtual int GetUserFlag ( ) ; /@} /@{ *
##  Set/Get parallel specific flag, indication to insert or not
##  the previous position after streaming.
##  No effect in serial.
##  virtual void SetPInsertPreviousPosition ( bool val ) ; virtual bool GetPInsertPreviousPosition ( ) ; /@} /@{ *
##  Set/Get parallel specific flag, indication that the particle
##  may be manually shifted after streaming.
##  No effect in serial.
##  virtual void SetPManualShift ( bool val ) ; virtual bool GetPManualShift ( ) ; /@} *
...
##  virtual void PrintSelf ( ostream & os , vtkIndent indent ) ; protected : vtkLagrangianParticle ( const vtkLagrangianParticle & ) = delete ; vtkLagrangianParticle ( ) = delete ; void operator = ( const vtkLagrangianParticle & ) = delete ; std :: vector < double > PrevEquationVariables ; double * PrevVelocity ; double * PrevUserVariables ; std :: vector < double > EquationVariables ; double * Velocity ; double * UserVariables ; std :: vector < double > NextEquationVariables ; double * NextVelocity ; double * NextUserVariables ; std :: vector < double > PrevTrackedUserData ; std :: vector < double > TrackedUserData ; std :: vector < double > NextTrackedUserData ; vtkLagrangianThreadedData * ThreadedData = nullptr ; vtkIdType Id ; vtkIdType ParentId ; vtkIdType SeedId ; vtkIdType NumberOfSteps ; vtkIdType SeedArrayTupleIndex ; vtkPointData * SeedData ; double StepTime ; double IntegrationTime ; double PrevIntegrationTime ; int Termination ; int Interaction ; int UserFlag ; vtkDataSet * LastSurfaceDataSet ; vtkIdType LastSurfaceCellId ; int NumberOfVariables ;  Parallel related flags bool PInsertPreviousPosition ; bool PManualShift ; } ;
## Error: token expected: ; but got: [identifier]!!!
