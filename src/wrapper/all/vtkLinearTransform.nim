...
## !!!Ignored construct:  # vtkLinearTransform_h [NewLine] # vtkLinearTransform_h [NewLine] # vtkCommonTransformsModule.h  For export macro # vtkHomogeneousTransform.h [NewLine] class VTKCOMMONTRANSFORMS_EXPORT vtkLinearTransform : public vtkHomogeneousTransform { public : protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkHomogeneousTransform Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkHomogeneousTransform :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkLinearTransform :: IsTypeOf ( type ) ; } static vtkLinearTransform * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkLinearTransform * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkLinearTransform * NewInstance ( ) const { return vtkLinearTransform :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkHomogeneousTransform :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkLinearTransform :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkLinearTransform :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; *
...
##  double * TransformNormal ( double x , double y , double z ) VTK_SIZEHINT ( 3 ) { return this -> TransformDoubleNormal ( x , y , z ) ; } double * TransformNormal ( const double normal [ 3 ] ) VTK_SIZEHINT ( 3 ) { return this -> TransformDoubleNormal ( normal [ 0 ] , normal [ 1 ] , normal [ 2 ] ) ; } /@{ *
##  Apply the transformation to an (x,y,z) normal.
##  Use this if you are programming in python or Java.
##  float * TransformFloatNormal ( float x , float y , float z ) VTK_SIZEHINT ( 3 ) { this -> InternalFloatPoint [ 0 ] = x ; this -> InternalFloatPoint [ 1 ] = y ; this -> InternalFloatPoint [ 2 ] = z ; this -> TransformNormal ( this -> InternalFloatPoint , this -> InternalFloatPoint ) ; return this -> InternalFloatPoint ; } float * TransformFloatNormal ( const float normal [ 3 ] ) VTK_SIZEHINT ( 3 ) { return this -> TransformFloatNormal ( normal [ 0 ] , normal [ 1 ] , normal [ 2 ] ) ; } /@} /@{ *
...
##  Use this if you are programming in python or Java.
##  double * TransformVector ( double x , double y , double z ) VTK_SIZEHINT ( 3 ) { return this -> TransformDoubleVector ( x , y , z ) ; } double * TransformVector ( const double normal [ 3 ] ) VTK_SIZEHINT ( 3 ) { return this -> TransformDoubleVector ( normal [ 0 ] , normal [ 1 ] , normal [ 2 ] ) ; } *
...
##  Use this if you are programming in python or Java.
##  float * TransformFloatVector ( float x , float y , float z ) VTK_SIZEHINT ( 3 ) { this -> InternalFloatPoint [ 0 ] = x ; this -> InternalFloatPoint [ 1 ] = y ; this -> InternalFloatPoint [ 2 ] = z ; this -> TransformVector ( this -> InternalFloatPoint , this -> InternalFloatPoint ) ; return this -> InternalFloatPoint ; } float * TransformFloatVector ( const float vec [ 3 ] ) VTK_SIZEHINT ( 3 ) { return this -> TransformFloatVector ( vec [ 0 ] , vec [ 1 ] , vec [ 2 ] ) ; } /@} /@{ *
##  Apply the transformation to a double-precision (x,y,z) vector.
##  Use this if you are programming in python or Java.
##  double * TransformDoubleVector ( double x , double y , double z ) VTK_SIZEHINT ( 3 ) { this -> InternalDoublePoint [ 0 ] = x ; this -> InternalDoublePoint [ 1 ] = y ; this -> InternalDoublePoint [ 2 ] = z ; this -> TransformVector ( this -> InternalDoublePoint , this -> InternalDoublePoint ) ; return this -> InternalDoublePoint ; } double * TransformDoubleVector ( const double vec [ 3 ] ) VTK_SIZEHINT ( 3 ) { return this -> TransformDoubleVector ( vec [ 0 ] , vec [ 1 ] , vec [ 2 ] ) ; } /@} *
...
##  vtkLinearTransform.
##  vtkLinearTransform * GetLinearInverse ( ) { return static_cast < vtkLinearTransform * > ( this -> GetInverse ( ) ) ; } /@{ *
...
##  This will calculate the transformation without calling Update.
##  Meant for use only within other VTK classes.
##  virtual void InternalTransformNormal ( const float in [ 3 ] , float out [ 3 ] ) ; virtual void InternalTransformNormal ( const double in [ 3 ] , double out [ 3 ] ) ; /@} /@{ *
##  This will calculate the transformation without calling Update.
##  Meant for use only within other VTK classes.
##  virtual void InternalTransformVector ( const float in [ 3 ] , float out [ 3 ] ) ; virtual void InternalTransformVector ( const double in [ 3 ] , double out [ 3 ] ) ; /@} /@{ *
##  This will calculate the transformation as well as its derivative
##  without calling Update.  Meant for use only within other VTK
##  classes.
##  void InternalTransformDerivative ( const float in [ 3 ] , float out [ 3 ] , float derivative [ 3 ] [ 3 ] ) override ; void InternalTransformDerivative ( const double in [ 3 ] , double out [ 3 ] , double derivative [ 3 ] [ 3 ] ) override ; /@} protected : vtkLinearTransform ( ) = default ; ~ vtkLinearTransform ( ) override = default ; private : vtkLinearTransform ( const vtkLinearTransform & ) = delete ; void operator = ( const vtkLinearTransform & ) = delete ; } ;
