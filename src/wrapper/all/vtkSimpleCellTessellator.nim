...
## !!!Ignored construct:  # vtkSimpleCellTessellator_h [NewLine] # vtkSimpleCellTessellator_h [NewLine] # vtkCommonDataModelModule.h  For export macro # vtkGenericCellTessellator.h [NewLine] class vtkTriangleTile ;
## Error: token expected: ; but got: [identifier]!!!

discard "forward decl of vtkTetraTile"
discard "forward decl of vtkCellArray"
discard "forward decl of vtkDoubleArray"
discard "forward decl of vtkGenericEdgeTable"
discard "forward decl of vtkGenericSubdivisionErrorMetric"
...
## !!!Ignored construct:  class VTKCOMMONDATAMODEL_EXPORT vtkSimpleCellTessellator : public vtkGenericCellTessellator { public : static vtkSimpleCellTessellator * New ( ) ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkGenericCellTessellator Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkGenericCellTessellator :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkSimpleCellTessellator :: IsTypeOf ( type ) ; } static vtkSimpleCellTessellator * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkSimpleCellTessellator * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkSimpleCellTessellator * NewInstance ( ) const { return vtkSimpleCellTessellator :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkGenericCellTessellator :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkSimpleCellTessellator :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkSimpleCellTessellator :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; /@{ *
##  Get the higher order cell in order to access the evaluation function.
##  virtual vtkGenericAdaptorCell * GetnameGenericCell ( ) { vtkDebugWithObjectMacro ( this , <<  returning  GenericCell  address  << static_cast < vtkGenericAdaptorCell * > ( this -> GenericCell ) ) ; return this -> GenericCell ; } ; /@} *
...
##  void SetSubdivisionLevels ( int fixed , int maxLevel ) ; protected : vtkSimpleCellTessellator ( ) ; ~ vtkSimpleCellTessellator ( ) override ; *
...
##  vtkGenericEdgeTable * EdgeTable ; void InsertEdgesIntoEdgeTable ( vtkTriangleTile & tri ) ; void RemoveEdgesFromEdgeTable ( vtkTriangleTile & tri ) ; void InsertPointsIntoEdgeTable ( vtkTriangleTile & tri ) ; void InsertEdgesIntoEdgeTable ( vtkTetraTile & tetra ) ; void RemoveEdgesFromEdgeTable ( vtkTetraTile & tetra ) ; *
...
##  The ScalarsCapacity>=(6+attributeCollection->GetNumberOfComponents())*3 double * Scalars ; int ScalarsCapacity ; *
...
##  To avoid New/Delete
##  vtkDoubleArray * TessellatePoints ;  Allow to use GetPointer vtkCellArray * TessellateCellArray ; vtkPointData * TessellatePointData ; /@} int FindEdgeReferenceCount ( double p1 [ 3 ] , double p2 [ 3 ] , vtkIdType & e1 , vtkIdType & e2 ) ; int GetNumberOfCellsUsingFace ( int faceId ) ; int GetNumberOfCellsUsingEdge ( int edgeId ) ; *
...
##  vtkIdType NumberOfPoints ; int FixedSubdivisions ; int MaxSubdivisionLevel ; int CurrentSubdivisionLevel ; *
...
##  const vtkIdType * FaceIds ;  The following variables are for complex cells.  Used to create tetra from more complex cells, because the tessellator
##  is supposed to deal with simplices only. vtkOrderedTriangulator * Triangulator ;  Used to store the sub-tetra during the tessellation of complex
##  cells. vtkCellArray * Connectivity ;  Used to create triangles from a face of a complex cell. vtkPolygon * Polygon ;  Used to store the sub-triangles during the tessellation of complex cells. vtkIdList * TriangleIds ; vtkIdType * PointIds ; int PointIdsCapacity ; private : vtkSimpleCellTessellator ( const vtkSimpleCellTessellator & ) = delete ; void operator = ( const vtkSimpleCellTessellator & ) = delete ; friend class vtkTetraTile ; friend class vtkTriangleTile ; } ;
## Error: token expected: ; but got: [identifier]!!!
