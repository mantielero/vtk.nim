...
## !!!Ignored construct:  # vtkGeneralTransform_h [NewLine] # vtkGeneralTransform_h [NewLine] # vtkAbstractTransform.h [NewLine] # vtkCommonTransformsModule.h  For export macro [NewLine] # vtkMatrix4x4.h  Needed for inline methods [NewLine] class VTKCOMMONTRANSFORMS_EXPORT vtkGeneralTransform : public vtkAbstractTransform { public : static vtkGeneralTransform * New ( ) ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkAbstractTransform Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkAbstractTransform :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkGeneralTransform :: IsTypeOf ( type ) ; } static vtkGeneralTransform * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkGeneralTransform * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkGeneralTransform * NewInstance ( ) const { return vtkGeneralTransform :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkAbstractTransform :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkGeneralTransform :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkGeneralTransform :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; *
...
##  has been set.
##  void Inverse ( ) override { this -> Concatenation -> Inverse ( ) ; this -> Modified ( ) ; } /@{ *
##  Create a translation matrix and concatenate it with the current
##  transformation according to PreMultiply or PostMultiply semantics.
##  void Translate ( double x , double y , double z ) { this -> Concatenation -> Translate ( x , y , z ) ; } void Translate ( const double x [ 3 ] ) { this -> Translate ( x [ 0 ] , x [ 1 ] , x [ 2 ] ) ; } void Translate ( const float x [ 3 ] ) { this -> Translate ( x [ 0 ] , x [ 1 ] , x [ 2 ] ) ; } /@} /@{ *
##  Create a rotation matrix and concatenate it with the current
##  transformation according to PreMultiply or PostMultiply semantics.
##  The angle is in degrees, and (x,y,z) specifies the axis that the
##  rotation will be performed around.
##  void RotateWXYZ ( double angle , double x , double y , double z ) { this -> Concatenation -> Rotate ( angle , x , y , z ) ; } void RotateWXYZ ( double angle , const double axis [ 3 ] ) { this -> RotateWXYZ ( angle , axis [ 0 ] , axis [ 1 ] , axis [ 2 ] ) ; } void RotateWXYZ ( double angle , const float axis [ 3 ] ) { this -> RotateWXYZ ( angle , axis [ 0 ] , axis [ 1 ] , axis [ 2 ] ) ; } /@} /@{ *
...
##  void RotateX ( double angle ) { this -> RotateWXYZ ( angle , 1 , 0 , 0 ) ; } void RotateY ( double angle ) { this -> RotateWXYZ ( angle , 0 , 1 , 0 ) ; } void RotateZ ( double angle ) { this -> RotateWXYZ ( angle , 0 , 0 , 1 ) ; } /@} /@{ *
##  Create a scale matrix (i.e. set the diagonal elements to x, y, z)
##  and concatenate it with the current transformation according to
##  PreMultiply or PostMultiply semantics.
##  void Scale ( double x , double y , double z ) { this -> Concatenation -> Scale ( x , y , z ) ; } void Scale ( const double s [ 3 ] ) { this -> Scale ( s [ 0 ] , s [ 1 ] , s [ 2 ] ) ; } void Scale ( const float s [ 3 ] ) { this -> Scale ( s [ 0 ] , s [ 1 ] , s [ 2 ] ) ; } /@} /@{ *
##  Concatenates the matrix with the current transformation according
##  to PreMultiply or PostMultiply semantics.
##  void Concatenate ( vtkMatrix4x4 * matrix ) { this -> Concatenate ( * matrix -> Element ) ; } void Concatenate ( const double elements [ 16 ] ) { this -> Concatenation -> Concatenate ( elements ) ; } /@} *
...
##  void PreMultiply ( ) { if ( this -> Concatenation -> GetPreMultiplyFlag ( ) ) { return ; } this -> Concatenation -> SetPreMultiplyFlag ( 1 ) ; this -> Modified ( ) ; } *
...
##  M is the current transformation matrix and A is the applied matrix.
##  The default is PreMultiply.
##  void PostMultiply ( ) { if ( ! this -> Concatenation -> GetPreMultiplyFlag ( ) ) { return ; } this -> Concatenation -> SetPreMultiplyFlag ( 0 ) ; this -> Modified ( ) ; } *
##  Get the total number of transformations that are linked into this
##  one via Concatenate() operations or via SetInput().
##  int GetNumberOfConcatenatedTransforms ( ) { return this -> Concatenation -> GetNumberOfTransforms ( ) + ( this -> Input == nullptr ? 0 : 1 ) ; } *
...
##  vtkAbstractTransform * GetConcatenatedTransform ( int i ) { if ( this -> Input == nullptr ) { return this -> Concatenation -> GetTransform ( i ) ; } else if ( i < this -> Concatenation -> GetNumberOfPreTransforms ( ) ) { return this -> Concatenation -> GetTransform ( i ) ; } else if ( i > this -> Concatenation -> GetNumberOfPreTransforms ( ) ) { return this -> Concatenation -> GetTransform ( i - 1 ) ; } else if ( this -> GetInverseFlag ( ) ) { return this -> Input -> GetInverse ( ) ; } else { return this -> Input ; } } /@{ *
...
##  void SetInput ( vtkAbstractTransform * input ) ; vtkAbstractTransform * GetInput ( ) { return this -> Input ; } /@} *
...
##  void Push ( ) { if ( this -> Stack == nullptr ) { this -> Stack = vtkTransformConcatenationStack :: New ( ) ; } this -> Stack -> Push ( & this -> Concatenation ) ; this -> Modified ( ) ; } /@} /@{ *
##  Deletes the transformation on the top of the stack and sets the top
##  to the next transformation on the stack.
##  void Pop ( ) { if ( this -> Stack == nullptr ) { return ; } this -> Stack -> Pop ( & this -> Concatenation ) ; this -> Modified ( ) ; } /@} /@{ *
...
##  vtkMTimeType GetMTime ( ) override ; protected : vtkGeneralTransform ( ) ; ~ vtkGeneralTransform ( ) override ; void InternalDeepCopy ( vtkAbstractTransform * t ) override ; void InternalUpdate ( ) override ; vtkAbstractTransform * Input ; vtkTransformConcatenation * Concatenation ; vtkTransformConcatenationStack * Stack ; private : vtkGeneralTransform ( const vtkGeneralTransform & ) = delete ; void operator = ( const vtkGeneralTransform & ) = delete ; } ;
