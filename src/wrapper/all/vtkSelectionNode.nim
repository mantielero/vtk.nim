...
## !!!Ignored construct:  # vtkSelectionNode_h [NewLine] # vtkSelectionNode_h [NewLine] # vtkCommonDataModelModule.h  For export macro # vtkDeprecation.h  for deprecation # vtkObject.h [NewLine] class vtkAbstractArray ;
...
## !!!Ignored construct:  class VTKCOMMONDATAMODEL_EXPORT vtkSelectionNode : public vtkObject { public : protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkObject Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkObject :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkSelectionNode :: IsTypeOf ( type ) ; } static vtkSelectionNode * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkSelectionNode * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkSelectionNode * NewInstance ( ) const { return vtkSelectionNode :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkObject :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkSelectionNode :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkSelectionNode :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; static vtkSelectionNode * New ( ) ; *
...
##  virtual void SetSelectionList ( vtkAbstractArray * ) ; virtual vtkAbstractArray * GetSelectionList ( ) ; /@} /@{ *
##  Sets the selection table.
##  virtual void SetSelectionData ( vtkDataSetAttributes * data ) ; virtual vtkDataSetAttributes * GetnameSelectionData ( ) { vtkDebugWithObjectMacro ( this , <<  returning  SelectionData  address  << static_cast < vtkDataSetAttributes * > ( this -> SelectionData ) ) ; return this -> SelectionData ; } ; /@} /@{ *
##  Returns the property map.
##  virtual vtkInformation * GetnameSelectionDataProperties ( ) { vtkDebugWithObjectMacro ( this , <<  returning  Properties  address  << static_cast < vtkInformation * > ( this -> Properties ) ) ; return this -> Properties ; } ; /@} *
...
##  vtkMTimeType GetMTime ( ) override ;  vtkSelectionNode specific keys follow: *
...
##  enum SelectionContent { GLOBALIDS , !< Select entities called out by their globally-unique IDs. PEDIGREEIDS , !< Select entities that have some identifiable pedigree. VALUES , !< Select entities that take on specific array values. INDICES , !< Select entities by their offsets into the dataset. FRUSTUM , !< Select entities contained within a viewing frustum. LOCATIONS , !< Select entities near the supplied world coordinates. THRESHOLDS , !< Select entities whose array values fall within a given threshold. BLOCKS , !< Select blocks within a composite dataset by their flat index. BLOCK_SELECTORS , !< Select datasets within a composite dataset using selector expressions. QUERY , !< Select entities with a text query. USER , !< Select entities with user-supplied, application-specific logic. NUM_CONTENT_TYPES } ; /@{ *
##  Get or set the content type of the selection.
##  This is the same as setting the CONTENT_TYPE() key on the property.
##  virtual void SetContentType ( int type ) ; virtual int GetContentType ( ) ; /@} *
...
##  static vtkInformationIntegerKey * FIELD_TYPE ( ) ; / Indicate the types of entities to which the selection-data applies. enum SelectionField { CELL , !< The selection data provided is cell-data. POINT , !< The selection data provided is point-data. FIELD , !< The selection data provided is field-data. VERTEX , !< The selection data provided is graph vertex-data. EDGE , !< The selection data provided is graph edge-data. ROW , !< The selection data provided is table row-data. NUM_FIELD_TYPES } ; /@{ *
##  Get or set the field type of the selection.
##  This is the same as setting the FIELD_TYPE() key on the property.
##  virtual void SetFieldType ( int type ) ; virtual int GetFieldType ( ) ; /@} *
...
##  static int ConvertSelectionFieldToAttributeType ( int val ) ; static int ConvertAttributeTypeToSelectionField ( int val ) ; /@} /@{ *
##  Set/Get the query expression string.
##  virtual void SetQueryString ( const char * _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  << QueryString  to  << ( _arg ? _arg : (null) ) ) ; if ( this -> QueryString == nullptr && _arg == nullptr ) { return ; } if ( this -> QueryString && _arg && ( ! strcmp ( this -> QueryString , _arg ) ) ) { return ; } delete [ ] this -> QueryString ; if ( _arg ) { size_t n = strlen ( _arg ) + 1 ; char * cp1 = new char [ n ] ; const char * cp2 = ( _arg ) ; this -> QueryString = cp1 ; do { * cp1 ++ = * cp2 ++ ; } while ( -- n ) ; } else { this -> QueryString = nullptr ; } this -> Modified ( ) ; } ; virtual char * GetQueryString ( ) { vtkDebugWithObjectMacro ( this , <<  returning  << QueryString  of  << ( this -> QueryString ? this -> QueryString : (null) ) ) ; return this -> QueryString ; } ; /@} *
...
##  static vtkInformationStringKey * ASSEMBLY_NAME ( ) ; static vtkInformationStringVectorKey * SELECTORS ( ) ; /@} *
...
##  static vtkInformationIntegerKey * HIERARCHICAL_LEVEL ( ) ; static vtkInformationIntegerKey * HIERARCHICAL_INDEX ( ) ; /@} *
##  @deprecated no longer relevant.
##  VTK_DEPRECATED_IN_9_1_0 ( no longer relevant ) static vtkInformationIntegerKey * INDEXED_VERTICES ( ) ; *
...
##  bool EqualProperties ( vtkSelectionNode * other , bool fullcompare = true ) ; protected : vtkSelectionNode ( ) ; ~ vtkSelectionNode ( ) override ; vtkInformation * Properties ; vtkDataSetAttributes * SelectionData ; char * QueryString ;  Map from content type to content type name static const char ContentTypeNames [ SelectionContent :: NUM_CONTENT_TYPES ] [ 16 ] ;  Map from integer field type to field type name static const char FieldTypeNames [ SelectionField :: NUM_FIELD_TYPES ] [ 8 ] ; private : vtkSelectionNode ( const vtkSelectionNode & ) = delete ; void operator = ( const vtkSelectionNode & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
