...
## !!!Ignored construct:  # QQuickVTKRenderItem_h [NewLine] # QQuickVTKRenderItem_h [NewLine]  Qt includes # < QOpenGLFunctions >  For QOpenGLFunctions # < QQuickItem > [NewLine]  vtk includes # QQuickVTKRenderWindow.h  For QQuickVTKRenderWindow # vtkNew.h  For vtkNew # vtkRenderer.h  For vtkRenderer [NewLine] # vtkGUISupportQtQuickModule.h  for export macro [NewLine]  Forward declarations class QHoverEvent ;
## Error: token expected: ; but got: [identifier]!!!

discard "forward decl of QKeyEvent"
discard "forward decl of QMouseEvent"
discard "forward decl of QQuickVTKInteractiveWidget"
discard "forward decl of vtkImageData"
## !!!Ignored construct:  class VTKGUISUPPORTQTQUICK_EXPORT QQuickVTKRenderItem : public QQuickItem , protected QOpenGLFunctions { Q_OBJECT typedef QQuickItem Superclass ; Q_PROPERTY ( QQuickVTKRenderWindow * renderWindow READ renderWindow WRITE setRenderWindow ) public : QQuickVTKRenderItem ( QQuickItem * parent = nullptr ) ; ~ QQuickVTKRenderItem ( ) = default ; /@{ *
##  Set/Get the render window for the item
##  QQuickVTKRenderWindow * renderWindow ( ) const ; virtual void setRenderWindow ( QQuickVTKRenderWindow * w ) ; /@} *
...
##  virtual void addWidget ( QQuickVTKInteractiveWidget * w ) ; virtual void removeWidget ( QQuickVTKInteractiveWidget * w ) ; /@} /@{ *
##  Get/Remove widgets from the view by their object name
##  virtual QQuickVTKInteractiveWidget * widgetByName ( QString name ) const ; virtual void removeWidgetByName ( QString name ) ; /@} public Q_SLOTS : *
...
##  virtual void cleanup ( ) ; protected Q_SLOTS : virtual void handleWindowChanged ( QQuickWindow * w ) ; protected :  Helper members QQuickVTKRenderWindow * m_renderWindow = nullptr ; vtkNew < vtkRenderer > m_renderer ; QVector < QQuickVTKInteractiveWidget * > m_widgets ; *
##  Set the viewport for this item
##  virtual void setViewport ( const QRectF & rect ) ;  Event handlers # QT_VERSION < QT_VERSION_CHECK ( 6 , 0 , 0 ) [NewLine] void geometryChanged ( const QRectF & newGeometry , const QRectF & oldGeometry ) override ; # [NewLine] void geometryChange ( const QRectF & newGeometry , const QRectF & oldGeometry ) override ; # [NewLine] bool event ( QEvent * ev ) override ; private : QQuickVTKRenderItem ( const QQuickVTKRenderItem & ) = delete ; void operator = ( const QQuickVTKRenderItem ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
