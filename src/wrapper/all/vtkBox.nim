...
## !!!Ignored construct:  # vtkBox_h [NewLine] # vtkBox_h [NewLine] # vtkCommonDataModelModule.h  For export macro # vtkImplicitFunction.h [NewLine] class vtkBoundingBox ;
## Error: token expected: ; but got: [identifier]!!!

## !!!Ignored construct:  class VTKCOMMONDATAMODEL_EXPORT vtkBox : public vtkImplicitFunction { public : protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkImplicitFunction Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkImplicitFunction :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkBox :: IsTypeOf ( type ) ; } static vtkBox * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkBox * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkBox * NewInstance ( ) const { return vtkBox :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkImplicitFunction :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkBox :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkBox :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; *
...
##  xyz).
##  static vtkTypeBool IntersectWithPlane ( double bounds [ 6 ] , double origin [ 3 ] , double normal [ 3 ] , double xout [ 18 ] ) ; *
...
##  intersection though.
##  static vtkTypeBool IsBoxInFrustum ( double planes [ 24 ] , double bounds [ 6 ] ) ; protected : vtkBox ( ) ; ~ vtkBox ( ) override ; vtkBoundingBox * BBox ; double Bounds [ 6 ] ;  supports the GetBounds() method private : vtkBox ( const vtkBox & ) = delete ; void operator = ( const vtkBox & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!

proc SetXMin*(p: array[3, cdouble]) {.importcpp: "VTKCOMMONDATAMODEL_EXPORT::SetXMin(@)",
                                  header: "vtkBox.h".}
proc SetXMax*(p: array[3, cdouble]) {.importcpp: "VTKCOMMONDATAMODEL_EXPORT::SetXMax(@)",
                                  header: "vtkBox.h".}