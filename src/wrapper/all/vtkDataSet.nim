...
## !!!Ignored construct:  # vtkDataSet_h [NewLine] # vtkDataSet_h [NewLine] # vtkCommonDataModelModule.h  For export macro # vtkDataObject.h [NewLine] class vtkCell ;
...
## !!!Ignored construct:  class VTKCOMMONDATAMODEL_EXPORT vtkDataSet : public vtkDataObject { public : protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkDataObject Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkDataObject :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkDataSet :: IsTypeOf ( type ) ; } static vtkDataSet * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkDataSet * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkDataSet * NewInstance ( ) const { return vtkDataSet :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkDataObject :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkDataSet :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkDataSet :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; *
...
##  VTK_NEWINSTANCE virtual vtkCellIterator * NewCellIterator ( ) ; *
...
##  virtual vtkCell * GetCell ( vtkIdType cellId ) = 0 ; virtual vtkCell * GetCell ( int vtkNotUsed ( i ) , int vtkNotUsed ( j ) , int vtkNotUsed ( k ) ) { do { if ( vtkObject :: GetGlobalWarningDisplay ( ) ) { vtkOStreamWrapper :: EndlType endl ; vtkOStreamWrapper :: UseEndl ( endl ) ; vtkOStrStreamWrapper vtkmsg ; vtkObject * _object = const_cast < vtkObject * > ( static_cast < const vtkObject * > ( this ) ) ; if ( _object ) { vtkmsg << _object -> GetObjectDescription ( ) << :  ; } else { vtkmsg << (nullptr):  ; } vtkmsg <<  ijk indices are only valid with structured data! ; std :: string _filename = vtksys :: SystemTools :: GetFilenameName ( __FILE__ ) ; vtkOutputWindowDisplayErrorText ( _filename . c_str ( ) , __LINE__ , vtkmsg . str ( ) , _object ) ; vtkmsg . rdbuf ( ) -> freeze ( 0 ) ; vtkObject :: BreakOnError ( ) ; } } while ( false ) ; return nullptr ; } void SetCellOrderAndRationalWeights ( vtkIdType cellId , vtkGenericCell * cell ) ; *
...
##  THIS METHOD IS THREAD SAFE IF FIRST CALLED FROM A SINGLE THREAD AND
##  THE DATASET IS NOT MODIFIED
##  virtual void GetCellPoints ( vtkIdType cellId , vtkIdType & npts , vtkIdType const * & pts , vtkIdList * ptIds ) VTK_SIZEHINT ( pts , npts ) ; *
...
##  THIS METHOD IS THREAD SAFE IF FIRST CALLED FROM A SINGLE THREAD AND
##  THE DATASET IS NOT MODIFIED
##  vtkIdType FindPoint ( double x , double y , double z ) { double xyz [ 3 ] ; xyz [ 0 ] = x ; xyz [ 1 ] = y ; xyz [ 2 ] = z ; return this -> FindPoint ( xyz ) ; } virtual vtkIdType FindPoint ( double x [ 3 ] ) = 0 ; /@} *
...
##  void ShallowCopy ( vtkDataObject * src ) override ; void DeepCopy ( vtkDataObject * src ) override ; /@} enum FieldDataType { DATA_OBJECT_FIELD = 0 , POINT_DATA_FIELD = 1 , CELL_DATA_FIELD = 2 } ; *
...
##  virtual void GenerateGhostArray ( int zeroExt [ 6 ] ) { this -> GenerateGhostArray ( zeroExt , false ) ; } virtual void GenerateGhostArray ( int zeroExt [ 6 ] , bool cellOnly ) ; /@} /@{ *
##  Retrieve an instance of this class from an information object.
##  static vtkDataSet * GetData ( vtkInformation * info ) ; static vtkDataSet * GetData ( vtkInformationVector * v , int i = 0 ) ; /@} *
...
##  comparison.
##  vtkUnsignedCharArray * GetGhostArray ( int type ) override ; protected :  Constructor with default bounds (0,1, 0,1, 0,1). vtkDataSet ( ) ; ~ vtkDataSet ( ) override ; *
...
##  bool IsAnyBitSet ( vtkUnsignedCharArray * a , int bitFlag ) ; vtkCellData * CellData ;  Scalars, vectors, etc. associated w/ each cell vtkPointData * PointData ;  Scalars, vectors, etc. associated w/ each point vtkCallbackCommand * DataObserver ;  Observes changes to cell/point data vtkTimeStamp ComputeTime ;  Time at which bounds, center, etc. computed double Bounds [ 6 ] ;  (xmin,xmax, ymin,ymax, zmin,zmax) geometric bounds double Center [ 3 ] ;  Cached scalar range double ScalarRange [ 2 ] ;  Time at which scalar range is computed vtkTimeStamp ScalarRangeComputeTime ; /@{ *
##  These arrays pointers are caches used to avoid a string comparison (when
##  getting ghost arrays using GetArray(name))
##  vtkUnsignedCharArray * PointGhostArray ; vtkUnsignedCharArray * CellGhostArray ; bool PointGhostArrayCached ; bool CellGhostArrayCached ; /@} private : void InternalDataSetCopy ( vtkDataSet * src ) ; *
##  Called when point/cell data is modified
##  Updates caches to point/cell ghost arrays.
##  static void OnDataModified ( vtkObject * source , unsigned long eid , void * clientdata , void * calldata ) ; friend class vtkImageAlgorithmToDataSetFriendship ; private : vtkDataSet ( const vtkDataSet & ) = delete ; void operator = ( const vtkDataSet & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!

proc GetPoint*(id: vtkIdType; x: array[3, cdouble]) {.
    importcpp: "VTKCOMMONDATAMODEL_EXPORT::GetPoint(@)", header: "vtkDataSet.h".}