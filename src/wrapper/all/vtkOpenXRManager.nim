...
## !!!Ignored construct:  # vtkOpenXRManager_h [NewLine] # vtkOpenXRManager_h [NewLine] # vtkRenderingOpenXRModule.h  needed for exports [NewLine] # vtkOpenXR.h [NewLine] # vtkSystemIncludes.h [NewLine] # < array > [NewLine] # < memory > [NewLine] # < string > [NewLine] # < vector > [NewLine] class vtkOpenGLRenderWindow ;
## Error: token expected: ; but got: [identifier]!!!

## !!!Ignored construct:  class VTKRENDERINGOPENXR_EXPORT vtkOpenXRManager { public : @{ *
##  Return the singleton instance.
##  static vtkOpenXRManager & GetInstance ( ) { static vtkOpenXRManager UniqueInstance ; return UniqueInstance ; } @} @{ *
...
##  void PrintInstanceProperties ( ) ; void PrintSystemProperties ( XrSystemProperties * system_properties ) ; void PrintSupportedViewConfigs ( ) ; void PrintViewConfigViewInfo ( const std :: vector < XrViewConfigurationView > & ) ; bool PrintReferenceSpaces ( ) ; @} @{ *
...
##  uint32_t GetViewCount ( ) { return static_cast < uint32_t > ( this -> RenderResources -> ConfigViews . size ( ) ) ; } @{ *
...
##  const XrPosef * GetViewPose ( uint32_t eye ) { if ( eye >= this -> GetViewCount ( ) ) { return nullptr ; } return & ( this -> RenderResources -> Views [ eye ] . pose ) ; } @} @{ *
...
##  const XrFovf * GetProjectionFov ( uint32_t eye ) { if ( eye >= this -> GetViewCount ( ) ) { return nullptr ; } return & ( this -> RenderResources -> Views [ eye ] . fov ) ; } @} @{ *
...
##  XrPath GetXrPath ( const std :: string & path ) ; @} const std :: array < XrPath , 2 > & GetSubactionPaths ( ) { return this -> SubactionPaths ; } @{ *
...
##  @} void DestroyActionSets ( ) ; struct Action_t ; @{ *
...
##  bool ApplyVibration ( const Action_t & actionT , const int hand , const float amplitude = 0.5f , const float duration = 25000000.0f , const float frequency = XR_FREQUENCY_UNSPECIFIED ) ; enum ControllerIndex { Inactive = - 1 , Left = 0 , Right = 1 , Head = 2 , Generic = 3 , NumberOfControllers = 4 } ; struct Action_t { XrAction Action ; XrActionType ActionType ; union { XrActionStateFloat _float ; XrActionStateBoolean _boolean ; XrActionStatePose _pose ; XrActionStateVector2f _vec2f ; } States [ ControllerIndex :: NumberOfControllers ] ; XrSpace PoseSpaces [ ControllerIndex :: NumberOfControllers ] ; XrSpaceLocation PoseLocations [ ControllerIndex :: NumberOfControllers ] ; XrSpaceVelocity PoseVelocities [ ControllerIndex :: NumberOfControllers ] ; } ; protected : vtkOpenXRManager ( ) = default ; ~ vtkOpenXRManager ( ) = default ; @{ *
##  OpenXR Instance creation.
##  This is where we select the extensions using
##  SelectExtensions
##  bool CreateInstance ( ) ; std :: vector < const char * > SelectExtensions ( ) ; @} @{ *
...
##  std :: tuple < int64_t , int64_t > SelectSwapchainPixelFormats ( ) ; const std :: vector < int64_t > & GetSupportedColorFormats ( ) ; const std :: vector < int64_t > & GetSupportedDepthFormats ( ) ; @} struct SwapchainOpenGL_t ; @{ *
...
##  bool CreateReferenceSpace ( ) ; @} bool LoadControllerModels ( ) ; @{ *
...
##  uint32_t WaitAndAcquireSwapchainImage ( const XrSwapchain & swapchainHandle ) ; @}  Currently VTK only supports HeadMountedDisplay (HMD) constexpr static XrFormFactor FormFactor = XR_FORM_FACTOR_HEAD_MOUNTED_DISPLAY ;  Pick the view type to be stereo rather than mono or anything else constexpr static XrViewConfigurationType ViewType = XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO ;  PRIMARY_STEREO view configuration always has 2 views constexpr static uint32_t StereoViewCount = 2 ;  Three available types: VIEW, LOCAL and STAGE.  We use LOCAL space which
##  establishes a world-locked origin, rather than VIEW space, which tracks the
##  view origin. constexpr static XrReferenceSpaceType ReferenceSpaceType = XR_REFERENCE_SPACE_TYPE_STAGE ;  Communication with the runtime happens through this instance XrInstance Instance ;  A system is defined by an id and is used to create a session XrSystemId SystemId ; XrSession Session ; XrSessionState SessionState ; XrSpace ReferenceSpace ;  At the end of a frame, we must select en environment blend mode
...
##  choose XR_ENVIRONMENT_BLEND_MODE_ADDITIVE or XR_ENVIRONMENT_BLEND_MODE_ALPHA_BLEND XrEnvironmentBlendMode EnvironmentBlendMode ;  See vtkXrExtensions.h xr :: ExtensionDispatchTable Extensions ;  Non optional extension bool HasOpenGLExtension = false ; @{ *
##  Structure to hold optional extensions
##  loaded with SelectExtensions
##  struct { bool DepthExtensionSupported { false } ; bool ControllerModelExtensionSupported { false } ; bool UnboundedRefSpaceSupported { false } ; bool SpatialAnchorSupported { false } ; bool HandTrackingSupported { false } ; } OptionalExtensions ; @} std :: shared_ptr < void > GraphicsBinding ; *
##  Swapchain structure for OpenGL backend.
##  struct SwapchainOpenGL_t { XrSwapchain Swapchain ; int64_t Format { GL_NONE } ; uint32_t Width { 0 } ; uint32_t Height { 0 } ; std :: vector < XrSwapchainImageOpenGLKHR > Images ; } ; @{ *
...
##  struct RenderResources_t { XrViewState ViewState { XR_TYPE_VIEW_STATE } ;  Each physical Display/Eye is described by a view std :: vector < XrView > Views ;  One configuration view per view : this store std :: vector < XrViewConfigurationView > ConfigViews ; std :: vector < SwapchainOpenGL_t > ColorSwapchains ; std :: vector < SwapchainOpenGL_t > DepthSwapchains ; std :: vector < XrCompositionLayerProjectionView > ProjectionLayerViews ; std :: vector < XrCompositionLayerDepthInfoKHR > DepthInfoViews ; } ; std :: unique_ptr < RenderResources_t > RenderResources { } ; @}  There is one subaction path for each hand. std :: array < XrPath , 2 > SubactionPaths ; std :: vector < XrActionSet > ActionSets ; XrActionSet * ActiveActionSet = nullptr ; *
...
##  XrTime PredictedDisplayTime ; bool SessionRunning = false ;  After each WaitAndBeginFrame, the OpenXR runtime may inform us that
##  the current frame should not be rendered. Store it to avoid a render bool ShouldRenderCurrentFrame = false ;  If true, the function UpdateActionData will store
##  pose velocities for pose actions bool StorePoseVelocities = false ; private : vtkOpenXRManager ( const vtkOpenXRManager & ) = delete ; void operator = ( const vtkOpenXRManager & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
