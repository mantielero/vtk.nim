...
## !!!Ignored construct:  # vtkMultiThreshold_h [NewLine] # vtkMultiThreshold_h [NewLine] # vtkFiltersGeneralModule.h  For export macro # vtkMath.h  for Inf() and NegInf() # vtkMultiBlockDataSetAlgorithm.h [NewLine] # < map >  for IntervalRules map # < set >  for UpdateDependents() # < string >  for holding array names in NormKey # < vector >  for lists of threshold rules [NewLine] class vtkCell ;
...
## !!!Ignored construct:  class VTKFILTERSGENERAL_EXPORT vtkMultiThreshold : public vtkMultiBlockDataSetAlgorithm { public : protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkMultiBlockDataSetAlgorithm Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkMultiBlockDataSetAlgorithm :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkMultiThreshold :: IsTypeOf ( type ) ; } static vtkMultiThreshold * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkMultiThreshold * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkMultiThreshold * NewInstance ( ) const { return vtkMultiThreshold :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkMultiBlockDataSetAlgorithm :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkMultiThreshold :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkMultiThreshold :: New ( ) ; } public : ; static vtkMultiThreshold * New ( ) ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; / Whether the endpoint value of an interval should be included or excluded. enum Closure { OPEN = 0 , !< Specify an open interval CLOSED = 1 !< Specify a closed interval } ; / Norms that can be used to threshold vector attributes. enum Norm { LINFINITY_NORM = - 3 , !< Use the \f$L_{\infty}\f$ norm for the specified array threshold. L2_NORM = - 2 , !< Use the \f$L_2\f$ norm for the specified array threshold. L1_NORM = - 1 !< Use the \f$L_1\f$ norm for the specified array threshold. } ; / Operations that can be performed on sets to generate another set. Most of these operators take
## / 2 or more input sets. enum SetOperation { AND , !< Only include an element if it belongs to all the input sets OR , !< Include an element if it belongs to any input set XOR , !< Include an element if it belongs to exactly one input set WOR , !< Include elements that belong to an odd number of input sets (a kind of "winding XOR") NAND !< Only include elements that don't belong to any input set } ; /@{ *
...
##  int AddIntervalSet ( double xmin , double xmax , int omin , int omax , int assoc , const char * arrayName , int component , int allScalars ) ; int AddIntervalSet ( double xmin , double xmax , int omin , int omax , int assoc , int attribType , int component , int allScalars ) ; /@} /@{ *
...
##  int AddLowpassIntervalSet ( double xmax , int assoc , const char * arrayName , int component , int allScalars ) ; int AddHighpassIntervalSet ( double xmin , int assoc , const char * arrayName , int component , int allScalars ) ; int AddBandpassIntervalSet ( double xmin , double xmax , int assoc , const char * arrayName , int component , int allScalars ) ; int AddNotchIntervalSet ( double xlo , double xhi , int assoc , const char * arrayName , int component , int allScalars ) ; /@} *
...
##  void Reset ( ) ; / A pointer to a function that returns a norm (or a single component) of a tuple with 1 or more
## / components. typedef double ( * TupleNorm ) ( vtkDataArray * arr , vtkIdType tuple , int component ) ;  NormKey must be able to use TupleNorm typedef: class NormKey ;  Interval must be able to use NormKey typedef: class Interval ;  Set needs to refer to boolean set pointers class BooleanSet ; / A class with comparison operator used to index input array norms used in threshold rules. class NormKey { public : int Association ;  vtkDataObject::FIELD_ASSOCIATION_POINTS or
##  vtkDataObject::FIELD_ASSOCIATION_CELLS int Type ;  -1 => use Name, otherwise: vtkDataSetAttributes::{SCALARS, VECTORS, TENSORS,
##  NORMALS, TCOORDS, GLOBALIDS} std :: string Name ;  Either empty or (when ArrayType == -1) an input array name int Component ;  LINFINITY_NORM, L1_NORM, L2_NORM or an integer component number int AllScalars ;  For Association == vtkDataObject::FIELD_ASSOCIATION_POINTS, must all points
##  be in the interval? int InputArrayIndex ;  The number passed to vtkAlgorithm::SetInputArrayToProcess() TupleNorm NormFunction ;  A function pointer to compute the norm (or fetcht the correct
##  component) of a tuple. / Compute the norm of a cell by calling NormFunction for all its points or for its single
## / cell-centered value. void ComputeNorm ( vtkIdType cellId , vtkCell * cell , vtkDataArray * array , double cellNorm [ 2 ] ) const ; / A partial ordering of NormKey objects is required for them to serve as keys in the
## / vtkMultiThreshold::IntervalRules map. bool operator < ( const NormKey & other ) const { if ( this -> Association < other . Association ) return true ; else if ( this -> Association > other . Association ) return false ; if ( this -> Component < other . Component ) return true ; else if ( this -> Component > other . Component ) return false ; if ( ( ! this -> AllScalars ) && other . AllScalars ) return true ; else if ( this -> AllScalars && ( ! other . AllScalars ) ) return false ; if ( this -> Type == - 1 ) { if ( other . Type == - 1 ) return this -> Name < other . Name ; return true ; } else { return this -> Type < other . Type ; } } } ; * A base class for representing threshold sets.
##  A set may be represented as a threshold interval over some attribute
##  or as a boolean combination of sets.
##  class Set { public : int Id ; / A unique identifier for this set. int OutputId ; / The index of the output mesh that will hold this set or -1 if the set is not
## / output. / Default constructur. The grid output ID is initialized to indicate that the set should not
## / be output. Set ( ) { this -> OutputId = - 1 ; } / Virtual destructor since we have virtual members. virtual ~ Set ( ) = default ; / Print a graphviz node label statement (with fancy node name and shape). virtual void PrintNodeName ( ostream & os ) ; / Print a graphviz node name for use in an edge statement. virtual void PrintNode ( ostream & os ) = 0 ; / Avoid dynamic_casts. Subclasses must override virtual BooleanSet * GetBooleanSetPointer ( ) ; virtual Interval * GetIntervalPointer ( ) ; } ; / A subset of a mesh represented by a range of acceptable attribute values. class Interval : public Set { public : / The values defining the interval. These must be in ascending order. double EndpointValues [ 2 ] ; / Are the endpoint values themselves included in the set (CLOSED) or not (OPEN)? int EndpointClosures [ 2 ] ; / This contains information about the attribute over which the interval is defined. NormKey Norm ; * Does the specified range fall inside the interval?
...
##  int Match ( double cellNorm [ 2 ] ) ; ~ Interval ( ) override = default ; void PrintNode ( ostream & os ) override ; Interval * GetIntervalPointer ( ) override ; } ; / A subset of a mesh represented as a boolean set operation class BooleanSet : public Set { public : / The boolean operation that will be performed on the inputs to obtain the output. int Operator ; / A list of input sets. These may be IntervalSets or BooleanSets. std :: vector < int > Inputs ; / Construct a new set with the given ID, operator, and inputs. BooleanSet ( int sId , int op , int * inBegin , int * inEnd ) : Inputs ( inBegin , inEnd ) { this -> Id = sId ; this -> Operator = op ; } ~ BooleanSet ( ) override = default ; void PrintNode ( ostream & os ) override ; BooleanSet * GetBooleanSetPointer ( ) override ; } ; protected : vtkMultiThreshold ( ) ; ~ vtkMultiThreshold ( ) override ; *
...
##  int NumberOfOutputs ; / A list of pointers to IntervalSets. typedef std :: vector < Interval * > IntervalList ; / A map describing the IntervalSets that share a common attribute and norm. typedef std :: map < NormKey , IntervalList > RuleMap ; typedef std :: vector < int > TruthTreeValues ; typedef std :: vector < TruthTreeValues > TruthTree ; *
...
##  void PrintGraph ( ostream & os ) ; vtkMultiThreshold ( const vtkMultiThreshold & ) = delete ; void operator = ( const vtkMultiThreshold & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!

proc AddLowpassIntervalSet*(xmax: cdouble; assoc: cint; arrayName: cstring;
                           component: cint; allScalars: cint): cint {.
    importcpp: "VTKFILTERSGENERAL_EXPORT::AddLowpassIntervalSet(@)",
    header: "vtkMultiThreshold.h".}
proc AddHighpassIntervalSet*(xmin: cdouble; assoc: cint; arrayName: cstring;
                            component: cint; allScalars: cint): cint {.
    importcpp: "VTKFILTERSGENERAL_EXPORT::AddHighpassIntervalSet(@)",
    header: "vtkMultiThreshold.h".}
proc AddBandpassIntervalSet*(xmin: cdouble; xmax: cdouble; assoc: cint;
                            arrayName: cstring; component: cint; allScalars: cint): cint {.
    importcpp: "VTKFILTERSGENERAL_EXPORT::AddBandpassIntervalSet(@)",
    header: "vtkMultiThreshold.h".}
proc AddNotchIntervalSet*(xlo: cdouble; xhi: cdouble; assoc: cint; arrayName: cstring;
                         component: cint; allScalars: cint): cint {.
    importcpp: "VTKFILTERSGENERAL_EXPORT::AddNotchIntervalSet(@)",
    header: "vtkMultiThreshold.h".}
## !!!Ignored construct:  inline vtkMultiThreshold :: Interval * vtkMultiThreshold :: Set :: GetIntervalPointer ( ) { return nullptr ; } inline vtkMultiThreshold :: BooleanSet * vtkMultiThreshold :: Set :: GetBooleanSetPointer ( ) { return nullptr ; } inline vtkMultiThreshold :: Interval * vtkMultiThreshold :: Interval :: GetIntervalPointer ( ) { return this ; } inline vtkMultiThreshold :: BooleanSet * vtkMultiThreshold :: BooleanSet :: GetBooleanSetPointer ( ) { return this ; } #  vtkMultiThreshold_h
