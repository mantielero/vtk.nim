...
## !!!Ignored construct:  # vtkSphericalPointIterator_h [NewLine] # vtkSphericalPointIterator_h [NewLine] # vtkCommonDataModelModule.h  For export macro # vtkDataSet.h  the dataset and its points to iterate over # vtkDoubleArray.h  For axes # vtkObject.h [NewLine] # vtkSmartPointer.h  auto destruct [NewLine] # < memory >  for std::unique_ptr [NewLine] class vtkDoubleArray ;
## Error: token expected: ; but got: [identifier]!!!

discard "forward decl of vtkPolyData"
discard "forward decl of SpiralPointIterator"
## !!!Ignored construct:  class VTKCOMMONDATAMODEL_EXPORT vtkSphericalPointIterator : public vtkObject { public : /@{ *
##  Standard methods to instantiate, obtain type information, and
##  print information about an instance of the class.
##  static vtkSphericalPointIterator * New ( ) ; protected : const char * GetClassNameInternal ( ) const override { return vtkSphericalPointIterator ; } public : typedef vtkObject Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( vtkSphericalPointIterator , type ) ) { return 1 ; } return vtkObject :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkSphericalPointIterator :: IsTypeOf ( type ) ; } static vtkSphericalPointIterator * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( vtkSphericalPointIterator ) ) { return static_cast < vtkSphericalPointIterator * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkSphericalPointIterator * NewInstance ( ) const { return vtkSphericalPointIterator :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( vtkSphericalPointIterator , type ) ) { return 0 ; } return 1 + vtkObject :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkSphericalPointIterator :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; /@} /@{ *
##  Define the dataset and its associated points over which to iterate.
##  virtual void SetDataSet ( vtkDataSet * _arg ) { { vtkDebugWithObjectMacro ( this , <<  setting  << DataSet  to  << _arg ) ; if ( this -> DataSet != _arg ) { this -> DataSet = _arg ; this -> Modified ( ) ; } } ; } ; virtual vtkDataSet * Getname ( ) { vtkDebugWithObjectMacro ( this , <<  returning  name  address  << static_cast < vtkDataSet * > ( this -> DataSet ) ) ; return this -> DataSet ; } ; /@} /@{ *
...
##  virtual void SetDataSetAxes ( vtkDoubleArray * _arg ) { { vtkDebugWithObjectMacro ( this , <<  setting  << Axes  to  << _arg ) ; if ( this -> Axes != _arg ) { this -> Axes = _arg ; this -> Modified ( ) ; } } ; } ; virtual vtkDoubleArray * Getname ( ) { vtkDebugWithObjectMacro ( this , <<  returning  name  address  << static_cast < vtkDoubleArray * > ( this -> Axes ) ) ; return this -> Axes ; } ; /@} *
...
##  specified.
##  enum AxesType { XY_CW_AXES = 0 ,  axes clockwise around center in x-y plane (resolution required) XY_CCW_AXES = 1 ,  axes counterclockwise around center (resolution required) XY_SQUARE_AXES = 2 ,  axes +x,-x, +y,-y: axes through the four faces of a square CUBE_AXES = 3 ,  axes +x,-x, +y,-y, +z,-z: axes through the six faces of a cube OCTAHEDRON_AXES = 4 ,  axes through the eight faces of a regular octahedron CUBE_OCTAHEDRON_AXES = 5 ,  axes through the eight faces of a regular octahedron and six faces of a cube DODECAHEDRON_AXES = 6 ,  axes through the twelve faces of a dedecahdron ICOSAHEDRON_AXES = 7 ,  axes through the twenty faces of a icosahedron } ; *
...
##  virtual void SetSorting ( int _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  << Sorting  to  << _arg ) ; if ( this -> Sorting != ( _arg < SORT_NONE ? SORT_NONE : ( _arg > SORT_DESCENDING ? SORT_DESCENDING : _arg ) ) ) { this -> Sorting = ( _arg < SORT_NONE ? SORT_NONE : ( _arg > SORT_DESCENDING ? SORT_DESCENDING : _arg ) ) ; this -> Modified ( ) ; } } virtual int GetSortingMinValue ( ) { return SORT_NONE ; } virtual int GetSortingMaxValue ( ) { return SORT_DESCENDING ; } ; virtual int GetSorting ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << Sorting  of  << this -> Sorting ) ; return this -> Sorting ; } ; void SetSortTypeToNone ( ) { this -> SetSorting ( SORT_NONE ) ; } void SetSortTypeToAscending ( ) { this -> SetSorting ( SORT_ASCENDING ) ; } void SetSortTypeToDescending ( ) { this -> SetSorting ( SORT_DESCENDING ) ; } /@}  The following methods support point iteration. The data members referred
...
##  bool Initialize ( double center [ 3 ] , vtkIdList * neighborhood ) ; bool Initialize ( double center [ 3 ] , vtkIdType numNei , vtkIdType * neighborhood ) ; bool Initialize ( double center [ 3 ] ) ;  all points of the specified dataset /@} *
...
##  void BuildRepresentation ( vtkPolyData * pd ) ; protected : vtkSphericalPointIterator ( ) ; ~ vtkSphericalPointIterator ( ) override = default ;  Information needed to define the spherical iterator. vtkSmartPointer < vtkDataSet > DataSet ;  The points to iterate over vtkSmartPointer < vtkDoubleArray > Axes ;  The axes defining the iteration pattern int Sorting ;  The direction of sorting, if sorting required  Iterator internals are represented using a PIMPL idiom struct SphericalPointIterator ; std :: unique_ptr < SphericalPointIterator > Iterator ;  Changes to the VTK class must be propagated to the internal iterator vtkTimeStamp BuildTime ; private : vtkSphericalPointIterator ( const vtkSphericalPointIterator & ) = delete ; void operator = ( const vtkSphericalPointIterator & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
