...
## !!!Ignored construct:  # vtkAtomicMutex_h [NewLine] # vtkAtomicMutex_h [NewLine] # vtkCommonCoreModule.h  For export macro # < atomic >  For std::atomic [NewLine] class VTKCOMMONCORE_EXPORT vtkAtomicMutex { public :  left public purposely to allow for copy construction vtkAtomicMutex ( ) ; ~ vtkAtomicMutex ( ) = default ; vtkAtomicMutex ( const vtkAtomicMutex & other ) ; vtkAtomicMutex & operator = ( const vtkAtomicMutex & other ) ; /@{ *
...
##  void lock ( ) ; void unlock ( ) ; /@} private : std :: atomic_bool Locked ; } ;
