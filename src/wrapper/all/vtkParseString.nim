...
## !!!Ignored construct:  # vtkParseString_h [NewLine] # vtkParseString_h [NewLine] # vtkWrappingToolsModule.h [NewLine] # < stddef . h > [NewLine] # __cplusplus [NewLine] extern C { # [NewLine] *
##  Various important char types for tokenization
##  typedef enum parse_char_type_ { CPRE_NONDIGIT = 0x01 ,  A-Z a-z and _ CPRE_DIGIT = 0x02 ,  0-9 CPRE_XDIGIT = 0x03 ,  0-9 A-Z a-z and _ CPRE_EXTEND = 0x04 ,  non-ascii character CPRE_ID = 0x05 ,  starting char for identifier CPRE_XID = 0x07 ,  continuing char for identifier CPRE_HEX = 0x08 ,  0-9 A-F a-f hexadecimal digits CPRE_SIGN = 0x10 ,  +- (sign for floats) CPRE_QUOTE = 0x20 ,  " and ' CPRE_HSPACE = 0x40 ,  space, tab, carriage return CPRE_VSPACE = 0x80 ,  newline, vertical tab, form feed CPRE_WHITE = 0xC0 ,  all whitespace characters } parse_char_type ; *
...
##  typedef enum parse_space_t_ { WS_DEFAULT = CPRE_WHITE ,  skip all whitespace WS_PREPROC = CPRE_HSPACE ,  skip horizontal whitespace only WS_COMMENT = ( CPRE_WHITE | 0x100 ) ,  comments as tokens } parse_space_t ; *
##  Preprocessor tokens for C++.
##  typedef enum preproc_token_t_ { TOK_OTHER = 257 , TOK_ID ,  any id TOK_CHAR ,  char literal TOK_STRING ,  string literal TOK_NUMBER ,  any numeric literal TOK_COMMENT ,  C or C++ comment TOK_DBLHASH ,  ## TOK_SCOPE ,  :: TOK_INCR ,  ++ TOK_DECR ,  -- TOK_RSHIFT ,  >> TOK_LSHIFT ,  << TOK_AND ,  && TOK_OR ,  || TOK_EQ ,  == TOK_NE ,  != TOK_GE ,  >= TOK_LE ,  <= TOK_ADD_EQ ,  += TOK_SUB_EQ ,  -= TOK_MUL_EQ ,  *= TOK_DIV_EQ ,  /= TOK_MOD_EQ ,  %= TOK_AND_EQ ,  &= TOK_OR_EQ ,  |= TOK_XOR_EQ ,  ^= TOK_ARROW ,  -> TOK_DOT_STAR ,  .* TOK_ARROW_STAR ,  ->* TOK_RSHIFT_EQ ,  >>= TOK_LSHIFT_EQ ,  <<= TOK_ELLIPSIS ,  ... } preproc_token_t ; *
...
##  VTKWRAPPINGTOOLS_EXPORT unsigned int vtkParse_HashId ( const char * cp ) ; *
...
##  VTKWRAPPINGTOOLS_EXPORT unsigned int vtkParse_DecodeUtf8 ( const char * * cpp , int * error_flag ) ; *
...
##  typedef struct StringCache_ { unsigned long NumberOfChunks ; char * * Chunks ; size_t ChunkSize ; size_t Position ; } StringCache ; *
##  Initialize the string cache.
##  VTKWRAPPINGTOOLS_EXPORT void vtkParse_InitStringCache ( StringCache * cache ) ; *
##  Allocate a new string from the cache.
##  A total of n+1 bytes will be allocated, to leave room for null.
##  VTKWRAPPINGTOOLS_EXPORT char * vtkParse_NewString ( StringCache * cache , size_t n ) ; *
...
##  VTKWRAPPINGTOOLS_EXPORT const char * vtkParse_CacheString ( StringCache * cache , const char * cp , size_t n ) ; *
##  Free all strings that were created with vtkParse_NewString() or
##  with vtkParse_CacheString().
##  VTKWRAPPINGTOOLS_EXPORT void vtkParse_FreeStringCache ( StringCache * cache ) ; *
##  Compute a hash for a string, using the first n bytes.
##  VTKWRAPPINGTOOLS_EXPORT unsigned int vtkParse_HashString ( const char * cp , size_t n ) ; # __cplusplus [NewLine] }  extern "C" # [NewLine] # [NewLine]  VTK-HeaderTest-Exclude: vtkParseString.h
