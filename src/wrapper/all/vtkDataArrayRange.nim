...
## !!!Ignored construct:  # vtkDataArrayRange_h [NewLine] # vtkDataArrayRange_h [NewLine] # vtkAOSDataArrayTemplate.h [NewLine] # vtkDataArray.h [NewLine] # vtkDataArrayMeta.h [NewLine] # vtkDataArrayTupleRange_AOS.h [NewLine] # vtkDataArrayTupleRange_Generic.h [NewLine] # vtkDataArrayValueRange_AOS.h [NewLine] # vtkDataArrayValueRange_Generic.h [NewLine] # vtkMeta.h [NewLine] # vtkSmartPointer.h [NewLine] # < cassert > [NewLine] # < iterator > [NewLine] # < type_traits > [NewLine] *
##  @file vtkDataArrayRange.h
##
...
##  VTK_ITER_OPTIMIZE_START namespace vtk { namespace detail {  Internal detail: This utility is not directly needed by users of
##  DataArrayRange.
##
...
##  Range. template < typename ArrayTypePtr , ComponentIdType TupleSize > struct SelectTupleRange { private :  Allow this to work with vtkNew, vtkSmartPointer, etc. using ArrayType = typename detail :: StripPointers < ArrayTypePtr > :: type ; static_assert ( detail :: IsValidTupleSize < TupleSize > :: value , Invalid tuple size. ) ; static_assert ( detail :: IsVtkDataArray < ArrayType > :: value , Invalid array type. ) ; public : using type = typename std :: decay < decltype ( vtk :: detail :: DeclareTupleRangeSpecialization < ArrayType , TupleSize > ( std :: declval < ArrayType * > ( ) ) ) > :: type ; } ; template < typename ArrayTypePtr , ComponentIdType TupleSize > struct SelectValueRange { private :  Allow this to work with vtkNew, vtkSmartPointer, etc. using ArrayType = typename detail :: StripPointers < ArrayTypePtr > :: type ; static_assert ( detail :: IsValidTupleSize < TupleSize > :: value , Invalid tuple size. ) ; static_assert ( detail :: IsVtkDataArray < ArrayType > :: value , Invalid array type. ) ; public : using type = typename std :: remove_reference < decltype ( vtk :: detail :: DeclareValueRangeSpecialization < ArrayType , TupleSize > ( std :: declval < ArrayType * > ( ) ) ) > :: type ; } ; }  end namespace detail *
...
##  }
##  ```
##  template < ComponentIdType TupleSize = detail :: DynamicTupleSize , typename ArrayTypePtr = vtkDataArray * > VTK_ITER_INLINE auto DataArrayTupleRange ( const ArrayTypePtr & array , TupleIdType start = - 1 , TupleIdType end = - 1 ) -> typename detail :: SelectTupleRange < ArrayTypePtr , TupleSize > :: type {  Lookup specializations: using RangeType = typename detail :: SelectTupleRange < ArrayTypePtr , TupleSize > :: type ; assert ( array ) ; return RangeType ( array , start < 0 ? 0 : start , end < 0 ? array -> GetNumberOfTuples ( ) : end ) ; } *
...
##  }
##  ```
##  template < ComponentIdType TupleSize = detail :: DynamicTupleSize , typename ArrayTypePtr = vtkDataArray * > VTK_ITER_INLINE auto DataArrayValueRange ( const ArrayTypePtr & array , ValueIdType start = - 1 , ValueIdType end = - 1 ) -> typename detail :: SelectValueRange < ArrayTypePtr , TupleSize > :: type { using RangeType = typename detail :: SelectValueRange < ArrayTypePtr , TupleSize > :: type ; assert ( array ) ; return RangeType ( array , start < 0 ? 0 : start , end < 0 ? array -> GetNumberOfValues ( ) : end ) ; } }  end namespace vtk VTK_ITER_OPTIMIZE_END #  vtkDataArrayRange_h [NewLine]  VTK-HeaderTest-Exclude: vtkDataArrayRange.h
