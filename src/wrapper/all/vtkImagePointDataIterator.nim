...
## !!!Ignored construct:  # vtkImagePointDataIterator_h [NewLine] # vtkImagePointDataIterator_h [NewLine] # vtkImagingCoreModule.h  for export macro # vtkSystemIncludes.h [NewLine] class vtkDataArray ;
## Error: token expected: ; but got: [identifier]!!!

discard "forward decl of vtkImageData"
discard "forward decl of vtkImageStencilData"
discard "forward decl of vtkAlgorithm"
## !!!Ignored construct:  class VTKIMAGINGCORE_EXPORT vtkImagePointDataIterator { public : *
...
##  vtkImagePointDataIterator ( vtkImageData * image , const int extent [ 6 ] = nullptr , vtkImageStencilData * stencil = nullptr , vtkAlgorithm * algorithm = nullptr , int threadId = 0 ) { this -> Initialize ( image , extent , stencil , algorithm , threadId ) ; } *
...
##  void GetIndex ( int result [ 3 ] ) { result [ 0 ] = this -> Index [ 0 ] ; result [ 1 ] = this -> Index [ 1 ] ; result [ 2 ] = this -> Index [ 2 ] ; } /@} *
##  Get the index at the beginning of the current span.
...
##  static void * GetVoidPointer ( vtkDataArray * array , vtkIdType i = 0 , int * pixelIncrement = nullptr ) ; protected : *
...
##  void ReportProgress ( ) ; vtkIdType Id ;  the current point Id vtkIdType SpanEnd ;  end of current span vtkIdType RowEnd ;  end of current row vtkIdType SliceEnd ;  end of current slice vtkIdType End ;  end of data  Increments vtkIdType RowIncrement ;  to same position in next row vtkIdType SliceIncrement ;  to same position in next slice vtkIdType RowEndIncrement ;  from end of row to start of next row vtkIdType SliceEndIncrement ;  from end of slice to start of next slice  The extent, adjusted for the stencil int Extent [ 6 ] ;  Index-related items int Index [ 3 ] ; int StartY ;  Stencil-related items bool HasStencil ; bool InStencil ; int SpanSliceEndIncrement ; int SpanSliceIncrement ; int SpanIndex ; int * SpanCountPointer ; int * * SpanListPointer ;  Progress-related items vtkAlgorithm * Algorithm ; vtkIdType Count ; vtkIdType Target ; int ThreadId ; } ;
## Error: token expected: ; but got: [identifier]!!!
