...
## !!!Ignored construct:  # vtkClustering2DLayoutStrategy_h [NewLine] # vtkClustering2DLayoutStrategy_h [NewLine] # vtkGraphLayoutStrategy.h [NewLine] # vtkInfovisLayoutModule.h  For export macro [NewLine] # vtkSmartPointer.h  Required for smart pointer internal ivars. [NewLine] class vtkFastSplatter ;
## Error: token expected: ; but got: [identifier]!!!

discard "forward decl of vtkImageData"
discard "forward decl of vtkIntArray"
discard "forward decl of vtkFloatArray"
## !!!Ignored construct:  class VTKINFOVISLAYOUT_EXPORT vtkClustering2DLayoutStrategy : public vtkGraphLayoutStrategy { public : static vtkClustering2DLayoutStrategy * New ( ) ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkGraphLayoutStrategy Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkGraphLayoutStrategy :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkClustering2DLayoutStrategy :: IsTypeOf ( type ) ; } static vtkClustering2DLayoutStrategy * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkClustering2DLayoutStrategy * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkClustering2DLayoutStrategy * NewInstance ( ) const { return vtkClustering2DLayoutStrategy :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkGraphLayoutStrategy :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkClustering2DLayoutStrategy :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkClustering2DLayoutStrategy :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; /@{ *
...
##  int IsLayoutComplete ( ) override { return this -> LayoutComplete ; } protected : vtkClustering2DLayoutStrategy ( ) ; ~ vtkClustering2DLayoutStrategy ( ) override ; int MaxNumberOfIterations ;  Maximum number of iterations. float InitialTemperature ; float CoolDownRate ;  Cool-down rate.  Note:  Higher # = Slower rate. private :  An edge consists of two vertices joined together.
##  This struct acts as a "pointer" to those two vertices. struct vtkLayoutEdge_t { vtkIdType from ; vtkIdType to ; float weight ; int dead_edge ;  I'm making this an int so that the edge array is
##  word boundary aligned... but I'm not sure what
##  really happens in these days of magical compilers } ; using vtkLayoutEdge = struct vtkLayoutEdge_t ;  This class 'has a' vtkFastSplatter for the density grid vtkSmartPointer < vtkFastSplatter > DensityGrid ; vtkSmartPointer < vtkImageData > SplatImage ; vtkSmartPointer < vtkFloatArray > RepulsionArray ; vtkSmartPointer < vtkFloatArray > AttractionArray ; vtkSmartPointer < vtkIntArray > EdgeCountArray ; vtkLayoutEdge * EdgeArray ; int RandomSeed ; int IterationsPerLayout ; int TotalIterations ; int LayoutComplete ; float Temp ; float RestDistance ; float CuttingThreshold ;  Private helper methods void GenerateCircularSplat ( vtkImageData * splat , int x , int y ) ; void GenerateGaussianSplat ( vtkImageData * splat , int x , int y ) ; void ResolveCoincidentVertices ( ) ; vtkClustering2DLayoutStrategy ( const vtkClustering2DLayoutStrategy & ) = delete ; void operator = ( const vtkClustering2DLayoutStrategy & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
