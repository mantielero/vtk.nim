...
## !!!Ignored construct:  # QVTKOpenGLWindow_h [NewLine] # QVTKOpenGLWindow_h [NewLine] # < QOpenGLWindow > [NewLine] # < QScopedPointer >  for QScopedPointer. [NewLine] # QVTKInteractor.h  needed for QVTKInteractor # vtkGUISupportQtModule.h  for export macro # vtkNew.h  needed for vtkNew # vtkSmartPointer.h  needed for vtkSmartPointer [NewLine] class QVTKInteractor ;
## Error: token expected: ; but got: [identifier]!!!

discard "forward decl of QVTKInteractorAdapter"
discard "forward decl of QVTKRenderWindowAdapter"
discard "forward decl of vtkGenericOpenGLRenderWindow"
## !!!Ignored construct:  class VTKGUISUPPORTQT_EXPORT QVTKOpenGLWindow : public QOpenGLWindow { Q_OBJECT typedef QOpenGLWindow Superclass ; public : QVTKOpenGLWindow ( QOpenGLWindow :: UpdateBehavior updateBehavior = NoPartialUpdate , QWindow * parent = nullptr ) ; QVTKOpenGLWindow ( QOpenGLContext * shareContext , QOpenGLWindow :: UpdateBehavior updateBehavior = NoPartialUpdate , QWindow * parent = nullptr ) ; QVTKOpenGLWindow ( vtkGenericOpenGLRenderWindow * renderWindow , QOpenGLWindow :: UpdateBehavior updateBehavior = NoPartialUpdate , QWindow * parent = nullptr ) ; QVTKOpenGLWindow ( vtkGenericOpenGLRenderWindow * renderWindow , QOpenGLContext * shareContext , QOpenGLWindow :: UpdateBehavior updateBehavior = NoPartialUpdate , QWindow * parent = nullptr ) ; ~ QVTKOpenGLWindow ( ) override ; /@{ *
...
##  void setRenderWindow ( vtkGenericOpenGLRenderWindow * win ) ; void setRenderWindow ( vtkRenderWindow * win ) ; /@} *
...
##  void setEnableHiDPI ( bool enable ) ; bool enableHiDPI ( ) const { return this -> EnableHiDPI ; } /@} /@{ *
##  Set/Get unscaled DPI value. Defaults to 72, which is also the default value
##  in vtkWindow.
##  void setUnscaledDPI ( int ) ; int unscaledDPI ( ) const { return this -> UnscaledDPI ; } /@} /@{ *
...
##  void setCustomDevicePixelRatio ( double cdpr ) ; double customDevicePixelRatio ( ) const { return this -> CustomDevicePixelRatio ; } double effectiveDevicePixelRatio ( ) const ; /@} /@{ *
##  Set/get the default cursor to use for this widget.
##  void setDefaultCursor ( const QCursor & cursor ) ; const QCursor & defaultCursor ( ) const { return this -> DefaultCursor ; } /@} Q_SIGNALS : *
##  Signal emitted when any event has been receive, with the corresponding
##  event as argument.
##  void windowEvent ( QEvent * e ) ; protected Q_SLOTS : *
...
##  void cleanupContext ( ) ; void updateSize ( ) ; *
...
##  widget goes away.
##  friend class QVTKOpenGLStereoWidget ; protected : bool event ( QEvent * evt ) override ; void initializeGL ( ) override ; void paintGL ( ) override ; void resizeGL ( int w , int h ) override ; protected : vtkSmartPointer < vtkGenericOpenGLRenderWindow > RenderWindow ; QScopedPointer < QVTKRenderWindowAdapter > RenderWindowAdapter ; private : Q_DISABLE_COPY ( QVTKOpenGLWindow ) ; bool EnableHiDPI ; int UnscaledDPI ; double CustomDevicePixelRatio ; QCursor DefaultCursor ; } ;
## Error: token expected: ; but got: [identifier]!!!
