...
## !!!Ignored construct:  # vtkScatterPlotMatrix_h [NewLine] # vtkScatterPlotMatrix_h [NewLine] # vtkChartMatrix.h [NewLine] # vtkChartsCoreModule.h  For export macro # vtkColor.h  For member function return # vtkNew.h  For ivars # vtkSmartPointer.h  For ivars # vtkStdString.h  For ivars # vtkWeakPointer.h  For currentPainter [NewLine] class vtkStringArray ;
...
## !!!Ignored construct:  class VTKCHARTSCORE_EXPORT vtkScatterPlotMatrix : public vtkChartMatrix { public : enum { SCATTERPLOT , HISTOGRAM , ACTIVEPLOT , NOPLOT } ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkChartMatrix Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkChartMatrix :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkScatterPlotMatrix :: IsTypeOf ( type ) ; } static vtkScatterPlotMatrix * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkScatterPlotMatrix * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkScatterPlotMatrix * NewInstance ( ) const { return vtkScatterPlotMatrix :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkChartMatrix :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkScatterPlotMatrix :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkScatterPlotMatrix :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; *
##  Creates a new object.
##  static vtkScatterPlotMatrix * New ( ) ; *
##  Perform any updates to the item that may be necessary before rendering.
##  void Update ( ) override ; *
##  Paint event for the chart matrix.
##  bool Paint ( vtkContext2D * painter ) override ; void SetScene ( vtkContextScene * scene ) override ; *
...
##  int GetPlotType ( const vtkVector2i & pos ) ; int GetPlotType ( int row , int column ) ; /@} /@{ *
##  Set/get the scatter plot title.
##  void SetTitle ( const vtkStdString & title ) ; vtkStdString GetTitle ( ) ; /@} /@{ *
##  Set/get the text properties for the chart title, i.e. color, font, size.
##  void SetTitleProperties ( vtkTextProperty * prop ) ; vtkTextProperty * GetTitleProperties ( ) ; /@} /@{ *
...
##  void SetGridVisibility ( int plotType , bool visible ) ; bool GetGridVisibility ( int plotType ) ; /@} /@{ *
##  Sets the background color for the chart given a plot type, which refers to
##  vtkScatterPlotMatrix::{SCATTERPLOT, HISTOGRAM, ACTIVEPLOT}.
##  void SetBackgroundColor ( int plotType , const vtkColor4ub & color ) ; vtkColor4ub GetBackgroundColor ( int plotType ) ; /@} /@{ *
##  Sets the color for the axes given a plot type, which refers to
##  vtkScatterPlotMatrix::{SCATTERPLOT, HISTOGRAM, ACTIVEPLOT}.
##  void SetAxisColor ( int plotType , const vtkColor4ub & color ) ; vtkColor4ub GetAxisColor ( int plotType ) ; /@} /@{ *
##  Sets the color for the axes given a plot type, which refers to
##  vtkScatterPlotMatrix::{SCATTERPLOT, HISTOGRAM, ACTIVEPLOT}.
##  void SetGridColor ( int plotType , const vtkColor4ub & color ) ; vtkColor4ub GetGridColor ( int plotType ) ; /@} /@{ *
##  Sets whether or not the labels for the axes are visible, given a plot type,
##  which refers to
##  vtkScatterPlotMatrix::{SCATTERPLOT, HISTOGRAM, ACTIVEPLOT}.
##  void SetAxisLabelVisibility ( int plotType , bool visible ) ; bool GetAxisLabelVisibility ( int plotType ) ; /@} /@{ *
...
##  void SetAxisLabelProperties ( int plotType , vtkTextProperty * prop ) ; vtkTextProperty * GetAxisLabelProperties ( int plotType ) ; /@} /@{ *
##  Sets the axis label notation for the axes given a plot type, which refers to
##  vtkScatterPlotMatrix::{SCATTERPLOT, HISTOGRAM, ACTIVEPLOT}.
##  void SetAxisLabelNotation ( int plotType , int notation ) ; int GetAxisLabelNotation ( int plotType ) ; /@} /@{ *
##  Sets the axis label precision for the axes given a plot type, which refers to
##  vtkScatterPlotMatrix::{SCATTERPLOT, HISTOGRAM, ACTIVEPLOT}.
##  void SetAxisLabelPrecision ( int plotType , int precision ) ; int GetAxisLabelPrecision ( int plotType ) ; /@} /@{ *
##  Set chart's tooltip notation and precision, given a plot type, which refers to
##  vtkScatterPlotMatrix::{SCATTERPLOT, HISTOGRAM, ACTIVEPLOT}.
##  void SetTooltipNotation ( int plotType , int notation ) ; void SetTooltipPrecision ( int plotType , int precision ) ; int GetTooltipNotation ( int plotType ) ; int GetTooltipPrecision ( int plotType ) ; /@} *
...
##  void SetScatterPlotSelectedRowColumnColor ( const vtkColor4ub & color ) ; vtkColor4ub GetScatterPlotSelectedRowColumnColor ( ) ; /@} /@{ *
##  Set the scatter plot selected active chart background color.
##  void SetScatterPlotSelectedActiveColor ( const vtkColor4ub & color ) ; vtkColor4ub GetScatterPlotSelectedActiveColor ( ) ; /@} *
...
##  virtual void SetSelectionMode ( int ) ; virtual int GetSelectionMode ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << SelectionMode  of  << this -> SelectionMode ) ; return this -> SelectionMode ; } ; /@} *
...
##  virtual vtkChart * GetMainChart ( ) ; protected : vtkScatterPlotMatrix ( ) ; ~ vtkScatterPlotMatrix ( ) override ; *
...
##  virtual void AddSupplementaryPlot ( vtkChart * vtkNotUsed ( chart ) , int vtkNotUsed ( plotType ) , vtkStdString vtkNotUsed ( row ) , vtkStdString vtkNotUsed ( column ) , int vtkNotUsed ( plotCorner ) = 0 ) { }  The position of the active plot (defaults to 0, 1). vtkVector2i ActivePlot ;  A flag to show if the ActivePlot vector is valid or not bool ActivePlotValid ;  Weakly owned input data for the scatter plot matrix. vtkSmartPointer < vtkTable > Input ;  Strongly owned internal data for the column visibility. vtkNew < vtkStringArray > VisibleColumns ;  The number of bins in the histograms. int NumberOfBins ;  The title of the scatter plot matrix. vtkStdString Title ; vtkSmartPointer < vtkTextProperty > TitleProperties ;  The mode when the chart is doing selection. int SelectionMode ;  How many frames should animations consist of, 0 means no transitions. int NumberOfFrames ;  A flag to know if we are animating the scatter plot along an animation path bool Animating ; private : vtkScatterPlotMatrix ( const vtkScatterPlotMatrix & ) = delete ; void operator = ( const vtkScatterPlotMatrix & ) = delete ; class PIMPL ; PIMPL * Private ; friend class PIMPL ; vtkWeakPointer < vtkContext2D > CurrentPainter ; vtkMTimeType LayoutUpdatedTime ;  Go through the process of calculating axis ranges, etc... void UpdateAxes ( ) ; void ApplyAxisSetting ( vtkChart * chart , const vtkStdString & x , const vtkStdString & y ) ; } ;
## Error: token expected: ; but got: [identifier]!!!
