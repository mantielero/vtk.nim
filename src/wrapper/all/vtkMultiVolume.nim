...
## !!!Ignored construct:  # vtkMultiVolume_h [NewLine] # vtkMultiVolume_h [NewLine] # < array >  for std::array # < unordered_map >  For std::unordered_map [NewLine] # vtkMatrix4x4.h  For Matrix # vtkRenderingVolumeModule.h  For export macro # vtkSmartPointer.h  For vtkSmartPointer # vtkVolume.h [NewLine] class vtkAbstractVolumeMapper ;
...
## !!!Ignored construct:  class VTKRENDERINGVOLUME_EXPORT vtkMultiVolume : public vtkVolume { public : static vtkMultiVolume * New ( ) ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkVolume Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkVolume :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkMultiVolume :: IsTypeOf ( type ) ; } static vtkMultiVolume * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkMultiVolume * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkMultiVolume * NewInstance ( ) const { return vtkMultiVolume :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkVolume :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkMultiVolume :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkMultiVolume :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; /@{ *
##  Add / Remove a vtkVolume instance.
##  void SetVolume ( vtkVolume * volume , int port = 0 ) ; vtkVolume * GetVolume ( int port = 0 ) ; void RemoveVolume ( int port ) { this -> SetVolume ( nullptr , port ) ; } /@} /@{ *
...
##  \sa vtkVolume
##  void SetProperty ( vtkVolumeProperty * property ) override ; vtkVolumeProperty * GetProperty ( ) override ; /@} *
...
##  using vtkVolume :: GetMatrix ; vtkMatrix4x4 * GetMatrix ( ) override { return this -> Matrix ; } *
...
##  double * GetDataBounds ( ) { return this -> DataBounds . data ( ) ; } vtkMTimeType GetBoundsTime ( ) { return this -> BoundsComputeTime . GetMTime ( ) ; } *
...
##  double * GetDataGeometry ( ) { return this -> DataGeometry . data ( ) ; } protected : vtkMultiVolume ( ) ; ~ vtkMultiVolume ( ) override ; *
...
##  the box in X.
##  std :: array < double , 6 > ComputeAABounds ( double bounds [ 6 ] , vtkMatrix4x4 * T ) const ; std :: array < double , 6 > DataBounds ; std :: array < double , 24 > DataGeometry ; std :: unordered_map < int , vtkVolume * > Volumes ; vtkTimeStamp BoundsComputeTime ; vtkSmartPointer < vtkMatrix4x4 > TexToBBox ; private : vtkMultiVolume ( const vtkMultiVolume & ) = delete ; void operator = ( const vtkMultiVolume & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
