...
## !!!Ignored construct:  # vtkTetra_h [NewLine] # vtkTetra_h [NewLine] # vtkCell3D.h [NewLine] # vtkCommonDataModelModule.h  For export macro [NewLine] class vtkLine ;
...
## !!!Ignored construct:  class VTKCOMMONDATAMODEL_EXPORT vtkTetra : public vtkCell3D { public : static vtkTetra * New ( ) ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkCell3D Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkCell3D :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkTetra :: IsTypeOf ( type ) ; } static vtkTetra * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkTetra * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkTetra * NewInstance ( ) const { return vtkTetra :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkCell3D :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkTetra :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkTetra :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; /@{ *
...
##  int JacobianInverse ( double * * inverse , double derivs [ 12 ] ) ; static void InterpolationFunctions ( const double pcoords [ 3 ] , double weights [ 4 ] ) ; static void InterpolationDerivs ( const double pcoords [ 3 ] , double derivs [ 12 ] ) ; /@{ *
##  Compute the interpolation functions/derivatives
##  (aka shape functions/derivatives)
##  void InterpolateFunctions ( const double pcoords [ 3 ] , double weights [ 4 ] ) override { vtkTetra :: InterpolationFunctions ( pcoords , weights ) ; } void InterpolateDerivs ( const double pcoords [ 3 ] , double derivs [ 12 ] ) override { vtkTetra :: InterpolationDerivs ( pcoords , derivs ) ; } /@} /@{ *
...
##  static const vtkIdType * GetEdgeArray ( vtkIdType edgeId ) VTK_SIZEHINT ( 2 ) ; static const vtkIdType * GetFaceArray ( vtkIdType faceId ) VTK_SIZEHINT ( 3 ) ; /@} *
...
##  static bool ComputeCentroid ( vtkPoints * points , const vtkIdType * pointIds , double centroid [ 3 ] ) ; protected : vtkTetra ( ) ; ~ vtkTetra ( ) override ; vtkLine * Line ; vtkTriangle * Triangle ; private : vtkTetra ( const vtkTetra & ) = delete ; void operator = ( const vtkTetra & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
