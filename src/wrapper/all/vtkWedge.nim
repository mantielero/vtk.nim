...
## !!!Ignored construct:  # vtkWedge_h [NewLine] # vtkWedge_h [NewLine] # vtkCell3D.h [NewLine] # vtkCommonDataModelModule.h  For export macro [NewLine] class vtkLine ;
...
## !!!Ignored construct:  class VTKCOMMONDATAMODEL_EXPORT vtkWedge : public vtkCell3D { public : static vtkWedge * New ( ) ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkCell3D Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkCell3D :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkWedge :: IsTypeOf ( type ) ; } static vtkWedge * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkWedge * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkWedge * NewInstance ( ) const { return vtkWedge :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkCell3D :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkWedge :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkWedge :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; /@{ *
...
##  int GetParametricCenter ( double pcoords [ 3 ] ) override ; static void InterpolationFunctions ( const double pcoords [ 3 ] , double weights [ 6 ] ) ; static void InterpolationDerivs ( const double pcoords [ 3 ] , double derivs [ 18 ] ) ; /@{ *
##  Compute the interpolation functions/derivatives
##  (aka shape functions/derivatives)
##  void InterpolateFunctions ( const double pcoords [ 3 ] , double weights [ 6 ] ) override { vtkWedge :: InterpolationFunctions ( pcoords , weights ) ; } void InterpolateDerivs ( const double pcoords [ 3 ] , double derivs [ 18 ] ) override { vtkWedge :: InterpolationDerivs ( pcoords , derivs ) ; } int JacobianInverse ( const double pcoords [ 3 ] , double * * inverse , double derivs [ 18 ] ) ; /@} /@{ *
...
##  static const vtkIdType * GetEdgeArray ( vtkIdType edgeId ) VTK_SIZEHINT ( 2 ) ; static const vtkIdType * GetFaceArray ( vtkIdType faceId ) VTK_SIZEHINT ( MaximumFaceSize + 1 ) ; /@} *
...
##  static bool ComputeCentroid ( vtkPoints * points , const vtkIdType * pointIds , double centroid [ 3 ] ) ; protected : vtkWedge ( ) ; ~ vtkWedge ( ) override ; vtkLine * Line ; vtkTriangle * Triangle ; vtkQuad * Quad ; private : vtkWedge ( const vtkWedge & ) = delete ; void operator = ( const vtkWedge & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
