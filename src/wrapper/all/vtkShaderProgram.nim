...
## !!!Ignored construct:  # vtkShaderProgram_h [NewLine] # vtkShaderProgram_h [NewLine] # vtkObject.h [NewLine] # vtkRenderingOpenGL2Module.h  for export macro [NewLine] # < map >  For member variables. # < string >  For member variables. [NewLine] class vtkMatrix3x3 ;
## Error: token expected: ; but got: [identifier]!!!

discard "forward decl of vtkMatrix4x4"
discard "forward decl of vtkTransformFeedback"
discard "forward decl of vtkShader"
discard "forward decl of VertexArrayObject"
discard "forward decl of vtkWindow"
## !!!Ignored construct:  class VTKRENDERINGOPENGL2_EXPORT vtkShaderProgram : public vtkObject { public : static vtkShaderProgram * New ( ) ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkObject Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkObject :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkShaderProgram :: IsTypeOf ( type ) ; } static vtkShaderProgram * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkShaderProgram * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkShaderProgram * NewInstance ( ) const { return vtkShaderProgram :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkObject :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkShaderProgram :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkShaderProgram :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; /@{ *
##  Get the vertex shader for this program
##  virtual vtkShader * GetnameVertexShader ( ) { vtkDebugWithObjectMacro ( this , <<  returning  VertexShader  address  << static_cast < vtkShader * > ( this -> VertexShader ) ) ; return this -> VertexShader ; } ; void SetVertexShader ( vtkShader * ) ; /@} /@{ *
##  Get the fragment shader for this program
##  virtual vtkShader * GetnameVertexShaderFragmentShader ( ) { vtkDebugWithObjectMacro ( this , <<  returning  FragmentShader  address  << static_cast < vtkShader * > ( this -> FragmentShader ) ) ; return this -> FragmentShader ; } ; void SetFragmentShader ( vtkShader * ) ; /@} /@{ *
##  Get the geometry shader for this program
##  virtual vtkShader * GetnameVertexShaderFragmentShaderGeometryShader ( ) { vtkDebugWithObjectMacro ( this , <<  returning  GeometryShader  address  << static_cast < vtkShader * > ( this -> GeometryShader ) ) ; return this -> GeometryShader ; } ; void SetGeometryShader ( vtkShader * ) ; /@} /@{ *
##  Get/Set a TransformFeedbackCapture object on this shader program.
##  virtual vtkTransformFeedback * GetnameVertexShaderFragmentShaderGeometryShaderTransformFeedback ( ) { vtkDebugWithObjectMacro ( this , <<  returning  TransformFeedback  address  << static_cast < vtkTransformFeedback * > ( this -> TransformFeedback ) ) ; return this -> TransformFeedback ; } ; void SetTransformFeedback ( vtkTransformFeedback * tfc ) ; /@} /@{ *
##  Set/Get flag for if this program is compiled
##  virtual bool GetCompiled ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << Compiled  of  << this -> Compiled ) ; return this -> Compiled ; } ; virtual void SetCompiled ( bool _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  Compiled  to  << _arg ) ; if ( this -> Compiled != _arg ) { this -> Compiled = _arg ; this -> Modified ( ) ; } } ; virtual void CompiledOn ( ) { this -> SetCompiled ( static_cast < bool > ( 1 ) ) ; } virtual void CompiledOff ( ) { this -> SetCompiled ( static_cast < bool > ( 0 ) ) ; } ; /@} *
##  Set/Get the md5 hash of this program
##  std :: string GetMD5Hash ( ) const { return this -> MD5Hash ; } void SetMD5Hash ( const std :: string & hash ) { this -> MD5Hash = hash ; } * Options for attribute normalization. enum NormalizeOption { / The values range across the limits of the numeric type.
...
## / the shader program. Normalize , / The values should be used as-is. Do not perform any normalization. NoNormalize } ; *
...
##  void ReleaseGraphicsResources ( vtkWindow * win ) ; * Get the handle of the shader program. int GetHandle ( ) const { return Handle ; } * Get the error message (empty if none) for the shader program. std :: string GetError ( ) const { return Error ; } *
...
##  template < class T > bool SetAttributeArray ( const char * name , const T & array , int tupleSize , NormalizeOption normalize ) ; * Set the @p name uniform value to int @p v. bool SetUniformi ( const char * name , int v ) ; bool SetUniformf ( const char * name , float v ) ; bool SetUniform2i ( const char * name , const int v [ 2 ] ) ; bool SetUniform2f ( const char * name , const float v [ 2 ] ) ; bool SetUniform3f ( const char * name , const float v [ 3 ] ) ; bool SetUniform3f ( const char * name , const double v [ 3 ] ) ; bool SetUniform4f ( const char * name , const float v [ 4 ] ) ; bool SetUniform3uc ( const char * name , const unsigned char v [ 3 ] ) ;  maybe remove bool SetUniform4uc ( const char * name , const unsigned char v [ 4 ] ) ;  maybe remove bool SetUniformMatrix ( const char * name , vtkMatrix3x3 * v ) ; bool SetUniformMatrix ( const char * name , vtkMatrix4x4 * v ) ; bool SetUniformMatrix3x3 ( const char * name , float * v ) ; bool SetUniformMatrix4x4 ( const char * name , float * v ) ; * Set the @p name uniform array to @p f with @p count elements bool SetUniform1iv ( const char * name , const int count , const int * f ) ; bool SetUniform1fv ( const char * name , const int count , const float * f ) ; bool SetUniform2fv ( const char * name , const int count , const float * f ) ; bool SetUniform2fv ( const char * name , const int count , const float ( * f ) [ 2 ] ) ; bool SetUniform3fv ( const char * name , const int count , const float * f ) ; bool SetUniform3fv ( const char * name , const int count , const float ( * f ) [ 3 ] ) ; bool SetUniform4fv ( const char * name , const int count , const float * f ) ; bool SetUniform4fv ( const char * name , const int count , const float ( * f ) [ 4 ] ) ; bool SetUniformMatrix4x4v ( const char * name , const int count , float * v ) ;  How many outputs does this program produce
##  only valid for OpenGL 3.2 or later virtual void SetCompiledNumberOfOutputs ( unsigned int _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  NumberOfOutputs  to  << _arg ) ; if ( this -> NumberOfOutputs != _arg ) { this -> NumberOfOutputs = _arg ; this -> Modified ( ) ; } } ; *
...
##  name.
##  bool IsAttributeUsed ( const char * name ) ;  maps of std::string are super slow when calling find
...
##  huge CPU hog. struct cmp_str { bool operator ( ) ( const char * a , const char * b ) const { return strcmp ( a , b ) < 0 ; } } ; /@{ *
...
##  used in production.
##  virtual void SetFileNamePrefixForDebugging ( VTK_FILEPATH const char * _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  << FileNamePrefixForDebugging  to  << ( _arg ? _arg : (null) ) ) ; if ( this -> FileNamePrefixForDebugging == nullptr && _arg == nullptr ) { return ; } if ( this -> FileNamePrefixForDebugging && _arg && ( ! strcmp ( this -> FileNamePrefixForDebugging , _arg ) ) ) { return ; } delete [ ] this -> FileNamePrefixForDebugging ; if ( _arg ) { size_t n = strlen ( _arg ) + 1 ; char * cp1 = new char [ n ] ; const char * cp2 = ( _arg ) ; this -> FileNamePrefixForDebugging = cp1 ; do { * cp1 ++ = * cp2 ++ ; } while ( -- n ) ; } else { this -> FileNamePrefixForDebugging = nullptr ; } this -> Modified ( ) ; } ; virtual VTK_FILEPATH VTK_FUTURE_CONST char * GetFileNamePrefixForDebugging ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << FileNamePrefixForDebugging  of  << ( this -> FileNamePrefixForDebugging ? this -> FileNamePrefixForDebugging : (null) ) ) ; return this -> FileNamePrefixForDebugging ; } ; /@} /@{ *
...
##  enum UniformGroups { CameraGroup , LightingGroup , UserGroup ,  always will be last } ; void SetUniformGroupUpdateTime ( int , vtkMTimeType tm ) ; vtkMTimeType GetUniformGroupUpdateTime ( int ) ; /@}  returns the location for a uniform or attribute in
##  this program. Is cached for performance. int FindUniform ( const char * name ) ; int FindAttributeArray ( const char * name ) ; protected : vtkShaderProgram ( ) ; ~ vtkShaderProgram ( ) override ; **************************************************************
...
##  bool AttachShader ( const vtkShader * shader ) ; * Detach the supplied shader from this program.
##  @note A maximum of one Vertex shader and one Fragment shader can be
##  attached to a shader program.
##  @return true on success.
...
##  bool Bind ( ) ; * Releases the shader program from the current context. void Release ( ) ; ************ end ************************************* vtkShader * VertexShader ; vtkShader * FragmentShader ; vtkShader * GeometryShader ; vtkTransformFeedback * TransformFeedback ;  hash of the shader program std :: string MD5Hash ; bool SetAttributeArrayInternal ( const char * name , void * buffer , int type , int tupleSize , NormalizeOption normalize ) ; int Handle ; int VertexShaderHandle ; int FragmentShaderHandle ; int GeometryShaderHandle ; bool Linked ; bool Bound ; bool Compiled ;  for glsl 1.5 or later, how many outputs
##  does this shader create
##  they will be bound in order to
##  fragOutput0 fragOutput1 etc... unsigned int NumberOfOutputs ; std :: string Error ;  since we are using const char * arrays we have to
##  free our memory :-) void ClearMaps ( ) ; std :: map < const char * , int , cmp_str > AttributeLocs ; std :: map < const char * , int , cmp_str > UniformLocs ; std :: map < int , vtkMTimeType > UniformGroupMTimes ; friend class VertexArrayObject ; private : vtkShaderProgram ( const vtkShaderProgram & ) = delete ; void operator = ( const vtkShaderProgram & ) = delete ; char * FileNamePrefixForDebugging ; } ;
## Error: token expected: ; but got: [identifier]!!!
