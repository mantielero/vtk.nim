...
## !!!Ignored construct:  # vtkHyperTree_h [NewLine] # vtkHyperTree_h [NewLine] # vtkCommonDataModelModule.h  For export macro # vtkObject.h [NewLine] # < cassert >  Used internally # < memory >  std::shared_ptr [NewLine] class vtkBitArray ;
...
type
  VTKCOMMONDATAMODEL_EXPORTvtkHyperTreeData* {.
      importcpp: "VTKCOMMONDATAMODEL_EXPORT::vtkHyperTreeData",
      header: "vtkHyperTree.h", bycopy.} = object ##  Index of this tree in the hypertree grid
    TreeIndex* {.importc: "TreeIndex".}: vtkIdType ##  Number of levels in the tree
    NumberOfLevels* {.importc: "NumberOfLevels".}: cuint ##  Number of vertices in this tree (coarse and leaves)
    NumberOfVertices* {.importc: "NumberOfVertices".}: vtkIdType ##  Number of nodes (non-leaf vertices) in the tree
    NumberOfNodes* {.importc: "NumberOfNodes".}: vtkIdType ##  Offset start for the implicit global index mapping fixed by
                                                       ...
                                                       ##  of cells.
    GlobalIndexStart* {.importc: "GlobalIndexStart".}: vtkIdType


## =============================================================================

## !!!Ignored construct:  class VTKCOMMONDATAMODEL_EXPORT vtkHyperTree : public vtkObject { public : protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkObject Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkObject :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkHyperTree :: IsTypeOf ( type ) ; } static vtkHyperTree * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkHyperTree * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkHyperTree * NewInstance ( ) const { return vtkHyperTree :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkObject :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkHyperTree :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkHyperTree :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; *
...
##  void SetTreeIndex ( vtkIdType treeIndex ) { this -> Datas -> TreeIndex = treeIndex ; } vtkIdType GetTreeIndex ( ) const { return this -> Datas -> TreeIndex ; } /@} *
##  Return the number of levels.
##  unsigned int GetNumberOfLevels ( ) const { assert ( post: result_greater_or_equal_to_one && this -> Datas -> NumberOfLevels >= 1 ) ; return this -> Datas -> NumberOfLevels ; } *
...
##  vtkIdType GetNumberOfLeaves ( ) const { return this -> Datas -> NumberOfVertices - this -> Datas -> NumberOfNodes ; } *
...
##  level (0).
##  void GetScale ( double s [ 3 ] ) const ; double GetScale ( unsigned int d ) const ; /@} *
...
##  is returning).
##  VTK_NEWINSTANCE static vtkHyperTree * CreateInstance ( unsigned char branchFactor , unsigned char dimension ) ; *
...
##  NB: Ignore the attribute array because its size is added by the data set.
##  unsigned int GetActualMemorySize ( ) {  in kilibytes return static_cast < unsigned int > ( this -> GetActualMemorySizeBytes ( ) / 1024 ) ; } *
...
##  Return all scales.
##  std :: shared_ptr < vtkHyperTreeGridScales > GetScales ( ) const { assert ( this -> Scales != nullptr ) ; return this -> Scales ; } /@} protected : vtkHyperTree ( ) ; ~ vtkHyperTree ( ) override = default ; virtual void InitializePrivate ( ) = 0 ; virtual void PrintSelfPrivate ( ostream & os , vtkIndent indent ) = 0 ; virtual void CopyStructurePrivate ( vtkHyperTree * ht ) = 0 ; -- Global information  Branching factor of tree (2 or 3) unsigned char BranchFactor ;  Dimension of tree (1, 2, or 3) unsigned char Dimension ;  Number of children for coarse cell unsigned char NumberOfChildren ; -- Local information std :: shared_ptr < vtkHyperTreeData > Datas ;  Storage of pre-computed per-level cell scales
##  In hypertree grid, one description by hypertree.
##  In Uniform hypertree grid, one description by hypertree grid
##  (all cells, differents hypertree, are identicals by level). mutable std :: shared_ptr < vtkHyperTreeGridScales > Scales ; private : void InitializeBase ( unsigned char branchFactor , unsigned char dimension , unsigned char numberOfChildren ) ; vtkHyperTree ( const vtkHyperTree & ) = delete ; void operator = ( const vtkHyperTree & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
