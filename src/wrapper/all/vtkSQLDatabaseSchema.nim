...
## !!!Ignored construct:  # vtkSQLDatabaseSchema_h [NewLine] # vtkSQLDatabaseSchema_h [NewLine] # vtkIOSQLModule.h  For export macro # vtkObject.h [NewLine] # < cstdarg >  Because one method has a variable list of arguments [NewLine]  This is a list of known supported VTK SQL backend classes.
##  A particular SQL backend does not have to be listed here to be supported, but
##  these macros allow for the specification of SQL backend-specific database schema items. # VTK_SQL_ALLBACKENDS *  works for all backends # VTK_SQL_MYSQL vtkMySQLDatabase [NewLine] # VTK_SQL_POSTGRESQL vtkPostgreSQLDatabase [NewLine] # VTK_SQL_SQLITE vtkSQLiteDatabase [NewLine] class vtkSQLDatabaseSchemaInternals ;
## Error: token expected: ; but got: [identifier]!!!

## !!!Ignored construct:  class VTKIOSQL_EXPORT vtkSQLDatabaseSchema : public vtkObject { public : protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkObject Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkObject :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkSQLDatabaseSchema :: IsTypeOf ( type ) ; } static vtkSQLDatabaseSchema * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkSQLDatabaseSchema * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkSQLDatabaseSchema * NewInstance ( ) const { return vtkSQLDatabaseSchema :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkObject :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkSQLDatabaseSchema :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkSQLDatabaseSchema :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; static vtkSQLDatabaseSchema * New ( ) ; *
##  Basic data types for database columns
##  enum DatabaseColumnType { SERIAL = 0 ,  specifying the indices explicitly to prevent bad compiler mishaps SMALLINT = 1 , INTEGER = 2 , BIGINT = 3 , VARCHAR = 4 , TEXT = 5 , REAL = 6 , DOUBLE = 7 , BLOB = 8 , TIME = 9 , DATE = 10 , TIMESTAMP = 11 } ; *
##  Types of indices that can be generated for database tables
##  enum DatabaseIndexType { INDEX = 0 ,  Non-unique index of values in named columns UNIQUE = 1 ,  Index of values in named columns required to have at most one entry per pair of
##  valid values. PRIMARY_KEY = 2  Like UNIQUE but additionally this serves as the primary key for the table to
##  speed up insertions. } ; *
##  Events where database triggers can be registered.
##  enum DatabaseTriggerType { BEFORE_INSERT = 0 ,  Just before a row is inserted AFTER_INSERT = 1 ,  Just after a row is inserted BEFORE_UPDATE = 2 ,  Just before a row's values are changed AFTER_UPDATE = 3 ,  Just after a row's values are changed BEFORE_DELETE = 4 ,  Just before a row is deleted AFTER_DELETE = 5  Just after a row is deleted } ; *
...
##  virtual int AddColumnToTable ( int tblHandle , int colType , const char * colName , int colSize , const char * colOpts ) ; virtual int AddColumnToTable ( const char * tblName , int colType , const char * colName , int colSize , const char * colAttribs ) { return this -> AddColumnToTable ( this -> GetTableHandleFromName ( tblName ) , colType , colName , colSize , colAttribs ) ; } /@} /@{ *
##  Add an index to table.
##
##  The returned value is an index handle or -1 if an error occurred.
##  virtual int AddIndexToTable ( int tblHandle , int idxType , const char * idxName ) ; virtual int AddIndexToTable ( const char * tblName , int idxType , const char * idxName ) { return this -> AddIndexToTable ( this -> GetTableHandleFromName ( tblName ) , idxType , idxName ) ; } /@} /@{ *
##  Add a column to a table index.
##
##  The returned value is an index-column handle or -1 if an error occurred.
##  virtual int AddColumnToIndex ( int tblHandle , int idxHandle , int colHandle ) ; virtual int AddColumnToIndex ( const char * tblName , const char * idxName , const char * colName ) { int tblHandle = this -> GetTableHandleFromName ( tblName ) ; return this -> AddColumnToIndex ( tblHandle , this -> GetIndexHandleFromName ( tblName , idxName ) , this -> GetColumnHandleFromName ( tblName , colName ) ) ; } /@} /@{ *
...
##  virtual int AddOptionToTable ( int tblHandle , const char * optText , const char * optBackend = VTK_SQL_ALLBACKENDS ) ; virtual int AddOptionToTable ( const char * tblName , const char * optStr , const char * optBackend = VTK_SQL_ALLBACKENDS ) { return this -> AddOptionToTable ( this -> GetTableHandleFromName ( tblName ) , optStr , optBackend ) ; } /@} *
...
##  virtual void SetName ( const char * _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  << Name  to  << ( _arg ? _arg : (null) ) ) ; if ( this -> Name == nullptr && _arg == nullptr ) { return ; } if ( this -> Name && _arg && ( ! strcmp ( this -> Name , _arg ) ) ) { return ; } delete [ ] this -> Name ; if ( _arg ) { size_t n = strlen ( _arg ) + 1 ; char * cp1 = new char [ n ] ; const char * cp2 = ( _arg ) ; this -> Name = cp1 ; do { * cp1 ++ = * cp2 ++ ; } while ( -- n ) ; } else { this -> Name = nullptr ; } this -> Modified ( ) ; } ; virtual char * GetName ( ) { vtkDebugWithObjectMacro ( this , <<  returning  << Name  of  << ( this -> Name ? this -> Name : (null) ) ) ; return this -> Name ; } ; /@}  Tokens passed to AddTable to indicate the type of data that follows. Random integers chosen to
##  prevent mishaps. enum VarargTokens { COLUMN_TOKEN = 58 , INDEX_TOKEN = 63 , INDEX_COLUMN_TOKEN = 65 , END_INDEX_TOKEN = 75 , TRIGGER_TOKEN = 81 , OPTION_TOKEN = 86 , END_TABLE_TOKEN = 99 } ; *
...
##  return 0;
##  }
##  int AddTableMultipleArguments ( const char * tblName , ... ) ; protected : vtkSQLDatabaseSchema ( ) ; ~ vtkSQLDatabaseSchema ( ) override ; char * Name ; class vtkSQLDatabaseSchemaInternals * Internals ; private : vtkSQLDatabaseSchema ( const vtkSQLDatabaseSchema & ) = delete ; void operator = ( const vtkSQLDatabaseSchema & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
