...
## !!!Ignored construct:  # vtkHyperTreeGrid_h [NewLine] # vtkHyperTreeGrid_h [NewLine] # vtkCommonDataModelModule.h  For export macro # vtkDataObject.h [NewLine] # vtkDeprecation.h  for deprecation macro # vtkNew.h  vtkSmartPointer # vtkSmartPointer.h  vtkSmartPointer [NewLine] # < cassert >  std::assert # < map >  std::map # < memory >  std::shared_ptr [NewLine] class vtkBitArray ;
## Error: token expected: ; but got: [identifier]!!!

discard "forward decl of vtkBoundingBox"
discard "forward decl of vtkCellLinks"
discard "forward decl of vtkCollection"
discard "forward decl of vtkDataArray"
discard "forward decl of vtkHyperTree"
discard "forward decl of vtkHyperTreeGridOrientedCursor"
...
discard "forward decl of vtkHyperTreeGridNonOrientedMooreSuperCursorLight"
...
## !!!Ignored construct:  class VTKCOMMONDATAMODEL_EXPORT vtkHyperTreeGrid : public vtkDataObject { public : static vtkInformationIntegerKey * LEVELS ( ) ; static vtkInformationIntegerKey * DIMENSION ( ) ; static vtkInformationIntegerKey * ORIENTATION ( ) ; static vtkInformationDoubleVectorKey * SIZES ( ) ; static vtkHyperTreeGrid * New ( ) ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkDataObject Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkDataObject :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkHyperTreeGrid :: IsTypeOf ( type ) ; } static vtkHyperTreeGrid * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkHyperTreeGrid * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkHyperTreeGrid * NewInstance ( ) const { return vtkHyperTreeGrid :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkDataObject :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkHyperTreeGrid :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkHyperTreeGrid :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; *
...
##  virtual void SetModeSqueeze ( const char * _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  << ModeSqueeze  to  << ( _arg ? _arg : (null) ) ) ; if ( this -> ModeSqueeze == nullptr && _arg == nullptr ) { return ; } if ( this -> ModeSqueeze && _arg && ( ! strcmp ( this -> ModeSqueeze , _arg ) ) ) { return ; } delete [ ] this -> ModeSqueeze ; if ( _arg ) { size_t n = strlen ( _arg ) + 1 ; char * cp1 = new char [ n ] ; const char * cp2 = ( _arg ) ; this -> ModeSqueeze = cp1 ; do { * cp1 ++ = * cp2 ++ ; } while ( -- n ) ; } else { this -> ModeSqueeze = nullptr ; } this -> Modified ( ) ; } ;  By copy virtual char * GetModeSqueeze ( ) { vtkDebugWithObjectMacro ( this , <<  returning  << ModeSqueeze  of  << ( this -> ModeSqueeze ? this -> ModeSqueeze : (null) ) ) ; return this -> ModeSqueeze ; } ; *
...
##  void SetDimensions ( const unsigned int dims [ 3 ] ) ; void SetDimensions ( const int dims [ 3 ] ) ; void SetDimensions ( unsigned int i , unsigned int j , unsigned int k ) ; void SetDimensions ( int i , int j , int k ) ; /@} /@{ *
##  Get dimensions of this rectilinear grid dataset.
##  The dimensions correspond to the number of points
##  const unsigned int * GetDimensions ( ) const VTK_SIZEHINT ( 3 ) ;  JB Dommage, car vtkGetVectorMacro(Dimensions,int,3); not const function void GetDimensions ( int dim [ 3 ] ) const ; void GetDimensions ( unsigned int dim [ 3 ] ) const ; /@} /@{ *
...
##  const unsigned int * GetCellDims ( ) const VTK_SIZEHINT ( 3 ) ; void GetCellDims ( int cellDims [ 3 ] ) const ; void GetCellDims ( unsigned int cellDims [ 3 ] ) const ; /@}  -------------------------------------------------------------------------- /@{ *
...
##  void Get1DAxis ( unsigned int & axis ) const { assert ( pre: valid_dim && this -> GetDimension ( ) == 1 ) ; axis = this -> Axis [ 0 ] ; } /@} /@{ *
##  JB Retourne l'indice des deux dimensions valides.
##  void Get2DAxes ( unsigned int & axis1 , unsigned int & axis2 ) const { assert ( pre: valid_dim && this -> GetDimension ( ) == 2 ) ; axis1 = this -> Axis [ 0 ] ; axis2 = this -> Axis [ 1 ] ; } /@} /@{ *
##  JB Get the axis information (used for CopyStructure)
##  const unsigned int * GetAxes ( ) const { return this -> Axis ; } /@} /@{ *
##  The number of children each node can have.
##   vtkGetMacro(NumberOfChildren, unsigned int); not const unsigned int GetNumberOfChildren ( ) const { return this -> NumberOfChildren ; } /@} *
...
##  virtual void SetTransposedRootIndexing ( bool _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  TransposedRootIndexing  to  << _arg ) ; if ( this -> TransposedRootIndexing != _arg ) { this -> TransposedRootIndexing = _arg ; this -> Modified ( ) ; } } ; virtual bool GetTransposedRootIndexing ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << TransposedRootIndexing  of  << this -> TransposedRootIndexing ) ; return this -> TransposedRootIndexing ; } ; void SetIndexingModeToKJI ( ) { this -> SetTransposedRootIndexing ( false ) ; } void SetIndexingModeToIJK ( ) { this -> SetTransposedRootIndexing ( true ) ; } /@} /@{ *
...
##  virtual bool GetTransposedRootIndexingFreezeState ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << FreezeState  of  << this -> FreezeState ) ; return this -> FreezeState ; } ; /@} /@{ *
##  Set/Get the subdivision factor in the grid refinement scheme
##  void SetBranchFactor ( unsigned int ) ; unsigned int GetBranchFactor ( ) const { return this -> BranchFactor ; } /@} *
...
##  VTK_DEPRECATED_IN_9_2_0 ( Please use the renamed version, GetNumberOfCells(). ) vtkIdType GetNumberOfVertices ( ) ; *
...
##  virtual void CopyCoordinates ( const vtkHyperTreeGrid * output ) ; virtual void SetFixedCoordinates ( unsigned int axis , double value ) ; /@} /@{ *
##  Set/Get the blanking mask of primal leaf cells
##  void SetMask ( vtkBitArray * ) ; virtual vtkBitArray * GetnameXCoordinatesYCoordinatesZCoordinatesMask ( ) { vtkDebugWithObjectMacro ( this , <<  returning  Mask  address  << static_cast < vtkBitArray * > ( this -> Mask ) ) ; return this -> Mask ; } ; /@} *
##  Determine whether blanking mask is empty or not
##  bool HasMask ( ) ; /@{ *
##  Set/Get presence or absence of interface
##  virtual void SetTransposedRootIndexingHasInterface ( bool _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  HasInterface  to  << _arg ) ; if ( this -> HasInterface != _arg ) { this -> HasInterface = _arg ; this -> Modified ( ) ; } } ; virtual bool GetTransposedRootIndexingFreezeStateHasInterface ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << HasInterface  of  << this -> HasInterface ) ; return this -> HasInterface ; } ; virtual void HasInterfaceOn ( ) { this -> SetHasInterface ( static_cast < bool > ( 1 ) ) ; } virtual void HasInterfaceOff ( ) { this -> SetHasInterface ( static_cast < bool > ( 0 ) ) ; } ; /@} /@{ *
##  Set/Get names of interface normal vectors arrays
##  virtual void SetModeSqueezeInterfaceNormalsName ( const char * _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  << InterfaceNormalsName  to  << ( _arg ? _arg : (null) ) ) ; if ( this -> InterfaceNormalsName == nullptr && _arg == nullptr ) { return ; } if ( this -> InterfaceNormalsName && _arg && ( ! strcmp ( this -> InterfaceNormalsName , _arg ) ) ) { return ; } delete [ ] this -> InterfaceNormalsName ; if ( _arg ) { size_t n = strlen ( _arg ) + 1 ; char * cp1 = new char [ n ] ; const char * cp2 = ( _arg ) ; this -> InterfaceNormalsName = cp1 ; do { * cp1 ++ = * cp2 ++ ; } while ( -- n ) ; } else { this -> InterfaceNormalsName = nullptr ; } this -> Modified ( ) ; } ; virtual char * GetModeSqueezeInterfaceNormalsName ( ) { vtkDebugWithObjectMacro ( this , <<  returning  << InterfaceNormalsName  of  << ( this -> InterfaceNormalsName ? this -> InterfaceNormalsName : (null) ) ) ; return this -> InterfaceNormalsName ; } ; /@} /@{ *
##  Set/Get names of interface intercepts arrays
##  virtual void SetModeSqueezeInterfaceNormalsNameInterfaceInterceptsName ( const char * _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  << InterfaceInterceptsName  to  << ( _arg ? _arg : (null) ) ) ; if ( this -> InterfaceInterceptsName == nullptr && _arg == nullptr ) { return ; } if ( this -> InterfaceInterceptsName && _arg && ( ! strcmp ( this -> InterfaceInterceptsName , _arg ) ) ) { return ; } delete [ ] this -> InterfaceInterceptsName ; if ( _arg ) { size_t n = strlen ( _arg ) + 1 ; char * cp1 = new char [ n ] ; const char * cp2 = ( _arg ) ; this -> InterfaceInterceptsName = cp1 ; do { * cp1 ++ = * cp2 ++ ; } while ( -- n ) ; } else { this -> InterfaceInterceptsName = nullptr ; } this -> Modified ( ) ; } ; virtual char * GetModeSqueezeInterfaceNormalsNameInterfaceInterceptsName ( ) { vtkDebugWithObjectMacro ( this , <<  returning  << InterfaceInterceptsName  of  << ( this -> InterfaceInterceptsName ? this -> InterfaceInterceptsName : (null) ) ) ; return this -> InterfaceInterceptsName ; } ; /@} /@{ *
##  Set/Get depth limiter value
##  virtual void SetTransposedRootIndexingHasInterfaceDepthLimiter ( unsigned int _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  DepthLimiter  to  << _arg ) ; if ( this -> DepthLimiter != _arg ) { this -> DepthLimiter = _arg ; this -> Modified ( ) ; } } ; virtual unsigned int GetTransposedRootIndexingFreezeStateHasInterfaceDepthLimiter ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << DepthLimiter  of  << this -> DepthLimiter ) ; return this -> DepthLimiter ; } ; /@} *
##  JB
##  void InitializeOrientedCursor ( vtkHyperTreeGridOrientedCursor * cursor , vtkIdType index , bool create = false ) ; vtkHyperTreeGridOrientedCursor * NewOrientedCursor ( vtkIdType index , bool create = false ) ; *
##  JB
##  void InitializeOrientedGeometryCursor ( vtkHyperTreeGridOrientedGeometryCursor * cursor , vtkIdType index , bool create = false ) ; vtkHyperTreeGridOrientedGeometryCursor * NewOrientedGeometryCursor ( vtkIdType index , bool create = false ) ; *
##  JB
##  void InitializeNonOrientedCursor ( vtkHyperTreeGridNonOrientedCursor * cursor , vtkIdType index , bool create = false ) ; vtkHyperTreeGridNonOrientedCursor * NewNonOrientedCursor ( vtkIdType index , bool create = false ) ; *
##  JB
##  void InitializeNonOrientedGeometryCursor ( vtkHyperTreeGridNonOrientedGeometryCursor * cursor , vtkIdType index , bool create = false ) ; vtkHyperTreeGridNonOrientedGeometryCursor * NewNonOrientedGeometryCursor ( vtkIdType index , bool create = false ) ; *
##  JB Retourne un curseur geometrique pointant une des mailles comportant la position spatiale x
##  vtkHyperTreeGridNonOrientedGeometryCursor * FindNonOrientedGeometryCursor ( double x [ 3 ] ) ; private : unsigned int RecurseDichotomic ( double value , vtkDoubleArray * coord , unsigned int ideb , unsigned int ifin ) const ; unsigned int FindDichotomic ( double value , vtkDataArray * coord ) const ; public : virtual unsigned int FindDichotomicX ( double value ) const ; virtual unsigned int FindDichotomicY ( double value ) const ; virtual unsigned int FindDichotomicZ ( double value ) const ; *
##  JB
##  void InitializeNonOrientedVonNeumannSuperCursor ( vtkHyperTreeGridNonOrientedVonNeumannSuperCursor * cursor , vtkIdType index , bool create = false ) ; vtkHyperTreeGridNonOrientedVonNeumannSuperCursor * NewNonOrientedVonNeumannSuperCursor ( vtkIdType index , bool create = false ) ; *
##  JB
##  void InitializeNonOrientedVonNeumannSuperCursorLight ( vtkHyperTreeGridNonOrientedVonNeumannSuperCursorLight * cursor , vtkIdType index , bool create = false ) ; vtkHyperTreeGridNonOrientedVonNeumannSuperCursorLight * NewNonOrientedVonNeumannSuperCursorLight ( vtkIdType index , bool create = false ) ; *
##  JB
##  void InitializeNonOrientedMooreSuperCursor ( vtkHyperTreeGridNonOrientedMooreSuperCursor * cursor , vtkIdType index , bool create = false ) ; vtkHyperTreeGridNonOrientedMooreSuperCursor * NewNonOrientedMooreSuperCursor ( vtkIdType index , bool create = false ) ; *
##  JB
##  void InitializeNonOrientedMooreSuperCursorLight ( vtkHyperTreeGridNonOrientedMooreSuperCursorLight * cursor , vtkIdType index , bool create = false ) ; vtkHyperTreeGridNonOrientedMooreSuperCursorLight * NewNonOrientedMooreSuperCursorLight ( vtkIdType index , bool create = false ) ; *
...
##  class VTKCOMMONDATAMODEL_EXPORT vtkHyperTreeGridIterator { public : vtkHyperTreeGridIterator ( ) = default ; *
...
##  Get the next tree and set its index then increment the iterator.
##  Returns 0 at the end.
##  vtkHyperTree * GetNextTree ( ) ; protected : std :: map < vtkIdType , vtkSmartPointer < vtkHyperTree >> :: iterator Iterator ; vtkHyperTreeGrid * Grid ; } ; *
...
##  static vtkHyperTreeGrid * GetData ( vtkInformation * info ) ; static vtkHyperTreeGrid * GetData ( vtkInformationVector * v , int i = 0 ) ; /@} *
...
##  vtkIdType GetNumberOfCells ( ) ; protected : *
##  Constructor with default bounds (0,1, 0,1, 0,1).
##  vtkHyperTreeGrid ( ) ; *
##  Destructor
##  ~ vtkHyperTreeGrid ( ) override ; *
##  JB ModeSqueeze
##  char * ModeSqueeze ; double Bounds [ 6 ] ;  (xmin,xmax, ymin,ymax, zmin,zmax) geometric bounds double Center [ 3 ] ;  geometric center bool FreezeState ; unsigned int BranchFactor ;  2 or 3 unsigned int Dimension ;  1, 2, or 3 /@{ *
##  These arrays pointers are caches used to avoid a string comparison (when
##  getting ghost arrays using GetArray(name))
##  vtkUnsignedCharArray * TreeGhostArray ; bool TreeGhostArrayCached ; /@} private : unsigned int Orientation ;  0, 1, or 2 unsigned int Axis [ 2 ] ; protected : unsigned int NumberOfChildren ; bool TransposedRootIndexing ;  --------------------------------
##  RectilinearGrid common fields
##  -------------------------------- private : unsigned int Dimensions [ 3 ] ;  Just for GetDimensions unsigned int CellDims [ 3 ] ;  Just for GetCellDims protected : int DataDescription ; int Extent [ 6 ] ; bool WithCoordinates ; vtkDataArray * XCoordinates ; vtkDataArray * YCoordinates ; vtkDataArray * ZCoordinates ;  -------------------------------- vtkBitArray * Mask ; vtkBitArray * PureMask ; bool InitPureMask ; bool HasInterface ; char * InterfaceNormalsName ; char * InterfaceInterceptsName ; std :: map < vtkIdType , vtkSmartPointer < vtkHyperTree >> HyperTrees ; vtkNew < vtkCellData > CellData ;  Scalars, vectors, etc. associated w/ each point unsigned int DepthLimiter ; private : vtkHyperTreeGrid ( const vtkHyperTreeGrid & ) = delete ; void operator = ( const vtkHyperTreeGrid & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
