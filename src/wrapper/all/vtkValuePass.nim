...
## !!!Ignored construct:  # vtkValuePass_h [NewLine] # vtkValuePass_h [NewLine] # vtkOpenGLRenderPass.h [NewLine] # vtkRenderingOpenGL2Module.h  For export macro # vtkSmartPointer.h for ivar [NewLine] class vtkAbstractArray ;
...
## !!!Ignored construct:  class VTKRENDERINGOPENGL2_EXPORT vtkValuePass : public vtkOpenGLRenderPass { public : enum Mode { INVERTIBLE_LUT = 1 , FLOATING_POINT = 2 } ; static vtkValuePass * New ( ) ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkOpenGLRenderPass Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkOpenGLRenderPass :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkValuePass :: IsTypeOf ( type ) ; } static vtkValuePass * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkValuePass * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkValuePass * NewInstance ( ) const { return vtkValuePass :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkOpenGLRenderPass :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkValuePass :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkValuePass :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; void SetInputArrayToProcess ( int fieldAssociation , const char * name ) ; void SetInputArrayToProcess ( int fieldAssociation , int fieldId ) ; void SetInputComponentToProcess ( int component ) ; *
...
##  the value array.
##  int * GetFloatImageExtents ( ) ; void ReleaseGraphicsResources ( vtkWindow * win ) override ; protected : vtkValuePass ( ) ; ~ vtkValuePass ( ) override ; //@{ *
...
##  void EndMapperRender ( vtkMapper * mapper , vtkProperty * property ) ; void InitializeBuffers ( vtkRenderer * ren ) ; *
##  Add necessary shader definitions.
##  bool UpdateShaders ( std :: string & VSSource , std :: string & FSSource ) ; *
##  Bind shader variables.
##  void BindAttributes ( vtkShaderProgram * prog , vtkOpenGLVertexArrayObject * VAO ) ; void BindUniforms ( vtkShaderProgram * prog ) ; /@{ *
##  Methods managing graphics resources required during FLOATING_POINT mode.
##  bool HasWindowSizeChanged ( vtkRenderer * ren ) ; bool InitializeFBO ( vtkRenderer * ren ) ; void ReleaseFBO ( vtkWindow * win ) ; /@} class vtkInternalsFloat ; vtkInternalsFloat * ImplFloat ; class vtkInternalsInvertible ; vtkInternalsInvertible * ImplInv ; struct Parameters ; Parameters * PassState ; int RenderingMode ; private : vtkDataArray * GetCurrentArray ( vtkMapper * mapper , Parameters * arrayPar ) ; vtkAbstractArray * GetArrayFromCompositeData ( vtkMapper * mapper , Parameters * arrayPar ) ; vtkSmartPointer < vtkAbstractArray > MultiBlocksArray ; void PopulateCellCellMap ( const vtkRenderState * s ) ; vtkValuePass ( const vtkValuePass & ) = delete ; void operator = ( const vtkValuePass & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
