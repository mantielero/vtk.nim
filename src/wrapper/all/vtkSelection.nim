...
## !!!Ignored construct:  # vtkSelection_h [NewLine] # vtkSelection_h [NewLine] # vtkCommonDataModelModule.h  For export macro # vtkDataObject.h [NewLine] # vtkSmartPointer.h  for  vtkSmartPointer. [NewLine] # < memory >  for unique_ptr. # < string >  for string. [NewLine] class vtkSelectionNode ;
## Error: token expected: ; but got: [identifier]!!!

discard "forward decl of vtkSignedCharArray"
## !!!Ignored construct:  class VTKCOMMONDATAMODEL_EXPORT vtkSelection : public vtkDataObject { public : protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkDataObject Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkDataObject :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkSelection :: IsTypeOf ( type ) ; } static vtkSelection * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkSelection * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkSelection * NewInstance ( ) const { return vtkSelection :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkDataObject :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkSelection :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkSelection :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; static vtkSelection * New ( ) ; *
...
##  virtual void RemoveNode ( unsigned int idx ) ; virtual void RemoveNode ( const std :: string & name ) ; virtual void RemoveNode ( vtkSelectionNode * ) ; /@} *
...
##  `Evaluate` call.
##  virtual void SetExpression ( std :: string _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  Expression  to  << _arg ) ; if ( this -> Expression != _arg ) { this -> Expression = _arg ; this -> Modified ( ) ; } } ; virtual std :: string GetExpression ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << Expression  of  << this -> Expression ) ; return this -> Expression ; } ; /@} *
...
##  virtual void Dump ( ) ; virtual void Dump ( ostream & os ) ; /@} /@{ *
##  Retrieve a vtkSelection stored inside an invormation object.
##  static vtkSelection * GetData ( vtkInformation * info ) ; static vtkSelection * GetData ( vtkInformationVector * v , int i = 0 ) ; /@} *
...
##  vtkSmartPointers).
##  template < typename MapType > vtkSmartPointer < vtkSignedCharArray > Evaluate ( const MapType & values_map ) const ; protected : vtkSelection ( ) ; ~ vtkSelection ( ) override ; std :: string Expression ; private : vtkSelection ( const vtkSelection & ) = delete ; void operator = ( const vtkSelection & ) = delete ; class vtkInternals ; vtkInternals * Internals ; } ;
## Error: token expected: ; but got: [identifier]!!!

## ----------------------------------------------------------------------------

proc Evaluate*(values_map: MapType): vtkSmartPointer[
    VTKCOMMONDATAMODEL_EXPORTvtkSignedCharArray] {.noSideEffect,
    importcpp: "VTKCOMMONDATAMODEL_EXPORT::Evaluate(@)", header: "vtkSelection.h".}