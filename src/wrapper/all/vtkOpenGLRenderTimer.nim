...
## !!!Ignored construct:  # vtkOpenGLRenderTimer_h [NewLine] # vtkOpenGLRenderTimer_h [NewLine] # vtkRenderingOpenGL2Module.h  For export macro # vtkType.h  For vtkTypeUint64, etc [NewLine] class VTKRENDERINGOPENGL2_EXPORT vtkOpenGLRenderTimer { public : vtkOpenGLRenderTimer ( ) ; ~ vtkOpenGLRenderTimer ( ) ; *
...
##  float GetElapsedSeconds ( ) ; float GetElapsedMilliseconds ( ) ; vtkTypeUInt64 GetElapsedNanoseconds ( ) ; /@} /@{ *
...
##  in this class.
##  void ReusableStart ( ) ; void ReusableStop ( ) ; float GetReusableElapsedSeconds ( ) ; /@} *
##  If Ready() returns true, return the start or stop time in nanoseconds.
##  @{
##  vtkTypeUInt64 GetStartTime ( ) ; vtkTypeUInt64 GetStopTime ( ) ; *@} *
##  Simply calls Reset() to ensure that query ids are freed. All stored timing
##  information will be lost.
##  void ReleaseGraphicsResources ( ) ; protected : bool StartReady ; bool EndReady ; vtkTypeUInt32 StartQuery ; vtkTypeUInt32 EndQuery ; vtkTypeUInt64 StartTime ; vtkTypeUInt64 EndTime ; bool ReusableStarted ; bool ReusableEnded ; private : vtkOpenGLRenderTimer ( const vtkOpenGLRenderTimer & ) = delete ; void operator = ( const vtkOpenGLRenderTimer & ) = delete ; } ;
