...
## !!!Ignored construct:  # vtkCellArrayIterator_h [NewLine] # vtkCellArrayIterator_h [NewLine] # vtkCommonDataModelModule.h  For export macro # vtkObject.h [NewLine] # vtkCellArray.h  Needed for inline methods # vtkIdList.h  Needed for inline methods # vtkSmartPointer.h  For vtkSmartPointer [NewLine] # < cassert >  for assert # < type_traits >  for std::enable_if [NewLine] class VTKCOMMONDATAMODEL_EXPORT vtkCellArrayIterator : public vtkObject { public : /@{ *
##  Standard methods for instantiation, type information, and printing.
##  protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkObject Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkObject :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkCellArrayIterator :: IsTypeOf ( type ) ; } static vtkCellArrayIterator * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkCellArrayIterator * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkCellArrayIterator * NewInstance ( ) const { return vtkCellArrayIterator :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkObject :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkCellArrayIterator :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkCellArrayIterator :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; static vtkCellArrayIterator * New ( ) ; /@} *
...
##  void GoToCell ( vtkIdType cellId ) { this -> CurrentCellId = cellId ; this -> NumberOfCells = this -> CellArray -> GetNumberOfCells ( ) ; assert ( cellId <= this -> NumberOfCells ) ; } *
...
##  (which is typical).
##  void GetCellAtId ( vtkIdType cellId , vtkIdType & numCellPts , vtkIdType const * & cellPts ) { this -> GoToCell ( cellId ) ; this -> GetCurrentCell ( numCellPts , cellPts ) ; } void GetCellAtId ( vtkIdType cellId , vtkIdList * cellIds ) { this -> GoToCell ( cellId ) ; this -> GetCurrentCell ( cellIds ) ; } vtkIdList * GetCellAtId ( vtkIdType cellId ) { this -> GoToCell ( cellId ) ; return this -> GetCurrentCell ( ) ; } /@} *
...
##  void GoToFirstCell ( ) { this -> CurrentCellId = 0 ; this -> NumberOfCells = this -> CellArray -> GetNumberOfCells ( ) ; } *
...
##  void GetCurrentCell ( vtkIdType & cellSize , vtkIdType const * & cellPoints ) { assert ( this -> CurrentCellId < this -> NumberOfCells ) ;  Either refer to vtkCellArray storage buffer, or copy into local buffer if ( this -> CellArray -> IsStorageShareable ( ) ) { this -> CellArray -> GetCellAtId ( this -> CurrentCellId , cellSize , cellPoints ) ; } else  or copy into local iterator buffer. { this -> CellArray -> GetCellAtId ( this -> CurrentCellId , this -> TempCell ) ; cellSize = this -> TempCell -> GetNumberOfIds ( ) ; cellPoints = this -> TempCell -> GetPointer ( 0 ) ; } } void GetCurrentCell ( vtkIdList * ids ) { assert ( this -> CurrentCellId < this -> NumberOfCells ) ; this -> CellArray -> GetCellAtId ( this -> CurrentCellId , ids ) ; } vtkIdList * GetCurrentCell ( ) { assert ( this -> CurrentCellId < this -> NumberOfCells ) ; this -> CellArray -> GetCellAtId ( this -> CurrentCellId , this -> TempCell ) ; return this -> TempCell ; } /@} *
...
##  cell's points).
##  void ReplaceCurrentCell ( vtkIdList * list ) { assert ( this -> CurrentCellId < this -> NumberOfCells ) ; this -> CellArray -> ReplaceCellAtId ( this -> CurrentCellId , list ) ; } *
...
##  void ReplaceCurrentCell ( vtkIdType npts , const vtkIdType * pts ) { assert ( this -> CurrentCellId < this -> NumberOfCells ) ; this -> CellArray -> ReplaceCellAtId ( this -> CurrentCellId , npts , pts ) ; } *
##  Reverses the order of the point ids in the current cell.
##  void ReverseCurrentCell ( ) { assert ( this -> CurrentCellId < this -> NumberOfCells ) ; this -> CellArray -> ReverseCellAtId ( this -> CurrentCellId ) ; } friend class vtkCellArray ; protected : vtkCellArrayIterator ( ) = default ; ~ vtkCellArrayIterator ( ) override = default ; virtual void SetCellArray ( vtkCellArray * _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  CellArray  to  << _arg ) ; if ( this -> CellArray != _arg ) { this -> CellArray = _arg ; this -> Modified ( ) ; } } ; vtkSmartPointer < vtkCellArray > CellArray ; vtkNew < vtkIdList > TempCell ; vtkIdType CurrentCellId ; vtkIdType NumberOfCells ; private : vtkCellArrayIterator ( const vtkCellArrayIterator & ) = delete ; void operator = ( const vtkCellArrayIterator & ) = delete ; } ;
