...
## !!!Ignored construct:  # vtkUnstructuredGrid_h [NewLine] # vtkUnstructuredGrid_h [NewLine] # vtkAbstractCellLinks.h  For vtkAbstractCellLinks # vtkCellArray.h  inline GetCellPoints() # vtkCommonDataModelModule.h  For export macro # vtkDeprecation.h  For deprecation # vtkIdTypeArray.h  inline GetCellPoints() # vtkUnstructuredGridBase.h [NewLine] # vtkSmartPointer.h  for smart pointer [NewLine] class vtkCellArray ;
...
discard "forward decl of vtkIdTypeArray"
## !!!Ignored construct:  class VTKCOMMONDATAMODEL_EXPORT vtkUnstructuredGrid : public vtkUnstructuredGridBase { public : *
##  Standard instantiation method.
##  static vtkUnstructuredGrid * New ( ) ; static vtkUnstructuredGrid * ExtendedNew ( ) ; /@{ *
##  Standard methods for type information and printing.
##  protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkUnstructuredGridBase Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkUnstructuredGridBase :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkUnstructuredGrid :: IsTypeOf ( type ) ; } static vtkUnstructuredGrid * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkUnstructuredGrid * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkUnstructuredGrid * NewInstance ( ) const { return vtkUnstructuredGrid :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkUnstructuredGridBase :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkUnstructuredGrid :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkUnstructuredGrid :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; /@} *
...
##  @sa Squeeze();
##  bool AllocateEstimate ( vtkIdType numCells , vtkIdType maxCellSize ) { return this -> AllocateExact ( numCells , numCells * maxCellSize ) ; } *
##  @brief Pre-allocate memory in internal data structures. Does not change
##  the number of cells, only the array capacities. Existing data is NOT
##  preserved.
##  @param numCells The number of expected cells in the dataset.
...
##  void Allocate ( vtkIdType numCells = 1000 , int vtkNotUsed ( extSize ) = 1000 ) override { this -> AllocateExact ( numCells , numCells ) ; } /@{ *
##  Standard vtkDataSet methods; see vtkDataSet.h for documentation.
##  void Reset ( ) ; void CopyStructure ( vtkDataSet * ds ) override ; vtkIdType GetNumberOfCells ( ) override ; using vtkDataSet :: GetCell ; vtkCell * GetCell ( vtkIdType cellId ) override ; void GetCell ( vtkIdType cellId , vtkGenericCell * cell ) override ; void GetCellBounds ( vtkIdType cellId , double bounds [ 6 ] ) override ; void GetCellPoints ( vtkIdType cellId , vtkIdList * ptIds ) override ; void GetPointCells ( vtkIdType ptId , vtkIdList * cellIds ) override ; vtkCellIterator * NewCellIterator ( ) override ; /@} *
...
##  VTK_DEPRECATED_IN_9_2_0 ( Please use GetDistinctCellTypesArray() instead. ) void GetCellTypes ( vtkCellTypes * types ) override ; *
##  Get a list of types of cells in a dataset. The list consists of an array
##  of types (not necessarily in any order), with a single entry per type.
##  For example a dataset with 5 triangles, 3 lines, and 100 hexahedra would
##  result in a list of three entries, corresponding to the types VTK_TRIANGLE,
...
##  void GetCellPoints ( vtkIdType cellId , vtkIdType & npts , vtkIdType const * & pts ) { this -> Connectivity -> GetCellAtId ( cellId , npts , pts ) ; } *
##  A higher-performing variant of the virtual vtkDataSet::GetCellPoints()
##  for unstructured grids. Given a cellId, return the number of defining
##  points and the list of points defining the cell.
##
...
##  void GetCellPoints ( vtkIdType cellId , vtkIdType & npts , vtkIdType const * & pts , vtkIdList * ptIds ) override { this -> Connectivity -> GetCellAtId ( cellId , npts , pts , ptIds ) ; } /@{ *
...
##  has been called).
##  void GetPointCells ( vtkIdType ptId , vtkIdType & ncells , vtkIdType * & cells ) VTK_SIZEHINT ( cells , ncells ) ; /@} *
...
##  virtual void SetLinks ( vtkAbstractCellLinks * _arg ) { { vtkDebugWithObjectMacro ( this , <<  setting  << Links  to  << _arg ) ; if ( this -> Links != _arg ) { this -> Links = _arg ; this -> Modified ( ) ; } } ; } ; virtual vtkAbstractCellLinks * Getname ( ) { vtkDebugWithObjectMacro ( this , <<  returning  name  address  << static_cast < vtkAbstractCellLinks * > ( this -> Links ) ) ; return this -> Links ; } ; /@} *
...
##  void SetCells ( int type , vtkCellArray * cells ) ; void SetCells ( int * types , vtkCellArray * cells ) ; void SetCells ( vtkUnsignedCharArray * cellTypes , vtkCellArray * cells ) ; void SetCells ( vtkUnsignedCharArray * cellTypes , vtkCellArray * cells , vtkIdTypeArray * faceLocations , vtkIdTypeArray * faces ) ; /@} *
...
##  MODIFIED.
##  void GetCellNeighbors ( vtkIdType cellId , vtkIdList * ptIds , vtkIdList * cellIds ) override { this -> GetCellNeighbors ( cellId , ptIds -> GetNumberOfIds ( ) , ptIds -> GetPointer ( 0 ) , cellIds ) ; } void GetCellNeighbors ( vtkIdType cellId , vtkIdType npts , const vtkIdType * ptIds , vtkIdList * cellIds ) ; /@} /@{ *
...
##  bool IsCellBoundary ( vtkIdType cellId , vtkIdType npts , const vtkIdType * ptIds ) ; bool IsCellBoundary ( vtkIdType cellId , vtkIdType npts , const vtkIdType * ptIds , vtkIdList * cellIds ) ; /@} /@{ *
##  Use these methods only if the dataset has been specified as
##  Editable. See vtkPointSet for more information.
##  vtkIdType InsertNextLinkedCell ( int type , int npts , const vtkIdType pts [ ] ) VTK_SIZEHINT ( pts , npts ) ; void RemoveReferenceToCell ( vtkIdType ptId , vtkIdType cellId ) ; void AddReferenceToCell ( vtkIdType ptId , vtkIdType cellId ) ; void ResizeCellList ( vtkIdType ptId , int size ) ; /@} /@{ *
##  Set / Get the piece and the number of pieces. Similar to extent in 3D.
##  virtual int GetPiece ( ) ; virtual int GetNumberOfPieces ( ) ; /@} *
##  Get the ghost level.
...
##  static vtkUnstructuredGrid * GetData ( vtkInformation * info ) ; static vtkUnstructuredGrid * GetData ( vtkInformationVector * v , int i = 0 ) ; /@} *
...
##  vtkIdTypeArray * GetFaces ( ) ; vtkIdTypeArray * GetFaceLocations ( ) ; /@} *
...
##  static void ConvertFaceStreamPointIds ( vtkIdType nfaces , vtkIdType * faceStream , vtkIdType * idMap ) ; ====================== Begin Legacy Methods ================================ *
...
##  all the cells that use a point), the cell links array is built. vtkSmartPointer < vtkCellArray > Connectivity ; vtkSmartPointer < vtkAbstractCellLinks > Links ; vtkSmartPointer < vtkUnsignedCharArray > Types ;  Set of all cell types present in the grid. All entries are unique. vtkSmartPointer < vtkCellTypes > DistinctCellTypes ;  The DistinctCellTypes is cached, so we keep track of the last time it was
##  updated so we can compare it to the modified time of the Types array. vtkMTimeType DistinctCellTypesUpdateMTime ;  Special support for polyhedra/cells with explicit face representations.
...
##  (n,i,j,k,n,i,j,k,...). vtkSmartPointer < vtkIdTypeArray > Faces ; vtkSmartPointer < vtkIdTypeArray > FaceLocations ;  Legacy support -- stores the old-style cell array locations. vtkSmartPointer < vtkIdTypeArray > CellLocations ; vtkIdType InternalInsertNextCell ( int type , vtkIdType npts , const vtkIdType ptIds [ ] ) override ; vtkIdType InternalInsertNextCell ( int type , vtkIdList * ptIds ) override ; vtkIdType InternalInsertNextCell ( int type , vtkIdType npts , const vtkIdType ptIds [ ] , vtkIdType nfaces , const vtkIdType faces [ ] ) override ; void InternalReplaceCell ( vtkIdType cellId , int npts , const vtkIdType pts [ ] ) override ; private :  Hide these from the user and the compiler. vtkUnstructuredGrid ( const vtkUnstructuredGrid & ) = delete ; void operator = ( const vtkUnstructuredGrid & ) = delete ; void Cleanup ( ) ; } ;
## Error: token expected: ; but got: [identifier]!!!
