...
## !!!Ignored construct:  # vtkGLTFDocumentLoader_h [NewLine] # vtkGLTFDocumentLoader_h [NewLine] # vtkIOGeometryModule.h  For export macro # vtkObject.h [NewLine] # vtkSmartPointer.h  For SmartPointer [NewLine] # < map >  For std::map # < memory >  For std::shared_ptr # < string >  For std::string # < vector >  For std::vector [NewLine] class vtkCellArray ;
...
## !!!Ignored construct:  class VTKIOGEOMETRY_EXPORT vtkGLTFDocumentLoader : public vtkObject { public : static vtkGLTFDocumentLoader * New ( ) ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkObject Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkObject :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkGLTFDocumentLoader :: IsTypeOf ( type ) ; } static vtkGLTFDocumentLoader * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkGLTFDocumentLoader * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkGLTFDocumentLoader * NewInstance ( ) const { return vtkGLTFDocumentLoader :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkObject :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkGLTFDocumentLoader :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkGLTFDocumentLoader :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; *
...
##  enum class ComponentType : unsigned short { BYTE = 5120 , UNSIGNED_BYTE = 5121 , SHORT = 5122 , UNSIGNED_SHORT = 5123 , UNSIGNED_INT = 5125 , FLOAT = 5126 } ;  The following structs help deserialize a glTF document, representing each object. As such,
...
##  struct BufferView { int Buffer ; int ByteOffset ; int ByteLength ; int ByteStride ; int Target ; std :: string Name ; } ; *
...
##  struct Sparse { int Count ; int IndicesBufferView ; int IndicesByteOffset ; ComponentType IndicesComponentType ; int ValuesBufferView ; int ValuesByteOffset ; } ; int BufferView ; int ByteOffset ; ComponentType ComponentTypeValue ; bool Normalized ; int Count ; unsigned int NumberOfComponents ; AccessorType Type ; std :: vector < double > Max ; std :: vector < double > Min ; bool IsSparse ; Sparse SparseObject ; std :: string Name ; } ; *
...
##  struct MorphTarget {  accessor indices from the .gltf file, the map's keys correspond to attribute names std :: map < std :: string , int > AttributeIndices ;  attribute values std :: map < std :: string , vtkSmartPointer < vtkFloatArray >> AttributeValues ; } ; *
...
##  struct Primitive {  accessor indices from the .glTF file, the map's keys correspond to attribute names std :: map < std :: string , int > AttributeIndices ; int IndicesId ; vtkSmartPointer < vtkCellArray > Indices ;  attribute values from buffer data std :: map < std :: string , vtkSmartPointer < vtkDataArray >> AttributeValues ; vtkSmartPointer < vtkPolyData > Geometry ; std :: vector < MorphTarget > Targets ; int Material ; int Mode ; int CellSize ;  1, 2 or 3, depending on draw mode } ; *
...
##  struct Node { std :: vector < int > Children ; int Camera ; int Mesh ; int Skin ; vtkSmartPointer < vtkMatrix4x4 > Transform ; vtkSmartPointer < vtkMatrix4x4 > GlobalTransform ; bool TRSLoaded ; vtkSmartPointer < vtkMatrix4x4 > Matrix ; std :: vector < float > InitialRotation ; std :: vector < float > InitialTranslation ; std :: vector < float > InitialScale ; std :: vector < float > InitialWeights ; std :: vector < float > Rotation ; std :: vector < float > Translation ; std :: vector < float > Scale ; std :: vector < float > Weights ;  Object-specific extension metadata struct Extensions {  KHR_lights_punctual extension struct KHRLightsPunctual { int Light = - 1 ; } ; Node :: Extensions :: KHRLightsPunctual KHRLightsPunctualMetaData ; } ; Node :: Extensions ExtensionMetaData ; std :: string Name ; void UpdateTransform ( ) ; } ; *
##  This struct describes a glTF mesh object.
##  A mesh contains an array of primitives.
##  struct Mesh { std :: vector < struct Primitive > Primitives ; std :: vector < float > Weights ; std :: string Name ; } ; *
...
##  struct TextureInfo { int Index = - 1 ; int TexCoord ; } ; *
...
##  struct Image { int BufferView ; std :: string MimeType ; std :: string Uri ; vtkSmartPointer < vtkImageData > ImageData ; std :: string Name ; } ; *
...
##  struct Material { enum class AlphaModeType : unsigned char { OPAQUE , MASK , BLEND } ; struct PbrMetallicRoughness { TextureInfo BaseColorTexture ; std :: vector < double > BaseColorFactor ; TextureInfo MetallicRoughnessTexture ; float MetallicFactor ; float RoughnessFactor ; } ; PbrMetallicRoughness PbrMetallicRoughness ; TextureInfo NormalTexture ; double NormalTextureScale ; TextureInfo OcclusionTexture ; double OcclusionTextureStrength ; TextureInfo EmissiveTexture ; std :: vector < double > EmissiveFactor ; AlphaModeType AlphaMode ; double AlphaCutoff ; bool DoubleSided ; std :: string Name ;  extension KHR_materials_unlit bool Unlit ; } ; *
...
##  struct Texture { int Sampler ; int Source ; std :: string Name ; } ; *
##  This struct describes a glTF sampler object.
##  Samplers specify filter and wrapping options corresponding to GL types.
##  struct Sampler { enum FilterType : unsigned short { NEAREST = 9728 , LINEAR = 9729 , NEAREST_MIPMAP_NEAREST = 9984 , LINEAR_MIPMAP_NEAREST = 9985 , NEAREST_MIPMAP_LINEAR = 9986 , LINEAR_MIPMAP_LINEAR = 9987 } ; enum WrapType : unsigned short { CLAMP_TO_EDGE = 33071 , MIRRORED_REPEAT = 33648 , REPEAT = 10497 } ; FilterType MagFilter ; FilterType MinFilter ; WrapType WrapS ; WrapType WrapT ; std :: string Name ; } ; *
...
##  struct Scene { std :: vector < unsigned int > Nodes ; std :: string Name ; } ; *
...
##  when relevant.
##  struct Skin { std :: vector < vtkSmartPointer < vtkMatrix4x4 >> InverseBindMatrices ; std :: vector < int > Joints ; int InverseBindMatricesAccessorId ; int Skeleton ; std :: string Name ; } ; *
...
##  struct Animation { struct Sampler { enum class InterpolationMode : unsigned char { LINEAR , STEP , CUBICSPLINE } ; InterpolationMode Interpolation ; unsigned int Input ; unsigned int Output ; int NumberOfComponents ; vtkSmartPointer < vtkFloatArray > InputData ; vtkSmartPointer < vtkFloatArray > OutputData ; *
##  Get the interpolated animation output at time t
##  void GetInterpolatedData ( float t , size_t numberOfComponents , std :: vector < float > * output , bool forceStep = false , bool isRotation = false ) const ; } ; struct Channel { enum class PathType : unsigned char { ROTATION , TRANSLATION , SCALE , WEIGHTS } ; int Sampler ; int TargetNode ; PathType TargetPath ; } ; float Duration ;  In seconds std :: vector < Animation :: Channel > Channels ; std :: vector < Animation :: Sampler > Samplers ; std :: string Name ; } ; *
...
##  struct Camera {  common properties double Znear ; double Zfar ; bool IsPerspective ;  if not, camera mode is orthographic  perspective double Xmag ; double Ymag ;  orthographic double Yfov ; double AspectRatio ; std :: string Name ; } ; *
...
##  example)
##  struct Extensions {  KHR_lights_punctual extension struct KHRLightsPunctual { struct Light { enum class LightType : unsigned char { DIRECTIONAL , POINT , SPOT } ; LightType Type ; std :: vector < double > Color ; double Intensity ; double Range ;  Type-specific parameters double SpotInnerConeAngle ; double SpotOuterConeAngle ; std :: string Name ; } ; std :: vector < Light > Lights ; } ; KHRLightsPunctual KHRLightsPunctualMetaData ; } ; *
##  This struct contains all data from a gltf asset
##  struct Model { std :: vector < Accessor > Accessors ; std :: vector < Animation > Animations ; std :: vector < std :: vector < char >> Buffers ; std :: vector < BufferView > BufferViews ; std :: vector < Camera > Cameras ; std :: vector < Image > Images ; std :: vector < Material > Materials ; std :: vector < Mesh > Meshes ; std :: vector < Node > Nodes ; std :: vector < Sampler > Samplers ; std :: vector < Scene > Scenes ; std :: vector < Skin > Skins ; std :: vector < Texture > Textures ; Extensions ExtensionMetaData ; std :: string BufferMetaData ; int DefaultScene ; std :: string FileName ; } ; *
...
##  static void ComputeJointMatrices ( const Model & model , const Skin & skin , Node & node , std :: vector < vtkSmartPointer < vtkMatrix4x4 >> & jointMats ) ; protected : vtkGLTFDocumentLoader ( ) = default ; ~ vtkGLTFDocumentLoader ( ) override = default ; private : struct AccessorLoadingWorker ; struct SparseAccessorLoadingWorker ; template < typename Type > struct BufferDataExtractionWorker ; vtkGLTFDocumentLoader ( const vtkGLTFDocumentLoader & ) = delete ; void operator = ( const vtkGLTFDocumentLoader & ) = delete ; *
...
##  bool LoadImageData ( ) ; std :: shared_ptr < Model > InternalModel ; static const std :: vector < std :: string > SupportedExtensions ; std :: vector < std :: string > UsedExtensions ; } ;
## Error: token expected: ; but got: [identifier]!!!
