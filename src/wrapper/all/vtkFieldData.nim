...
## !!!Ignored construct:  # vtkFieldData_h [NewLine] # vtkFieldData_h [NewLine] # vtkCommonDataModelModule.h  For export macro # vtkObject.h [NewLine] # vtkAbstractArray.h  Needed for inline methods. [NewLine] # < array >  For CachedGhostRangeType # < tuple >  For CachedGhostRangeType # < vector >  For list indices [NewLine] class vtkDoubleArray ;
## Error: token expected: ; but got: [identifier]!!!

discard "forward decl of vtkIdList"
discard "forward decl of vtkUnsignedCharArray"
## !!!Ignored construct:  class VTKCOMMONDATAMODEL_EXPORT vtkFieldData : public vtkObject { public : static vtkFieldData * New ( ) ; static vtkFieldData * ExtendedNew ( ) ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkObject Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkObject :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkFieldData :: IsTypeOf ( type ) ; } static vtkFieldData * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkFieldData * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkFieldData * NewInstance ( ) const { return vtkFieldData :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkObject :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkFieldData :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkFieldData :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; *
...
##  virtual void RemoveArray ( const char * name ) ; virtual void RemoveArray ( int index ) ; /@} *
...
##  vtkAbstractArray * GetAbstractArray ( const char * arrayName ) { int i ; return this -> GetAbstractArray ( arrayName , i ) ; } /@} /@{ *
##  Return 1 if an array with the given name could be found. 0 otherwise.
##  int HasArray ( const char * name ) { int i ; vtkAbstractArray * array = this -> GetAbstractArray ( name , i ) ;  assert( i == -1); return array ? 1 : 0 ; } /@} /@{ *
##  Get the name of ith array.
##  Note that this is equivalent to:
##  GetAbstractArray(i)->GetName() if ith array pointer is not nullptr
##  const char * GetArrayName ( int i ) { vtkAbstractArray * da = this -> GetAbstractArray ( i ) ; return da ? da -> GetName ( ) : nullptr ; } /@} *
...
##  void CopyFieldOn ( const char * name ) { this -> CopyFieldOnOff ( name , 1 ) ; } void CopyFieldOff ( const char * name ) { this -> CopyFieldOnOff ( name , 0 ) ; } *
...
##  bool GetRange ( const char * name , double range [ 2 ] , int comp = 0 ) ; bool GetRange ( int index , double range [ 2 ] , int comp = 0 ) ; bool GetFiniteRange ( const char * name , double range [ 2 ] , int comp = 0 ) ; bool GetFiniteRange ( int index , double range [ 2 ] , int comp = 0 ) ; /@} /@{ *
...
##  vtkPointData
##  vtkCellData
##  virtual unsigned char GetGhostsToSkip ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << GhostsToSkip  of  << this -> GhostsToSkip ) ; return this -> GhostsToSkip ; } ; virtual void SetGhostsToSkip ( unsigned char ) ; /@} *
...
##  @sa
##  vtkDataSetAttributes
##  virtual vtkUnsignedCharArray * GetnameGhostArray ( ) { vtkDebugWithObjectMacro ( this , <<  returning  GhostArray  address  << static_cast < vtkUnsignedCharArray * > ( this -> GhostArray ) ) ; return this -> GhostArray ; } ; protected : vtkFieldData ( ) ; ~ vtkFieldData ( ) override ; int NumberOfArrays ; int NumberOfActiveArrays ; vtkAbstractArray * * Data ; *
##  Set an array to define the field.
##  void SetArray ( int i , vtkAbstractArray * array ) ; *
##  Release all data but do not delete object.
##  virtual void InitializeFields ( ) ; struct CopyFieldFlag { char * ArrayName ; int IsCopied ; } ; CopyFieldFlag * CopyFieldFlags ;  the names of fields not to be copied int NumberOfFieldFlags ;  the number of fields not to be copied void CopyFieldOnOff ( const char * name , int onOff ) ; void ClearFieldFlags ( ) ; int FindFlag ( const char * field ) ; int GetFlag ( const char * field ) ; void CopyFlags ( const vtkFieldData * source ) ; int DoCopyAllOn ; int DoCopyAllOff ;
...
##  using CachedGhostRangeType = std :: tuple < vtkMTimeType , vtkMTimeType , std :: vector < double >> ; unsigned char GhostsToSkip ; vtkUnsignedCharArray * GhostArray ; /@{ *
...
##  std :: vector < std :: array < CachedGhostRangeType , 2 >> Ranges ; std :: vector < std :: array < CachedGhostRangeType , 2 >> FiniteRanges ; /@} private : vtkFieldData ( const vtkFieldData & ) = delete ; void operator = ( const vtkFieldData & ) = delete ; public : class VTKCOMMONDATAMODEL_EXPORT BasicIterator { public : BasicIterator ( ) = default ; BasicIterator ( const BasicIterator & source ) ; BasicIterator ( const int * list , unsigned int listSize ) ; BasicIterator & operator = ( const BasicIterator & source ) ; virtual ~ BasicIterator ( ) = default ; void PrintSelf ( ostream & os , vtkIndent indent ) ; int GetListSize ( ) const { return static_cast < int > ( this -> List . size ( ) ) ; } int GetCurrentIndex ( ) { return this -> List [ this -> Position ] ; } int BeginIndex ( ) { this -> Position = - 1 ; return this -> NextIndex ( ) ; } int End ( ) const { return ( this -> Position >= static_cast < int > ( this -> List . size ( ) ) ) ; } int NextIndex ( ) { this -> Position ++ ; return ( this -> End ( ) ? - 1 : this -> List [ this -> Position ] ) ; }  Support C++ range-for loops; e.g, code like
##  "for (const auto& i : basicIterator)". std :: vector < int > :: const_iterator begin ( ) { return this -> List . begin ( ) ; } std :: vector < int > :: const_iterator end ( ) { return this -> List . end ( ) ; } protected : std :: vector < int > List ; int Position ; } ; class VTKCOMMONDATAMODEL_EXPORT Iterator : public BasicIterator { public : Iterator ( const Iterator & source ) ; Iterator & operator = ( const Iterator & source ) ; ~ Iterator ( ) override ; Iterator ( vtkFieldData * dsa , const int * list = nullptr , unsigned int listSize = 0 ) ; vtkDataArray * Begin ( ) { this -> Position = - 1 ; return this -> Next ( ) ; } vtkDataArray * Next ( ) { this -> Position ++ ; if ( this -> End ( ) ) { return nullptr ; }  vtkFieldData::GetArray() can return null, which implies that
##  a the array at the given index in not a vtkDataArray subclass.
##  This iterator skips such arrays. vtkDataArray * cur = Fields -> GetArray ( this -> List [ this -> Position ] ) ; return ( cur ? cur : this -> Next ( ) ) ; } void DetachFieldData ( ) ; protected : vtkFieldData * Fields ; int Detached ; } ; } ;
## Error: token expected: ; but got: [identifier]!!!
