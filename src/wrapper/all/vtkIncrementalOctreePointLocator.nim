...
## !!!Ignored construct:  # vtkIncrementalOctreePointLocator_h [NewLine] # vtkIncrementalOctreePointLocator_h [NewLine] # vtkCommonDataModelModule.h  For export macro # vtkIncrementalPointLocator.h [NewLine] class vtkPoints ;
...
discard "forward decl of vtkIncrementalOctreeNode"
## !!!Ignored construct:  class VTKCOMMONDATAMODEL_EXPORT vtkIncrementalOctreePointLocator : public vtkIncrementalPointLocator { public : protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkIncrementalPointLocator Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkIncrementalPointLocator :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkIncrementalOctreePointLocator :: IsTypeOf ( type ) ; } static vtkIncrementalOctreePointLocator * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkIncrementalOctreePointLocator * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkIncrementalOctreePointLocator * NewInstance ( ) const { return vtkIncrementalOctreePointLocator :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkIncrementalPointLocator :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkIncrementalOctreePointLocator :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkIncrementalOctreePointLocator :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; static vtkIncrementalOctreePointLocator * New ( ) ; /@{ *
...
##  virtual void SetMaxPointsPerLeaf ( int _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  MaxPointsPerLeaf  to  << _arg ) ; if ( this -> MaxPointsPerLeaf != _arg ) { this -> MaxPointsPerLeaf = _arg ; this -> Modified ( ) ; } } ; virtual int GetMaxPointsPerLeaf ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << MaxPointsPerLeaf  of  << this -> MaxPointsPerLeaf ) ; return this -> MaxPointsPerLeaf ; } ; /@} /@{ *
##  Set/Get whether the search octree is built as a cubic shape or not.
##  virtual void SetMaxPointsPerLeafBuildCubicOctree ( vtkTypeBool _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  BuildCubicOctree  to  << _arg ) ; if ( this -> BuildCubicOctree != _arg ) { this -> BuildCubicOctree = _arg ; this -> Modified ( ) ; } } ; virtual vtkTypeBool GetMaxPointsPerLeafBuildCubicOctree ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << BuildCubicOctree  of  << this -> BuildCubicOctree ) ; return this -> BuildCubicOctree ; } ; virtual void BuildCubicOctreeOn ( ) { this -> SetBuildCubicOctree ( static_cast < vtkTypeBool > ( 1 ) ) ; } virtual void BuildCubicOctreeOff ( ) { this -> SetBuildCubicOctree ( static_cast < vtkTypeBool > ( 0 ) ) ; } ; /@} /@{ *
##  Get access to the vtkPoints object in which point coordinates are stored
##  for either point location or point insertion.
##  virtual vtkPoints * GetnameLocatorPoints ( ) { vtkDebugWithObjectMacro ( this , <<  returning  LocatorPoints  address  << static_cast < vtkPoints * > ( this -> LocatorPoints ) ) ; return this -> LocatorPoints ; } ; /@} *
...
##  Get the spatial bounding box of the octree.
##  double * GetBounds ( ) override { this -> GetBounds ( this -> Bounds ) ; return this -> Bounds ; } *
##  Get the number of points maintained by the octree.
##  int GetNumberOfPoints ( ) ; *
##  Get number of nodes in the tree.
##  virtual int GetMaxPointsPerLeafBuildCubicOctreeNumberOfNodes ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << NumberOfNodes  of  << this -> NumberOfNodes ) ; return this -> NumberOfNodes ; } ; *
...
##  void GenerateRepresentation ( int level , vtkPolyData * polysData ) override ; void GenerateRepresentation ( int level , vtkPolyData * polysData , bool ( * UserGetBounds ) ( void * data , vtkIncrementalOctreeNode * node , double * bounds ) , void * data ) ; @}  -------------------------------------------------------------------------
##  ---------------------------- Point  Location ----------------------------
##  ------------------------------------------------------------------------- *
...
##  void InsertPointWithoutChecking ( const double point [ 3 ] , vtkIdType & pntId , int insert ) ; vtkIncrementalOctreeNode * GetRoot ( ) const { return OctreeRootNode ; } *
...
##  int GetNumberOfLevels ( ) ; protected : vtkIncrementalOctreePointLocator ( ) ; ~ vtkIncrementalOctreePointLocator ( ) override ; private : vtkTypeBool BuildCubicOctree ; int MaxPointsPerLeaf ; double InsertTolerance2 ; double OctreeMaxDimSize ; double FudgeFactor ; vtkPoints * LocatorPoints ; vtkIncrementalOctreeNode * OctreeRootNode ; int NumberOfNodes ; void BuildLocatorInternal ( ) override ; *
...
##  static void AddPolys ( vtkIncrementalOctreeNode * node , vtkPoints * points , vtkCellArray * polygs , vtkIntArray * nodeIndexes , vtkIdType & cellIndex , bool ( * GetBounds ) ( void * data , vtkIncrementalOctreeNode * node , double * bounds ) , void * data ) ; *
...
##  vtkIdType FindClosestPointInLeafNode ( vtkIncrementalOctreeNode * leafNode , const double point [ 3 ] , double * dist2 ) ; *
...
##  vtkIdType FindClosestPointInSphere ( const double point [ 3 ] , double radius2 , vtkIncrementalOctreeNode * maskNode , double * minDist2 , const double * refDist2 ) ;  -------------------------------------------------------------------------
##  ---------------------------- Point  Location ----------------------------
##  ------------------------------------------------------------------------- *
...
##  vtkIdType FindClosestPointInSphereWithoutTolerance ( const double point [ 3 ] , double radius2 , vtkIncrementalOctreeNode * maskNode , double * minDist2 ) ; *
...
##  void FindPointsWithinSquaredRadius ( vtkIncrementalOctreeNode * node , double radius2 , const double point [ 3 ] , vtkIdList * idList ) ;  -------------------------------------------------------------------------
##  ---------------------------- Point Insertion ----------------------------
##  ------------------------------------------------------------------------- *
...
##  vtkIdType FindClosestPointInSphereWithTolerance ( const double point [ 3 ] , double radius2 , vtkIncrementalOctreeNode * maskNode , double * minDist2 ) ; *
##  Determine whether or not a given point has been inserted into the octree.
##  Return the id of the already inserted point if true, otherwise return -1.
...
##  vtkIdType IsInsertedPointForZeroTolerance ( const double x [ 3 ] , vtkIncrementalOctreeNode * * leafContainer ) ; *
##  Determine whether or not a given point has been inserted into the octree.
##  Return the id of the already inserted point if true, otherwise return -1.
##  Argument leafContainer is useful for access only if -1 is returned. This
##  returned parameter indicates the leaf node that contains the given point.
...
##  called in advance.
##  vtkIdType IsInsertedPointForNonZeroTolerance ( const double x [ 3 ] , vtkIncrementalOctreeNode * * leafContainer ) ; *
...
##  vtkIdType FindDuplicateFloatTypePointInVisitedLeafNode ( vtkIncrementalOctreeNode * leafNode , const double point [ 3 ] ) ; *
##  Given a point (under check for zero tolerance insertion) and a leaf node,
##  find its duplicate, if any, in the node and return the point index (-1 if
##  no duplicate is found). Note that the leaf node, already with at least one
##  point, is the container of the point under check. This function is invoked
##  for type VTK_DOUBLE. InitPointInsertion() should have been called.
##  vtkIdType FindDuplicateDoubleTypePointInVisitedLeafNode ( vtkIncrementalOctreeNode * leafNode , const double point [ 3 ] ) ; vtkIncrementalOctreePointLocator ( const vtkIncrementalOctreePointLocator & ) = delete ; void operator = ( const vtkIncrementalOctreePointLocator & ) = delete ; } ;
## Error: token expected: ; but got: [identifier]!!!
