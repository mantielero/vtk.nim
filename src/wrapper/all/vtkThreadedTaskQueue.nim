...
## !!!Ignored construct:  # vtkThreadedTaskQueue_h [NewLine] # vtkThreadedTaskQueue_h [NewLine] # vtkObject.h [NewLine] # < atomic > [NewLine] # < condition_variable > [NewLine] # < cstdint > [NewLine] # < functional > [NewLine] # < memory > [NewLine] # < mutex > [NewLine] # < thread > [NewLine] # ! defined ( __WRAP__ ) [NewLine] namespace vtkThreadedTaskQueueInternals { template < typename R > [end of template] class TaskQueue ; template < typename R > [end of template] class ResultQueue ; } ;
## Error: identifier expected, but got: ...!!!

## !!!Ignored construct:  template < typename R , typename ... Args > [end of template] class vtkThreadedTaskQueue { public : vtkThreadedTaskQueue ( std :: function < R ( Args ... ) > worker , bool strict_ordering = true , int buffer_size = - 1 , int max_concurrent_tasks = - 1 ) ; ~ vtkThreadedTaskQueue ( ) ; *
...
##  void Flush ( ) ; private : vtkThreadedTaskQueue ( const vtkThreadedTaskQueue & ) = delete ; void operator = ( const vtkThreadedTaskQueue & ) = delete ; std :: function < R ( Args ... ) > Worker ; std :: unique_ptr < vtkThreadedTaskQueueInternals :: TaskQueue < R >> Tasks ; std :: unique_ptr < vtkThreadedTaskQueueInternals :: ResultQueue < R >> Results ; int NumberOfThreads ; std :: unique_ptr < std :: thread [ ] > Threads ; } ;
## Error: identifier expected, but got: ...!!!

## !!!Ignored construct:  template < typename ... Args > [end of template] class vtkThreadedTaskQueue < void , Args ... > { public : vtkThreadedTaskQueue ( std :: function < void ( Args ... ) > worker , bool strict_ordering = true , int buffer_size = - 1 , int max_concurrent_tasks = - 1 ) ; ~ vtkThreadedTaskQueue ( ) ; *
##  Push arguments for the work
##  void Push ( Args && ... args ) ; *
##  Returns false if there's some result that may be popped right now or in the
##  future.
##  bool IsEmpty ( ) const ; *
##  Blocks till the queue becomes empty.
##  void Flush ( ) ; private : vtkThreadedTaskQueue ( const vtkThreadedTaskQueue & ) = delete ; void operator = ( const vtkThreadedTaskQueue & ) = delete ; std :: function < void ( Args ... ) > Worker ; std :: unique_ptr < vtkThreadedTaskQueueInternals :: TaskQueue < void >> Tasks ; std :: condition_variable ResultsCV ; std :: mutex NextResultIdMutex ; std :: atomic < std :: uint64_t > NextResultId ; int NumberOfThreads ; std :: unique_ptr < std :: thread [ ] > Threads ; } ;
## Error: identifier expected, but got: ...!!!
