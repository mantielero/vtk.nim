...
## !!!Ignored construct:  # vtkChartMatrix_h [NewLine] # vtkChartMatrix_h [NewLine] # vtkAbstractContextItem.h [NewLine] # vtkChartsCoreModule.h  For export macro # vtkRect.h  for ivars # vtkVector.h  For ivars [NewLine] # < map >  For specific gutter # < utility >  For specific gutter [NewLine] class vtkChart ;
## Error: token expected: ; but got: [identifier]!!!

## !!!Ignored construct:  class VTKCHARTSCORE_EXPORT vtkChartMatrix : public vtkAbstractContextItem { public : protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkAbstractContextItem Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkAbstractContextItem :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkChartMatrix :: IsTypeOf ( type ) ; } static vtkChartMatrix * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkChartMatrix * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkChartMatrix * NewInstance ( ) const { return vtkChartMatrix :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkAbstractContextItem :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkChartMatrix :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkChartMatrix :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; *
...
##  virtual void SetBorders ( int left , int bottom , int right , int top ) ; void SetBorderLeft ( int value ) ; void SetBorderBottom ( int value ) ; void SetBorderRight ( int value ) ; void SetBorderTop ( int value ) ; virtual void GetBorders ( int borders [ 4 ] ) { for ( int i = 0 ; i < 4 ; i ++ ) { borders [ i ] = this -> Borders [ i ] ; } } /@} /@{ *
##  Set the gutter that should be left between the charts in the matrix.
##  virtual void SetGutter ( const vtkVector2f & gutter ) ; void SetGutterX ( float value ) ; void SetGutterY ( float value ) ; /@} /@{ *
##  Set the gutter that should be left between the charts in the matrix.
##  virtual void SetPadding ( const float & padding ) ; /@} /@{ *
##  Set a specific resize that will move the bottom left point of a chart.
##  virtual void SetSpecificResize ( const vtkVector2i & index , const vtkVector2f & resize ) ; virtual void ClearSpecificResizes ( ) ; /@} *
...
##  virtual void Link ( const vtkVector2i & index1 , const vtkVector2i & index2 , int axis = 1 ) ; virtual void Link ( const size_t & flatIndex1 , const size_t & flatIndex2 , int axis = 1 ) ; /@} /@{ *
##  Link a chart to all other charts in this chart matrix for specified axis
##  virtual void LinkAll ( const vtkVector2i & index , int axis = 1 ) ; virtual void LinkAll ( const size_t & flatIndex , int axis = 1 ) ; /@} /@{ *
##  Unlink the two charts for specified axis i.e,
##  Chart at index2 will no longer mimic the axis range of chart at index1
##  virtual void Unlink ( const vtkVector2i & index1 , const vtkVector2i & index2 , int axis = 1 ) ; virtual void Unlink ( const size_t & flatIndex1 , const size_t & flatIndex2 , int axis = 1 ) ; /@} /@{ *
##  Unlink all charts from given chart for a specified axis.
##  virtual void UnlinkAll ( const vtkVector2i & index , int axis = 1 ) ; virtual void UnlinkAll ( const size_t & flatIndex , int axis = 1 ) ; /@} /@{ *
...
##  virtual void ResetLinks ( int axis = 1 ) ; virtual void ResetLinkedLayout ( ) ; /@} /@{ *
##  Set the rectangular region that this chart matrix will occupy.
##  Must also set FillStrategy to StretchType::CUSTOM
##  virtual void SetRect ( vtkRecti rect ) ; virtual vtkRecti GetRect ( ) VTK_FUTURE_CONST { vtkDebugWithObjectMacro ( this , <<  returning  << Rect  of  << this -> Rect ) ; return this -> Rect ; } ; /@} *
...
##  virtual void InitLayoutTraversal ( vtkVector2i & index , vtkVector2f & offset , vtkVector2f & increment ) ; virtual void GoToNextElement ( vtkVector2i & index , vtkVector2f & offset ) ; virtual bool IsDoneWithTraversal ( ) ; /@} *
...
##  virtual vtkRectf ComputeCurrentElementSceneRect ( const vtkVector2i & index , const vtkVector2f & offset , const vtkVector2f & increment ) ; enum class StretchType : unsigned int { SCENE = 0 , CUSTOM } ; /@{ *
...
##  virtual void SetFillStrategy ( StretchType _arg ) { vtkDebugWithObjectMacro ( this , <<  setting  FillStrategy  to  << static_cast < std :: underlying_type < StretchType > :: type > ( _arg ) ) ; if ( this -> FillStrategy != _arg ) { this -> FillStrategy = _arg ; this -> Modified ( ) ; } } ; StretchType GetFillStrategy ( ) { return this -> FillStrategy ; } /@} protected : vtkChartMatrix ( ) ; ~ vtkChartMatrix ( ) override ;  The number of charts in x and y. vtkVector2i Size ;  The gutter between each chart. vtkVector2f Gutter ;  The padding used inside each chart float Padding ; std :: map < vtkVector2i , vtkVector2f > SpecificResize ; int Borders [ 4 ] ; bool LayoutIsDirty ;  The rectangular region to occupy. (in scene coordinates.) vtkRecti Rect = { 0 , 0 , 100 , 100 } ; StretchType FillStrategy = StretchType :: SCENE ; virtual void SynchronizeAxisRanges ( vtkObject * caller , unsigned long eventId , void * calldata ) ; private : vtkChartMatrix ( const vtkChartMatrix & ) = delete ; void operator = ( const vtkChartMatrix & ) = delete ; class PIMPL ; PIMPL * Private ; } ;
## Error: token expected: ; but got: [identifier]!!!
