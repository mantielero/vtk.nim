...
## !!!Ignored construct:  # vtkLine_h [NewLine] # vtkLine_h [NewLine] # vtkCell.h [NewLine] # vtkCommonDataModelModule.h  For export macro # vtkDeprecation.h  For VTK_DEPRECATED_IN_9_1_0 class vtkIncrementalPointLocator ;
## Error: did not expect [NewLine]!!!

## !!!Ignored construct:  [NewLine] class VTKCOMMONDATAMODEL_EXPORT vtkLine : public vtkCell { public : static vtkLine * New ( ) ; protected : const char * GetClassNameInternal ( ) const override { return thisClass ; } public : typedef vtkCell Superclass ; static vtkTypeBool IsTypeOf ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 1 ; } return vtkCell :: IsTypeOf ( type ) ; } vtkTypeBool IsA ( const char * type ) override { return this -> vtkLine :: IsTypeOf ( type ) ; } static vtkLine * SafeDownCast ( vtkObjectBase * o ) { if ( o && o -> IsA ( thisClass ) ) { return static_cast < vtkLine * > ( o ) ; } return nullptr ; } VTK_NEWINSTANCE vtkLine * NewInstance ( ) const { return vtkLine :: SafeDownCast ( this -> NewInstanceInternal ( ) ) ; } static vtkIdType GetNumberOfGenerationsFromBaseType ( const char * type ) { if ( ! strcmp ( thisClass , type ) ) { return 0 ; } return 1 + vtkCell :: GetNumberOfGenerationsFromBaseType ( type ) ; } vtkIdType GetNumberOfGenerationsFromBase ( const char * type ) override { return this -> vtkLine :: GetNumberOfGenerationsFromBaseType ( type ) ; } public : protected : vtkObjectBase * NewInstanceInternal ( ) const override { return vtkLine :: New ( ) ; } public : ; void PrintSelf ( ostream & os , vtkIndent indent ) override ; /@{ *
##  See the vtkCell API for descriptions of these methods.
##  int GetCellType ( ) override { return VTK_LINE ; } int GetCellDimension ( ) override { return 1 ; } int GetNumberOfEdges ( ) override { return 0 ; } int GetNumberOfFaces ( ) override { return 0 ; } vtkCell * GetEdge ( int ) override { return nullptr ; } vtkCell * GetFace ( int ) override { return nullptr ; } int CellBoundary ( int subId , const double pcoords [ 3 ] , vtkIdList * pts ) override ; void Contour ( double value , vtkDataArray * cellScalars , vtkIncrementalPointLocator * locator , vtkCellArray * verts , vtkCellArray * lines , vtkCellArray * polys , vtkPointData * inPd , vtkPointData * outPd , vtkCellData * inCd , vtkIdType cellId , vtkCellData * outCd ) override ; int EvaluatePosition ( const double x [ 3 ] , double closestPoint [ 3 ] , int & subId , double pcoords [ 3 ] , double & dist2 , double weights [ ] ) override ; void EvaluateLocation ( int & subId , const double pcoords [ 3 ] , double x [ 3 ] , double * weights ) override ; int Triangulate ( int index , vtkIdList * ptIds , vtkPoints * pts ) override ; void Derivatives ( int subId , const double pcoords [ 3 ] , const double * values , int dim , double * derivs ) override ; double * GetParametricCoords ( ) override ; /@} *
...
##  int IntersectWithLine ( const double p1 [ 3 ] , const double p2 [ 3 ] , double tol , double & t , double x [ 3 ] , double pcoords [ 3 ] , int & subId ) override ;  Return result type for Intersection() and Intersection3D() enum IntersectionType { NoIntersect = 0 , Intersect = 2 , OnLine = 3 } ;  Control the meaning of the provided tolerance.  Fuzzy tolerances allow
...
##  within the (0,1) parametric range (inclusive) enum ToleranceType { Relative = 0 , Absolute = 1 , RelativeFuzzy = 2 , AbsoluteFuzzy = 3 } ; *
...
##  VTK_DEPRECATED_IN_9_1_0 ( Use vtkLine::Intersection(...) instead. ) static int Intersection3D ( double p1 [ 3 ] , double p2 [ 3 ] , double x1 [ 3 ] , double x2 [ 3 ] , double & u , double & v , const double tolerance = 1e-6 ) ; *
...
##  static double DistanceBetweenLineSegments ( double l0 [ 3 ] , double l1 [ 3 ] , double m0 [ 3 ] , double m1 [ 3 ] , double closestPt1 [ 3 ] , double closestPt2 [ 3 ] , double & t1 , double & t2 ) ; static void InterpolationFunctions ( const double pcoords [ 3 ] , double weights [ 2 ] ) ; static void InterpolationDerivs ( const double pcoords [ 3 ] , double derivs [ 2 ] ) ; /@{ *
##  Compute the interpolation functions/derivatives
##  (aka shape functions/derivatives)
##  void InterpolateFunctions ( const double pcoords [ 3 ] , double weights [ 2 ] ) override { vtkLine :: InterpolationFunctions ( pcoords , weights ) ; } void InterpolateDerivs ( const double pcoords [ 3 ] , double derivs [ 2 ] ) override { vtkLine :: InterpolationDerivs ( pcoords , derivs ) ; } /@} protected : vtkLine ( ) ; ~ vtkLine ( ) override = default ; private : vtkLine ( const vtkLine & ) = delete ; void operator = ( const vtkLine & ) = delete ; } ;
## Error: did not expect [NewLine]!!!

## ----------------------------------------------------------------------------

proc GetParametricCenter*(pcoords: array[3, cdouble]): cint {.
    importcpp: "::GetParametricCenter(@)", header: "vtkLine.h".}