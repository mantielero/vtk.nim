...
import
  vtkDataArray, vtkGenericDataArray

## !!!Ignored construct:  # vtkDataArrayAccessor_h [NewLine] # vtkDataArrayAccessor_h [NewLine]  Generic form for all (non-bit) vtkDataArray subclasses. template < typename ArrayT > [end of template] struct vtkDataArrayAccessor { typedef ArrayT ArrayType ; typedef typename ArrayType :: ValueType APIType ; ArrayType * Array ; vtkDataArrayAccessor ( ArrayType * array ) : Array ( array ) { } VTK_ALWAYS_INLINE APIType Get ( vtkIdType tupleIdx , int compIdx ) const { return this -> Array -> GetTypedComponent ( tupleIdx , compIdx ) ; } VTK_ALWAYS_INLINE void Set ( vtkIdType tupleIdx , int compIdx , APIType val ) const { this -> Array -> SetTypedComponent ( tupleIdx , compIdx , val ) ; } VTK_ALWAYS_INLINE void Insert ( vtkIdType tupleIdx , int compIdx , APIType val ) const { this -> Array -> InsertTypedComponent ( tupleIdx , compIdx , val ) ; } VTK_ALWAYS_INLINE void Get ( vtkIdType tupleIdx , APIType * tuple ) const { this -> Array -> GetTypedTuple ( tupleIdx , tuple ) ; } VTK_ALWAYS_INLINE void Set ( vtkIdType tupleIdx , const APIType * tuple ) const { this -> Array -> SetTypedTuple ( tupleIdx , tuple ) ; } VTK_ALWAYS_INLINE void Insert ( vtkIdType tupleIdx , const APIType * tuple ) const { this -> Array -> InsertTypedTuple ( tupleIdx , tuple ) ; } } ;
## Error: identifier expected, but got: <!!!

##  Specialization for vtkDataArray.

## !!!Ignored construct:  template < > [end of template] struct vtkDataArrayAccessor < vtkDataArray > { typedef vtkDataArray ArrayType ; typedef double APIType ; ArrayType * Array ; vtkDataArrayAccessor ( ArrayType * array ) : Array ( array ) { } VTK_ALWAYS_INLINE APIType Get ( vtkIdType tupleIdx , int compIdx ) const { return this -> Array -> GetComponent ( tupleIdx , compIdx ) ; } VTK_ALWAYS_INLINE void Set ( vtkIdType tupleIdx , int compIdx , APIType val ) const { this -> Array -> SetComponent ( tupleIdx , compIdx , val ) ; } VTK_ALWAYS_INLINE void Insert ( vtkIdType tupleIdx , int compIdx , APIType val ) const { this -> Array -> InsertComponent ( tupleIdx , compIdx , val ) ; } VTK_ALWAYS_INLINE void Get ( vtkIdType tupleIdx , APIType * tuple ) const { this -> Array -> GetTuple ( tupleIdx , tuple ) ; } VTK_ALWAYS_INLINE void Set ( vtkIdType tupleIdx , const APIType * tuple ) const { this -> Array -> SetTuple ( tupleIdx , tuple ) ; } VTK_ALWAYS_INLINE void Insert ( vtkIdType tupleIdx , const APIType * tuple ) const { this -> Array -> InsertTuple ( tupleIdx , tuple ) ; } } ;
## Error: identifier expected, but got: <!!!
